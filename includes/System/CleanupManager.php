...
final class CleanupManager {
	...
	public function cleanup( array $params, array $actor = [] ): array|\WP_Error {
		...
		if ( empty( $resources ) ) {
			return new \WP_Error( 'wpcc_invalid_cleanup_resources', __( 'Select at least one cleanup resource.', 'siteradian' ) );
		}
		if ( ! $dry_run && 'production' === $mode && ( empty( $params['allow_production'] ) || 'DELETE PRODUCTION DATA' !== $confirm ) ) {
			...
			return new \WP_Error( 'wpcc_production_cleanup_blocked', __( 'Production cleanup requires allow_production=true and the exact confirmation phrase.', 'siteradian' ) );
		}
		if ( ! $dry_run && 'production' !== $mode && 'CLEANUP' !== $confirm ) {
			return new \WP_Error( 'wpcc_cleanup_confirmation_required', __( 'Live cleanup requires the confirmation phrase CLEANUP.', 'siteradian' ) );
		}
