...
final class RecommendationEngine {
	...
	public function scan( array $actor = [] ): array|\WP_Error {
		...
		try {
			...
			return new \WP_Error( 'wpcc_recommendation_scan_failed', __( 'Recommendation scan failed.', 'siteradian' ) );
		}
	}
...
// phpcs:enable WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter,WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery
	...
	public function transition( string $recommendation_id, string $status, array $actor = [] ): array|\WP_Error {
		...
		if ( ! $record ) {
			return new \WP_Error( 'wpcc_recommendation_not_found', __( 'Recommendation not found.', 'siteradian' ) );
		}
		$allowed_from = [
			'dismissed' => [ 'open' ],
		...
		if ( ! isset( $allowed_from[ $status ] ) || ! in_array( $record['status'], $allowed_from[ $status ], true ) ) {
			return new \WP_Error( 'wpcc_invalid_recommendation_status', __( 'The recommendation cannot make that status transition.', 'siteradian' ) );
		}
		...
		if ( false === $updated ) {
			return new \WP_Error( 'wpcc_recommendation_update_failed', __( 'Failed to update recommendation.', 'siteradian' ) );
		}
	...
	public function convert_to_action( string $recommendation_id, string $session_id, string $task_id, array $actor = [] ): array|\WP_Error {
		...
		if ( 'open' !== $record['status'] ) {
			return new \WP_Error( 'wpcc_invalid_recommendation_status', __( 'Only open recommendations can be converted to actions.', 'siteradian' ) );
		}
		...
		if ( $task['session_id'] !== $session_id ) {
			return new \WP_Error( 'wpcc_task_session_mismatch', __( 'The agent task does not belong to the supplied session.', 'siteradian' ) );
		}
		...
		if ( false === $inserted ) {
			return new \WP_Error( 'wpcc_action_create_failed', __( 'Failed to create the agent action.', 'siteradian' ) );
		}
		...
		if ( false === $updated ) {
			...
			return new \WP_Error( 'wpcc_recommendation_update_failed', __( 'Failed to link recommendation to action.', 'siteradian' ) );
		}
	...
	public function create_plan( string $recommendation_id, array $data, array $actor = [] ): array|\WP_Error {
		...
		if ( 'converted_to_action' !== $record['status'] || ! $record['action_id'] ) {
			return new \WP_Error( 'wpcc_invalid_recommendation_status', __( 'Only recommendations converted to an action can create a plan.', 'siteradian' ) );
		}

		$action = $this->get_action( $record['action_id'] );
		if ( ! $action ) {
			return new \WP_Error( 'wpcc_action_not_found', __( 'Linked agent action not found.', 'siteradian' ) );
		}
		...
		if ( '' === $title || '' === $objective || ! is_array( $steps ) || empty( $steps ) ) {
			return new \WP_Error( 'wpcc_invalid_plan', __( 'Plan title, objective, and at least one step are required.', 'siteradian' ) );
		}
		...
		foreach ( array_values( $steps ) as $index => $step ) {
			...
			if ( '' === $step_title ) {
				return new \WP_Error( 'wpcc_invalid_plan_step', __( 'Each plan step needs a title.', 'siteradian' ) );
			}
			$normalized_steps[] = [
				'step_order' => $index + 1,
				'title' => $step_title,
		...
		if ( false === $inserted ) {
			$wpdb->query( 'ROLLBACK' );
			return new \WP_Error( 'wpcc_plan_create_failed', __( 'Failed to create the recommendation plan.', 'siteradian' ) );
		}
		foreach ( $normalized_steps as $step ) {
			if ( false === $wpdb->insert( $wpdb->prefix . 'wpcc_agent_plan_steps', [ 'plan_id' => $plan_id ] + $step ) ) {
				$wpdb->query( 'ROLLBACK' );
				return new \WP_Error( 'wpcc_plan_step_create_failed', __( 'Failed to create a recommendation plan step.', 'siteradian' ) );
			}
		}
		$context = $record['context'] + [];
		...
		if ( false === $linked ) {
			$wpdb->query( 'ROLLBACK' );
			return new \WP_Error( 'wpcc_recommendation_update_failed', __( 'Failed to link recommendation to plan.', 'siteradian' ) );
		}
		$wpdb->query( 'COMMIT' );
	...
	private function upsert( array $candidate, array $actor ): array|\WP_Error {
		...
		if ( ! $existing ) {
			...
			if ( false === $inserted ) {
				return new \WP_Error( 'wpcc_recommendation_create_failed', __( 'Failed to create recommendation.', 'siteradian' ) );
			}
