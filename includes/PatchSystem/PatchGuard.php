	...
	public static function validate_change( string $relative_path, string $original, string $modified ): ?\WP_Error {
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Patch rejected: it would remove or invalidate the "%1$s" header of %2$s, which would deactivate the %3$s. Keep a valid "%1$s" header (within the first 8 KB) in the modified file.', 'siteradian' ),
				$header,
				$relative_path,
				$type
			),
