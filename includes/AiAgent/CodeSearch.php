...
final class CodeSearch {
	...
	public function search( string $query, array $args = [] ): array|\WP_Error {
		...
		if ( '' === $query ) {
			return new \WP_Error( 'wpcc_empty_query', __( 'Please enter a search term.', 'siteradian' ) );
		}
		...
		if ( ! in_array( $type, self::VALID_TYPES, true ) ) {
			return new \WP_Error( 'wpcc_invalid_type', __( 'Invalid search type. Use text, function, class, or hook.', 'siteradian' ) );
		}
	...
	private function resolve_search_roots( string $relative_path ): array|\WP_Error {
		...
		if ( ! is_dir( $real ) ) {
			return new \WP_Error( 'wpcc_not_a_directory', __( 'The search path must be a directory.', 'siteradian' ) );
		}
	...
	private function resolve_scope( string $relative_path ): array|\WP_Error {
		...
		return new \WP_Error( 'wpcc_not_found', __( 'The search path does not exist.', 'siteradian' ) );
	}
