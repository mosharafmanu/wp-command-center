...
final class RestApi {
	...
	private function check_token( \WP_REST_Request $request, string $required_scope ): bool|\WP_Error {
		...
		if ( '' === $raw ) {
			return new \WP_Error(
				'wpcc_missing_token',
				__( 'Missing API token. Provide an "Authorization: Bearer <token>" header.', 'siteradian' ),
				[ 'status' => 401 ]
			);
		}
		...
		if ( AuthTokens::SCOPE_FULL === $required_scope && AuthTokens::SCOPE_FULL !== $record['scope'] ) {
			return new \WP_Error(
				...
				__( 'This API token is read-only and cannot perform this action.', 'siteradian' ),
				[ 'status' => 403 ]
			);
		}
	...
	public function get_agent_context( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( '' !== $session_id ) {
			...
			if ( null === $session ) {
				return $this->with_status( new \WP_Error( 'wpcc_session_not_found', __( 'Agent session not found.', 'siteradian' ) ) );
			}
	...
	public function list_recommendations( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( $filters['type'] && ! in_array( $filters['type'], RecommendationEngine::TYPES, true ) ) {
			return $this->with_status( new \WP_Error( 'wpcc_invalid_type', __( 'Invalid recommendation type.', 'siteradian' ) ) );
		}
		if ( $filters['severity'] && ! in_array( $filters['severity'], RecommendationEngine::SEVERITIES, true ) ) {
			return $this->with_status( new \WP_Error( 'wpcc_invalid_recommendation_severity', __( 'Invalid recommendation severity.', 'siteradian' ) ) );
		}
		if ( $filters['status'] && ! in_array( $filters['status'], RecommendationEngine::STATUSES, true ) ) {
			return $this->with_status( new \WP_Error( 'wpcc_invalid_recommendation_status', __( 'Invalid recommendation status.', 'siteradian' ) ) );
		}
	...
	public function get_recommendation( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $record ) {
			return $this->with_status( new \WP_Error( 'wpcc_recommendation_not_found', __( 'Recommendation not found.', 'siteradian' ) ) );
		}
	...
	private function run_recommendation_action( \WP_REST_Request $request, string $action ): \WP_REST_Response|\WP_Error {
		...
		if ( 'convert-to-action' === $action ) {
			...
			$result = new \WP_Error( 'wpcc_invalid_action', __( 'Invalid recommendation action.', 'siteradian' ) );
		}
	...
	public function get_operation( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( null === $operation ) {
			return $this->with_status( new \WP_Error( 'wpcc_operation_not_found', __( 'Operation not found.', 'siteradian' ) ) );
		}
	...
	public function get_operation_request( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $row ) {
			return $this->with_status( new \WP_Error( 'wpcc_request_not_found', __( 'Operation request not found.', 'siteradian' ) ) );
		}
	...
	public function create_operation_request( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( empty( $op_id ) ) {
			return $this->with_status( new \WP_Error( 'wpcc_missing_operation_id', __( 'Operation ID is required.', 'siteradian' ) ) );
		}
	...
	public function get_queue_item( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $item ) {
			return $this->with_status( new \WP_Error( 'wpcc_queue_item_not_found', __( 'Queue item not found.', 'siteradian' ) ) );
		}
	...
	public function get_operation_result( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $result ) {
			return $this->with_status( new \WP_Error( 'wpcc_result_not_found', __( 'Operation result not found.', 'siteradian' ) ) );
		}
		...
		switch ( $type ) {
			...
			default:
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_type',
					__( 'Invalid diagnostics type. Use performance, security, or woocommerce.', 'siteradian' )
				) );
		}
	...
	public function read_file( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		return $this->catch_file_access( function () use ( $request ) {
			...
			if ( '' === $path ) {
				return $this->with_status( new \WP_Error( 'wpcc_missing_path', __( 'The path parameter is required.', 'siteradian' ) ) );
			}
	...
	public function create_agent_session( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! in_array( $source, self::SESSION_SOURCES, true ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Invalid session source. Use claude, codex, gpt, api, or manual.', 'siteradian' )
			) );
		}
		...
		if ( '' === $label ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'The session label is required.', 'siteradian' )
			) );
		}
		...
		if ( $expires_at <= $now ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'The session expiry must be a future Unix timestamp.', 'siteradian' )
			) );
		}
		...
		if ( false === $inserted ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to create the agent session.', 'siteradian' )
			), 500 );
		}
	...
	public function close_agent_session( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( self::SESSION_STATUS_ACTIVE !== $session['status'] ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Only active agent sessions can be closed.', 'siteradian' )
			) );
		}
		...
		if ( false === $updated ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to close the agent session.', 'siteradian' )
			), 500 );
		}
	...
	public function create_agent_task( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( null === $this->find_agent_session( $session_id ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Agent session not found.', 'siteradian' )
			) );
		}
		...
		if ( ! in_array( $source, self::SESSION_SOURCES, true ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Invalid task source. Use claude, codex, gpt, api, or manual.', 'siteradian' )
			) );
		}
		...
		if ( '' === $user_prompt ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'The user prompt is required.', 'siteradian' )
			) );
		}
		...
		if ( false === $inserted ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to create the agent task.', 'siteradian' )
			), 500 );
		}
	...
	public function get_agent_task( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( null === $task ) {
			return $this->with_status( new \WP_Error( 'wpcc_task_not_found', __( 'Agent task not found.', 'siteradian' ) ) );
		}
	...
	public function update_agent_task_status( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! in_array( $status, self::TASK_STATUSES, true ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Invalid task status.', 'siteradian' )
			) );
		}
		...
		if ( false === $updated ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to update the agent task.', 'siteradian' )
			), 500 );
		}
	...
	public function create_agent_action( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! in_array( $type, self::ACTION_TYPES, true ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Invalid action type. Use investigate, recommendation, diagnosis, code_change, configuration_change, or maintenance.', 'siteradian' )
			) );
		}
		...
		if ( '' === $title ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'The action title is required.', 'siteradian' )
			) );
		}
		...
		if ( false === $inserted ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to create the agent action.', 'siteradian' )
			), 500 );
		}
	...
	public function get_agent_action( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( null === $action ) {
			return $this->with_status( new \WP_Error( 'wpcc_action_not_found', __( 'Agent action not found.', 'siteradian' ) ) );
		}
	...
	private function run_agent_action_transition( \WP_REST_Request $request, string $transition ): \WP_REST_Response|\WP_Error {
		...
		if ( 'accept' === $transition ) {
			if ( ! in_array( $action['status'], self::ACTION_ACCEPTABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					...
					__( 'Only proposed actions can be accepted.', 'siteradian' )
				) );
			}
			...
			if ( ! in_array( $action['status'], self::ACTION_REJECTABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_action_status',
					__( 'Only proposed actions can be rejected.', 'siteradian' )
				) );
			}
			...
			if ( ! in_array( $action['status'], self::ACTION_CANCELLABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_action_status',
					__( 'Only proposed or accepted actions can be cancelled.', 'siteradian' )
				) );
			}
			...
			if ( ! in_array( $action['status'], self::ACTION_COMPLETABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_action_status',
					__( 'Only accepted actions can be completed.', 'siteradian' )
				) );
			}
			...
			return $this->with_status( new \WP_Error( 'wpcc_invalid_agent_action', __( 'Invalid agent action.', 'siteradian' ) ) );
		}
		...
		if ( false === $updated ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to update the agent action.', 'siteradian' )
			), 500 );
		}
	...
	public function create_agent_plan( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( '' === $title || '' === $objective ) {
			return $this->with_status( new \WP_Error(
				'wpcc_invalid_plan',
				__( 'Plan title and objective are required.', 'siteradian' )
			) );
		}
		...
		if ( ! in_array( $status, self::PLAN_CREATE_STATUSES, true ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'New plans must be draft or pending_review.', 'siteradian' )
			) );
		}
		...
		if ( ! is_array( $steps ) || empty( $steps ) ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'A plan must include at least one step.', 'siteradian' )
			) );
		}
		...
		foreach ( array_values( $steps ) as $index => $step ) {
			...
			if ( '' === $step_title || ! in_array( $step_status, self::PLAN_STEP_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					...
					__( 'Each plan step needs a title and a valid status.', 'siteradian' )
				) );
			}
		...
		if ( false === $inserted ) {
			...
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to create the agent plan.', 'siteradian' )
			), 500 );
		}
		...
		foreach ( $normalized_steps as $step ) {
			...
			if ( false === $step_inserted ) {
				...
				return $this->with_status( new \WP_Error(
					...
					__( 'Failed to create an agent plan step.', 'siteradian' )
				), 500 );
			}
		}
	...
	public function get_agent_plan( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( null === $plan ) {
			return $this->with_status( new \WP_Error( 'wpcc_plan_not_found', __( 'Agent plan not found.', 'siteradian' ) ) );
		}
	...
	private function run_agent_plan_action( \WP_REST_Request $request, string $action ): \WP_REST_Response|\WP_Error {
		...
		if ( 'approve' === $action ) {
			if ( ! in_array( $plan['status'], self::PLAN_APPROVABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_plan_status',
					__( 'Only pending_review or draft plans can be approved.', 'siteradian' )
				) );
			}
			...
			if ( ! in_array( $plan['status'], self::PLAN_REJECTABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_plan_status',
					__( 'Only pending_review or draft plans can be rejected.', 'siteradian' )
				) );
			}
			...
			if ( ! in_array( $plan['status'], self::PLAN_CANCELLABLE_STATUSES, true ) ) {
				return $this->with_status( new \WP_Error(
					'wpcc_invalid_plan_status',
					__( 'Only draft, pending_review, or approved plans can be cancelled.', 'siteradian' )
				) );
			}
			...
			return $this->with_status( new \WP_Error( 'wpcc_invalid_plan_action', __( 'Invalid plan action.', 'siteradian' ) ) );
		}
		...
		if ( false === $updated ) {
			return $this->with_status( new \WP_Error(
				...
				__( 'Failed to update the agent plan.', 'siteradian' )
			), 500 );
		}
	...
	private function validate_session_task_relationship( string $session_id, string $task_id ): bool|\WP_Error {
		...
		if ( null === $session ) {
			return new \WP_Error( 'wpcc_session_not_found', __( 'Agent session not found.', 'siteradian' ) );
		}

		$task = $this->find_agent_task( $task_id );

		if ( null === $task ) {
			return new \WP_Error( 'wpcc_task_not_found', __( 'Agent task not found.', 'siteradian' ) );
		}

		if ( $task['session_id'] !== $session_id ) {
			return new \WP_Error(
				'wpcc_task_session_mismatch',
				__( 'The agent task does not belong to the supplied session.', 'siteradian' )
			);
		}
	...
	public function create_patch( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! is_array( $files ) || empty( $files ) ) {
			return $this->with_status( new \WP_Error(
				'wpcc_no_files',
				__( 'The "files" parameter must be a non-empty array of {path, modified}.', 'siteradian' )
			) );
		}
	...
	private function run_patch_action( \WP_REST_Request $request, string $action ): \WP_REST_Response|\WP_Error {
		...
		$result = match ( $action ) {
			...
			default    => new \WP_Error( 'wpcc_invalid_action', __( 'Invalid action.', 'siteradian' ) ),
		};
	...
	public function get_ai_client_config( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! $client ) {
			return $this->with_status( new \WP_Error(
				...
				sprintf( /* translators: %s: value */ __( 'AI Client "%s" not found.', 'siteradian' ), $client_id ),
			), 404 );
		}
		...
		if ( ! $client['config_generator'] ) {
			return $this->with_status( new \WP_Error(
				...
				sprintf( /* translators: %s: value */ __( 'Configuration generator not yet implemented for "%s".', 'siteradian' ), $client['name'] ),
			), 501 );
		}
