...
final class SnapshotManager {
	...
	public function create( string $relative_path, string $label = '', ?string $patch_id = null ): array|\WP_Error {
		...
		if ( $fsize > $max ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'File is too large to snapshot (%1$d bytes > %2$d limit). Set WPCC_SNAPSHOT_MAX_BYTES to raise the cap.', 'siteradian' ),
					$fsize,
					$max
				)
			);
		...
		if ( false === $contents ) {
			return new \WP_Error( 'wpcc_read_failed', __( 'Failed to read the file.', 'siteradian' ) );
		}
		if ( microtime( true ) > $deadline ) {
			return new \WP_Error( 'wpcc_snapshot_timeout', __( 'Snapshot read exceeded the time budget.', 'siteradian' ) );
		}
		...
		if ( false === file_put_contents( $tmp, $contents ) ) {
			return new \WP_Error( 'wpcc_write_failed', __( 'Failed to store the snapshot.', 'siteradian' ) );
		}
		...
		if ( ! @rename( $tmp, $dest ) ) {
			wp_delete_file( $tmp );
			return new \WP_Error( 'wpcc_write_failed', __( 'Failed to finalize the snapshot.', 'siteradian' ) );
		}
	...
	public function get( string $id ): array|\WP_Error {
		...
		if ( ! is_array( $row ) ) {
			return new \WP_Error( 'wpcc_snapshot_not_found', __( 'Snapshot not found.', 'siteradian' ) );
		}
	...
	public function get_contents( string $id ): string|\WP_Error {
		...
		if ( ! is_readable( $file ) ) {
			return new \WP_Error( 'wpcc_snapshot_missing', __( 'Snapshot file is missing on disk.', 'siteradian' ) );
		}
	...
	private function get_storage_dir(): string|\WP_Error {
		return \WPCommandCenter\Security\PrivateStore::dir(
			self::DIR_NAME,
			__( 'Failed to create the snapshot storage directory.', 'siteradian' )
		);
	}
