...
if ( isset( $_POST['wpcc_sr_action'] ) && check_admin_referer( 'wpcc_sr_action' ) && current_user_can( 'manage_options' ) ) {
	...
	if ( '' === $search || empty( $tables ) ) {
		$sr_error = __( 'Search string and at least one table are required.', 'siteradian' );
	} elseif ( ! $dry_run && ! $confirmed ) {
		$sr_error = __( 'Please confirm the Search & Replace request in the confirmation dialog before it is created, or enable Dry Run.', 'siteradian' );
	} else {
		$payload = [
			'search'         => $search,
			'replace'        => $replace,
		...
		if ( ! is_wp_error( $req ) ) {
			if ( $dry_run ) {
				...
				if ( $q_item ) {
					...
					if ( is_wp_error( $sr_result ) ) {
						...
						if ( ! empty( $sr_result['result']['errors'] ) ) {
							$sr_error = $sr_result['result']['errors'][0]['message'] ?? __( 'Dry run failed.', 'siteradian' );
						} else {
							$sr_preview = [
								'search'          => $search,
								'replace'         => $replace,
				...
				$sr_success_msg = sprintf(
					...
					__( 'Live Search & Replace request "%s" created and is pending review. Approve and run it from Approvals, or wait for the background worker.', 'siteradian' ),
					$req['request_id']
				);
			}
		} else {
...
<div class="wpcc-tools-wrap">
	<h1><?php esc_html_e( 'Tools', 'siteradian' ); ?></h1>
	<p class="description" style="max-width:680px;">
		<?php esc_html_e( 'Governed maintenance tools. Each runs through the same engine as everything else: changes are previewed, approved when your protection mode requires it, and audited. A database search and replace rewrites rows in place and cannot be undone.', 'siteradian' ); ?>
	</p>
	<?php
	/*
	...
	<div class="wpcc-tools-panel">
		<h2 class="wpcc-tools-panel-header"><?php esc_html_e( 'Safe Search & Replace', 'siteradian' ); ?></h2>
		<div class="wpcc-tools-panel-body">
			<p class="description" style="margin-top:0;"><?php esc_html_e( 'Find and replace text across database tables (for example after a domain change). Preview safely with Dry Run; a live run creates a governed request you approve under Approvals.', 'siteradian' ); ?></p>
			<div class="wpcc-sr-grid">
				<div class="wpcc-sr-form">
					<form method="post" id="wpcc-sr-form">
						...
						<p>
							<label for="wpcc-sr-search"><strong><?php esc_html_e( 'Search For:', 'siteradian' ); ?></strong></label>
						...
						<p>
							<label for="wpcc-sr-replace"><strong><?php esc_html_e( 'Replace With:', 'siteradian' ); ?></strong></label>
						...
						<p>
							<label for="wpcc-sr-preset"><strong><?php esc_html_e( 'Table Preset:', 'siteradian' ); ?></strong></label>
							<select id="wpcc-sr-preset">
								<option value=""><?php esc_html_e( '— Select a preset —', 'siteradian' ); ?></option>
								<option value="content"><?php esc_html_e( 'Content Tables', 'siteradian' ); ?></option>
								<option value="content_meta"><?php esc_html_e( 'Content + Meta', 'siteradian' ); ?></option>
								<option value="options"><?php esc_html_e( 'Options', 'siteradian' ); ?></option>
								<option value="all"><?php esc_html_e( 'All WordPress Content', 'siteradian' ); ?></option>
								<option value="custom"><?php esc_html_e( 'Custom Selection', 'siteradian' ); ?></option>
							</select>
						</p>
						<?php
						/*
						...
						?>
						<p>
							<span><strong><?php esc_html_e( 'Target Tables:', 'siteradian' ); ?></strong></span>
						...
						<details class="wpcc-sr-advanced" id="wpcc-sr-advanced">
							<summary><?php esc_html_e( 'Choose specific tables', 'siteradian' ); ?></summary>
							<p class="description" style="margin:8px 0;">
								<?php esc_html_e( 'Most jobs are covered by a preset above. Pick individual tables only if you know why you need them.', 'siteradian' ); ?>
							</p>
						...
						<p>
							<label><input type="checkbox" name="dry_run" id="wpcc-sr-dry-run" value="1" <?php checked( $wpcc_dry_run_checked ); ?>> <?php esc_html_e( 'Dry Run (Preview changes only)', 'siteradian' ); ?></label>
						</p>
						<p>
							<?php esc_html_e( 'Computed Risk Level:', 'siteradian' ); ?>
						...
						<p class="wpcc-sr-gate" id="wpcc-sr-gate-note" role="note" hidden>
							<?php esc_html_e( 'Run a Dry Preview first. At this risk level the preview is what tells you how many rows a live run would rewrite — tick Dry Run, run it, then come back.', 'siteradian' ); ?>
						</p>
						<p>
							<button type="submit" name="wpcc_sr_action" value="run" id="wpcc-sr-submit-btn" class="button button-primary"><?php esc_html_e( 'Run Dry Preview', 'siteradian' ); ?></button>
						</p>
						<noscript>
							<p style="color: #d63638;"><?php esc_html_e( 'JavaScript is required to create a live Search & Replace request (a confirmation dialog is shown first). Dry Run previews work without JavaScript.', 'siteradian' ); ?></p>
						</noscript>
					</form>
				</div>
				<div>
					<?php if ( $sr_preview ) : ?>
						<div class="wpcc-tools-panel" style="margin-top: 10px; border-color: #2271b1;">
							<h3 class="wpcc-tools-panel-header" style="font-size: 14px; padding: 10px 15px;"><?php esc_html_e( 'Dry Run Preview', 'siteradian' ); ?></h3>
							<div class="wpcc-tools-panel-body" style="padding: 15px;">
								<table class="wpcc-sr-preview-table">
									<tr><td><?php esc_html_e( 'Matches Found:', 'siteradian' ); ?></td><td><?php echo esc_html( (string) $sr_preview['matches_found'] ); ?></td></tr>
									<tr><td><?php esc_html_e( 'Affected Rows:', 'siteradian' ); ?></td><td><?php echo esc_html( (string) $sr_preview['rows_affected'] ); ?></td></tr>
									<tr><td><?php esc_html_e( 'Affected Tables:', 'siteradian' ); ?></td><td>
										<?php echo $sr_preview['tables_affected'] ? esc_html( implode( ', ', $sr_preview['tables_affected'] ) ) : esc_html__( 'None — no matches in the selected tables.', 'siteradian' ); ?>
									</td></tr>
									<tr><td><?php esc_html_e( 'Risk Level:', 'siteradian' ); ?></td><td><span class="wpcc-risk-badge wpcc-risk-<?php echo esc_attr( $sr_preview['risk_level'] ); ?>"><?php echo esc_html( strtoupper( $sr_preview['risk_level'] ) ); ?></span></td></tr>
								</table>
					...
					<?php else : ?>
						<div style="background: #f6f7f7; border: 1px solid #dcdcde; border-radius: 8px; padding: 20px; text-align: center; color: #646970;">
							<?php esc_html_e( 'Enter search parameters, choose tables, and click "Run Dry Preview" to see matches found, affected rows, affected tables, and the computed risk level.', 'siteradian' ); ?>
						</div>
					<?php endif; ?>
				</div>
			</div>
	...
	<div class="wpcc-modal-overlay" id="wpcc-sr-confirm-overlay">
		<div class="wpcc-modal">
			<h3><?php esc_html_e( 'Confirm Search & Replace Request', 'siteradian' ); ?></h3>
			<table>
				<tr><th><?php esc_html_e( 'Search For', 'siteradian' ); ?></th><td id="wpcc-confirm-search"></td></tr>
				<tr><th><?php esc_html_e( 'Replace With', 'siteradian' ); ?></th><td id="wpcc-confirm-replace"></td></tr>
				<tr><th><?php esc_html_e( 'Affected Tables', 'siteradian' ); ?></th><td id="wpcc-confirm-tables"></td></tr>
				<tr><th><?php esc_html_e( 'Affected Rows', 'siteradian' ); ?></th><td id="wpcc-confirm-rows"></td></tr>
				<tr><th><?php esc_html_e( 'Risk Level', 'siteradian' ); ?></th><td><span id="wpcc-confirm-risk" class="wpcc-risk-badge"></span></td></tr>
			</table>
			<p><?php esc_html_e( 'This creates a pending operation request only — no data changes until it is approved and executed from Approvals, or by the background worker.', 'siteradian' ); ?></p>
			<div class="wpcc-modal-actions">
				<button type="button" class="button" id="wpcc-sr-confirm-cancel"><?php esc_html_e( 'Cancel', 'siteradian' ); ?></button>
				<button type="submit" class="button button-primary" name="wpcc_sr_action" value="run" form="wpcc-sr-form" id="wpcc-sr-confirm-submit"><?php esc_html_e( 'Confirm & Create Request', 'siteradian' ); ?></button>
			</div>
		</div>
	</div>
	...
	( function () {
		...
		var LABEL_DRY_RUN   = <?php echo wp_json_encode( __( 'Run Dry Preview', 'siteradian' ) ); ?>;
		var LABEL_LIVE_RUN  = <?php echo wp_json_encode( __( 'Create Replace Request', 'siteradian' ) ); ?>;
		var LABEL_NONE      = <?php echo wp_json_encode( __( 'None selected', 'siteradian' ) ); ?>;
		/* translators: %1$d: number of database tables selected. */
		var LABEL_SELECTED  = <?php echo wp_json_encode( /* translators: %1$d: number */ __( '%1$d table(s) selected', 'siteradian' ) ); ?>;
		var LABEL_NONE_SELECTED = <?php echo wp_json_encode( __( 'No tables selected yet — pick a preset above.', 'siteradian' ) ); ?>;
		var LABEL_FROM_PREVIEW = <?php echo wp_json_encode( ' ' . __( '(from last Dry Preview)', 'siteradian' ) ); ?>;
		var LABEL_UNKNOWN_ROWS = <?php echo wp_json_encode( __( 'Unknown — run "Run Dry Preview" first for an exact count.', 'siteradian' ) ); ?>;
