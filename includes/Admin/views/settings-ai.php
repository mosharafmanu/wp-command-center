...
<div class="wpcc-settings-ai">
	<h1><?php esc_html_e( 'Built-in AI', 'siteradian' ); ?></h1>
	...
	<div class="wpcc-ai-paths">
		<div class="wpcc-ai-path">
			<span class="wpcc-ai-path__tag"><?php esc_html_e( 'Path 1 — the main way', 'siteradian' ); ?></span>
			<strong><?php esc_html_e( 'Your own AI assistant', 'siteradian' ); ?></strong>
			<p><?php
				printf(
					/* translators: %s: the mode-aware guarantee sentence. */
					esc_html__( 'You work in Claude, Cursor or ChatGPT and ask it to change this site. It brings its own AI, so no key is needed here. %s', 'siteradian' ),
					esc_html( \WPCommandCenter\Operations\SecurityModeManager::promise() )
				);
			?></p>
			<span class="wpcc-ai-path__note"><?php esc_html_e( 'No provider key required', 'siteradian' ); ?></span>
		</div>
		<div class="wpcc-ai-path">
			<span class="wpcc-ai-path__tag"><?php esc_html_e( 'Path 2 — optional extra', 'siteradian' ); ?></span>
			<strong><?php esc_html_e( 'Built-in AI (this screen)', 'siteradian' ); ?></strong>
			<p><?php esc_html_e( 'The plugin generates text itself — SEO descriptions, image alt text, draft content — without you opening an assistant. This is the part that needs your own provider key, and only for the tools you switch on.', 'siteradian' ); ?></p>
			<span class="wpcc-ai-path__note"><?php esc_html_e( 'Needs a provider key', 'siteradian' ); ?></span>
		</div>
	</div>
	<p class="description" style="max-width:680px;margin:0 0 18px;">
		...
		printf(
			/* translators: %s: the mode-aware guarantee sentence. */
			esc_html__( 'Both paths obey the same rules. %s', 'siteradian' ),
			esc_html( \WPCommandCenter\Operations\SecurityModeManager::promise() )
		);
		?>
	</p>
	...
	<?php if ( $wpcc_ai_tools_off ) : ?>
		<p class="wpcc-builtin-note" role="note" style="margin:14px 0;padding:10px 14px;background:#f0f6fc;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;max-width:680px;font-size:13px;color:#1d2327;">
			<?php esc_html_e( 'The SEO, Alt Text, and Content tools are not switched on for this site. They appear here once enabled — adding a provider key on its own does not turn them on.', 'siteradian' ); ?>
		</p>
	<?php else : ?>
		<nav class="wpcc-cds-subnav" aria-label="<?php esc_attr_e( 'Built-in AI sections', 'siteradian' ); ?>">
