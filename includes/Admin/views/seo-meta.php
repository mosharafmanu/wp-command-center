...
<div class="wrap wpcc-wrap wpcc-seo">
	<h1><?php esc_html_e( 'SEO', 'siteradian' ); ?></h1>
	<p class="description" style="max-width:720px;">
		<?php esc_html_e( 'Generate clear SEO titles and descriptions for posts and pages that need them. Review each suggestion, then approve to apply — nothing changes until you say so.', 'siteradian' ); ?>
	</p>
	<?php require WPCC_PLUGIN_DIR . 'includes/Admin/views/partials/trust-strip.php'; ?>

	<h2 class="nav-tab-wrapper">
		<a href="#" class="nav-tab nav-tab-active" id="wpcc-seo-tab-review"><?php esc_html_e( 'Review', 'siteradian' ); ?><span class="wpcc-seo-tabcount" id="wpcc-seo-tabcount-review"></span></a>
		<a href="#" class="nav-tab" id="wpcc-seo-tab-suggestions"><?php esc_html_e( 'Suggestions', 'siteradian' ); ?><span class="wpcc-seo-tabcount" id="wpcc-seo-tabcount-suggestions"></span></a>
		<a href="#" class="nav-tab" id="wpcc-seo-tab-applied"><?php esc_html_e( 'Applied', 'siteradian' ); ?><span class="wpcc-seo-tabcount" id="wpcc-seo-tabcount-applied"></span></a>
	</h2>
	...
	<div class="wpcc-seo-filters" id="wpcc-seo-controls" style="display:none;">
		<label for="wpcc-seo-filter"><?php esc_html_e( 'Show:', 'siteradian' ); ?></label>
		<select id="wpcc-seo-filter">
			<option value="missing"><?php esc_html_e( 'Missing', 'siteradian' ); ?></option>
			<option value="weak"><?php esc_html_e( 'Weak', 'siteradian' ); ?></option>
			<option value="all"><?php esc_html_e( 'All content', 'siteradian' ); ?></option>
		</select>
		...
		<label><input type="checkbox" id="wpcc-seo-selectall"> <?php esc_html_e( 'Select all on this page', 'siteradian' ); ?></label>
		<button type="button" class="button button-primary" id="wpcc-seo-generate" disabled><?php esc_html_e( 'Generate suggestions', 'siteradian' ); ?></button>
		<span class="description"><?php
			/* translators: %d: max per generation */
			printf( esc_html__( 'Up to %d at a time. Suggestions are drafts — nothing is applied.', 'siteradian' ), 25 );
		?></span>
	...
	<div id="wpcc-seo-panel-suggestions" style="display:none;">
		<p style="margin:12px 0;">
			<span class="description"><?php esc_html_e( 'AI suggestions awaiting your review. Edit the title or description, dismiss a suggestion, or apply individually or in bulk. Nothing is applied to your site until you choose to.', 'siteradian' ); ?></span>
		...
		<div class="wpcc-seo-bulkbar" id="wpcc-seo-sg-bulkbar">
			<label><input type="checkbox" id="wpcc-seo-sg-selectall"> <?php esc_html_e( 'Select all on this page', 'siteradian' ); ?></label>
			<button type="button" class="button button-primary" id="wpcc-seo-sg-apply" disabled><?php esc_html_e( 'Apply selected', 'siteradian' ); ?></button>
			<button type="button" class="button" id="wpcc-seo-sg-dismiss" disabled><?php esc_html_e( 'Dismiss selected', 'siteradian' ); ?></button>
		</div>
		...
		<table class="widefat striped wpcc-seo-sg-table">
			<thead>
				<tr>
					<th scope="col" style="width:28px;"><span class="screen-reader-text"><?php esc_html_e( 'Select', 'siteradian' ); ?></span></th>
					...
					<th scope="col" style="width:130px;"><?php esc_html_e( 'Actions', 'siteradian' ); ?></th>
				</tr>
			</thead>
	...
	<div id="wpcc-seo-panel-applied" style="display:none;">
		<p style="margin:12px 0;">
			<span class="description"><?php esc_html_e( 'Applied SEO descriptions and items awaiting approval.', 'siteradian' ); ?></span>
		...
		<div class="wpcc-seo-segbar" id="wpcc-seo-ap-segbar" role="group" aria-label="<?php esc_attr_e( 'Filter applied items by status', 'siteradian' ); ?>">
			<?php
			/*
			 ...
			 */
			?>
			<button type="button" class="button button-primary wpcc-seo-seg" data-seg="applied"><?php esc_html_e( 'Applied', 'siteradian' ); ?><span class="wpcc-seo-segcount" id="wpcc-seo-segcount-applied"></span></button>
			<button type="button" class="button wpcc-seo-seg" data-seg="pending_approval"><?php esc_html_e( 'Awaiting approval', 'siteradian' ); ?><span class="wpcc-seo-segcount" id="wpcc-seo-segcount-pending_approval"></span></button>
			<button type="button" class="button wpcc-seo-seg" data-seg="failed"><?php esc_html_e( 'Failed', 'siteradian' ); ?><span class="wpcc-seo-segcount" id="wpcc-seo-segcount-failed"></span></button>
		</div>
		<table class="widefat striped wpcc-seo-sg-table">
			<thead>
				<tr>
					<th scope="col" style="width:26%;"><?php esc_html_e( 'Content', 'siteradian' ); ?></th>
					<th scope="col"><?php esc_html_e( 'Applied SEO meta', 'siteradian' ); ?></th>
			...
			<tbody id="wpcc-seo-ap-rows">
				<tr><td colspan="4"><?php esc_html_e( 'Loading…', 'siteradian' ); ?></td></tr>
			</tbody>
		</table>
...
( function () {
	...
	const STR = {
		...
		empty:    <?php echo wp_json_encode( esc_html__( 'No content in this view. 🎉', 'siteradian' ) ); ?>,
		noPlugin: <?php echo wp_json_encode( esc_html__( 'No supported SEO plugin (Rank Math or Yoast SEO) is active. Activate one to audit SEO meta.', 'siteradian' ) ); ?>,
		none:     <?php echo wp_json_encode( esc_html__( '(not set)', 'siteradian' ) ); ?>,
		missing:  <?php echo wp_json_encode( esc_html__( 'Missing', 'siteradian' ) ); ?>,
		weak:     <?php echo wp_json_encode( esc_html__( 'Weak', 'siteradian' ) ); ?>,
		ok:       <?php echo wp_json_encode( esc_html__( 'OK', 'siteradian' ) ); ?>,
		colPost:  <?php echo wp_json_encode( esc_html__( 'Content', 'siteradian' ) ); ?>,
		colTitle: <?php echo wp_json_encode( esc_html__( 'SEO title', 'siteradian' ) ); ?>,
		colDesc:  <?php echo wp_json_encode( esc_html__( 'Meta description', 'siteradian' ) ); ?>,
		colState: <?php echo wp_json_encode( esc_html__( 'State', 'siteradian' ) ); ?>,
		colScore: <?php echo wp_json_encode( esc_html__( 'Score', 'siteradian' ) ); ?>,
		edit:     <?php echo wp_json_encode( esc_html__( 'Edit', 'siteradian' ) ); ?>,
		optimized:<?php echo wp_json_encode( esc_html__( 'Optimized', 'siteradian' ) ); ?>,
		missingN: <?php echo wp_json_encode( esc_html__( 'Missing meta', 'siteradian' ) ); ?>,
		weakN:    <?php echo wp_json_encode( esc_html__( 'Weak meta', 'siteradian' ) ); ?>,
		totalN:   <?php echo wp_json_encode( esc_html__( 'Total content', 'siteradian' ) ); ?>,
		// U3 — action-first dashboard.
		dashNeedsYou:  <?php echo wp_json_encode( esc_html__( 'Needs you', 'siteradian' ) ); ?>,
		dashHealthy:   <?php echo wp_json_encode( esc_html__( 'Healthy', 'siteradian' ) ); ?>,
		stMissing:     <?php echo wp_json_encode( esc_html__( 'Missing', 'siteradian' ) ); ?>,
		stNeedsWork:   <?php echo wp_json_encode( esc_html__( 'Needs work', 'siteradian' ) ); ?>,
		stOptimized:   <?php echo wp_json_encode( esc_html__( 'Optimized', 'siteradian' ) ); ?>,
		dashSugReady:  <?php echo wp_json_encode( esc_html__( 'suggestions ready', 'siteradian' ) ); ?>,
		dashApplied:   <?php echo wp_json_encode( esc_html__( 'applied (reversible)', 'siteradian' ) ); ?>,
		/* translators: %d: optimized percentage (literal percent sign follows) */
		dashPct:       <?php echo wp_json_encode( /* translators: %d: number */ __( '%d% optimized', 'siteradian' ) ); ?>,
		/* translators: %d: number of published items */
		dashPublished: <?php echo wp_json_encode( /* translators: %d: number */ __( '%d published', 'siteradian' ) ); ?>,
		// U1.2 — Generate → Suggestions handoff.
		/* translators: %d: suggestions created */
		viewSug:       <?php echo wp_json_encode( /* translators: %d: number */ __( 'Review %d suggestions →', 'siteradian' ) ); ?>,
		// U1.4 — no AI provider connected.
		noKey:         <?php echo wp_json_encode( esc_html__( 'Built-in AI has no provider key yet, so nothing was generated and nothing on your site changed. Add a key on Built-in AI › Providers, then try again.', 'siteradian' ) ); ?>,
		...
		genCreated:    <?php echo wp_json_encode( esc_html__( 'SEO suggestion created. Review it below and apply when you’re ready.', 'siteradian' ) ); ?>,
		...
		genNoPlugin:   <?php echo wp_json_encode( esc_html__( 'No supported SEO plugin (Rank Math or Yoast SEO) is active.', 'siteradian' ) ); ?>,
		genUnsupported: <?php echo wp_json_encode( esc_html__( 'Some items have a status that cannot receive SEO suggestions (e.g. trashed or auto-draft) and were skipped. Draft, pending, scheduled, private, and published content are all supported.', 'siteradian' ) ); ?>,
		...
		bulkAllExist:  <?php echo wp_json_encode( esc_html__( 'All selected items already have open suggestions — review them below.', 'siteradian' ) ); ?>,
		bulkNone:      <?php echo wp_json_encode( esc_html__( 'No suggestions were created for the selected items.', 'siteradian' ) ); ?>,
		...
		colSel:    <?php echo wp_json_encode( esc_html__( 'Select', 'siteradian' ) ); ?>,
		gen:       <?php echo wp_json_encode( esc_html__( 'Generate suggestions', 'siteradian' ) ); ?>,
		generating:<?php echo wp_json_encode( esc_html__( 'Generating…', 'siteradian' ) ); ?>,
		/* translators: %1$d created, %2$d skipped, %3$d failed */
		genDone:   <?php echo wp_json_encode( /* translators: %1$d: number, %2$d: number, %3$d: number */ __( '%1$d drafts created, %2$d skipped, %3$d failed.', 'siteradian' ) ); ?>,
		/*
		...
		skipWhy:      <?php echo wp_json_encode( /* translators: %1$d: number, %2$s: value */ __( '%1$d skipped — %2$s', 'siteradian' ) ); ?>,
		skHasDraft:   <?php echo wp_json_encode( esc_html__( 'already has a draft waiting for review', 'siteradian' ) ); ?>,
		skUpToDate:   <?php echo wp_json_encode( esc_html__( 'already up to date', 'siteradian' ) ); ?>,
		skStatus:     <?php echo wp_json_encode( esc_html__( 'trashed or not started yet', 'siteradian' ) ); ?>,
		skNoProvider: <?php echo wp_json_encode( esc_html__( 'no provider key', 'siteradian' ) ); ?>,
		skNoPlugin:   <?php echo wp_json_encode( esc_html__( 'no supported SEO plugin active', 'siteradian' ) ); ?>,
		skUnsupported:<?php echo wp_json_encode( esc_html__( 'does not support this kind of suggestion', 'siteradian' ) ); ?>,
		skNotFound:   <?php echo wp_json_encode( esc_html__( 'no longer exists', 'siteradian' ) ); ?>,
		skOther:      <?php echo wp_json_encode( esc_html__( 'nothing needed generating', 'siteradian' ) ); ?>,
		skReview:     <?php echo wp_json_encode( esc_html__( 'Open the Suggestions tab to review the drafts already waiting.', 'siteradian' ) ); ?>,
		/* translators: %1$d: number of items that were skipped. */
		skipHead:     <?php echo wp_json_encode( /* translators: %1$d: number */ __( '%1$d item(s) were skipped:', 'siteradian' ) ); ?>,
		skipOpenDraft:<?php echo wp_json_encode( esc_html__( 'Review the existing draft', 'siteradian' ) ); ?>,
		genCap:    <?php echo wp_json_encode( esc_html__( 'Up to 25 at a time; only the first 25 are used.', 'siteradian' ) ); ?>,
		genErr:    <?php echo wp_json_encode( esc_html__( 'Generation failed. Please retry.', 'siteradian' ) ); ?>,
		// Slice 3 — Suggestions tab.
		noSug:     <?php echo wp_json_encode( esc_html__( 'No suggestions yet. Generate some from the Review tab.', 'siteradian' ) ); ?>,
		sgCurTitle: <?php echo wp_json_encode( esc_html__( 'Title', 'siteradian' ) ); ?>,
		sgCurDesc:  <?php echo wp_json_encode( esc_html__( 'Description', 'siteradian' ) ); ?>,
		...
		selectSug:   <?php echo wp_json_encode( esc_html__( 'Select suggestion', 'siteradian' ) ); ?>,
		/*
		...
		genFor:      <?php echo wp_json_encode( /* translators: %s: value */ esc_html__( 'Generate suggestions for %s', 'siteradian' ) ); ?>,
		/* translators: %s: the post or page title. */
		selectSugFor:<?php echo wp_json_encode( /* translators: %s: value */ esc_html__( 'Select suggestion for %s', 'siteradian' ) ); ?>,
		applySel:    <?php echo wp_json_encode( esc_html__( 'Apply selected', 'siteradian' ) ); ?>,
		dismissSel:  <?php echo wp_json_encode( esc_html__( 'Dismiss selected', 'siteradian' ) ); ?>,
		bulkProcessing: <?php echo wp_json_encode( esc_html__( 'Processing', 'siteradian' ) ); ?>,
		bulkDone:    <?php echo wp_json_encode( esc_html__( 'processed', 'siteradian' ) ); ?>,
		lblApplied:  <?php echo wp_json_encode( esc_html__( 'applied', 'siteradian' ) ); ?>,
		lblPending:  <?php echo wp_json_encode( esc_html__( 'submitted', 'siteradian' ) ); ?>,
		lblDismissed:<?php echo wp_json_encode( esc_html__( 'dismissed', 'siteradian' ) ); ?>,
		lblFailed:   <?php echo wp_json_encode( esc_html__( 'failed', 'siteradian' ) ); ?>,
		/* translators: %d: number of selected suggestions */
		confirmBulkApplyDev:  <?php echo wp_json_encode( /* translators: %d: number */ __( 'Apply the %d selected suggestions now? Each is applied individually and can be undone.', 'siteradian' ) ); ?>,
		/* translators: %d: number of selected suggestions */
		confirmBulkApplyGate: <?php echo wp_json_encode( /* translators: %d: number */ __( 'Submit the %d selected suggestions for approval? Each becomes its own approval request.', 'siteradian' ) ); ?>,
		/* translators: %d: number of selected suggestions */
		confirmBulkDismiss:   <?php echo wp_json_encode( /* translators: %d: number */ __( 'Dismiss the %d selected suggestions? This discards the drafts.', 'siteradian' ) ); ?>,
		edit:      <?php echo wp_json_encode( esc_html__( 'Edit', 'siteradian' ) ); ?>,
		none:      <?php echo wp_json_encode( esc_html__( '(not set)', 'siteradian' ) ); ?>,
		...
		ccDesc:    <?php echo wp_json_encode( /* translators: %1$d: number, %2$d: number, %3$d: number */ __( '%1$d (target %2$d–%3$d)', 'siteradian' ) ); ?>,
		...
		colStatus2:<?php echo wp_json_encode( esc_html__( 'Status', 'siteradian' ) ); ?>,
		...
		colActions:<?php echo wp_json_encode( esc_html__( 'Actions', 'siteradian' ) ); ?>,
