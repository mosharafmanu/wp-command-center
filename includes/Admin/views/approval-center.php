...
<div class="wrap wpcc-wrap">
	<h1><?php esc_html_e( 'Approvals', 'siteradian' ); ?>
	...
	<?php if ( $human_only ) : ?>
		<p class="description">
			...
			<?php esc_html_e( 'Only you can approve these. An AI assistant cannot approve its own request.', 'siteradian' ); ?>
		</p>
	<?php endif; ?>
	...
	if ( '' === $detail_id && $wpcc_pending_plan_cnt > 0 ) :
		...
		<p class="wpcc-approvals-recsignal" style="margin:0 0 14px;padding:10px 14px;background:#f0f6fc;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;max-width:760px;font-size:13px;">
			...
			printf(
				...
				esc_html( _n( '%d suggested fix is awaiting your review.', '%d suggested fixes are awaiting your review.', $wpcc_pending_plan_cnt, 'siteradian' ) ),
				(int) $wpcc_pending_plan_cnt
			);
			?>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-settings&wpcc_tab=advanced&apane=diagnostics&dpane=recommendations' ) ); ?>"><?php esc_html_e( 'Review in Recommendations →', 'siteradian' ); ?></a>
		</p>
		<?php
	endif;
	?>
	...
	<p><a href="<?php echo esc_url( $base_url . '&tab=pending' ); ?>" class="wpcc-detail-back">&larr; <?php esc_html_e( 'Back to Approvals', 'siteradian' ); ?></a></p>
	<div id="wpcc-detail" data-id="<?php echo esc_attr( $detail_id ); ?>">
		<p><span class="spinner is-active" style="float:none;margin:0 6px 0 0;vertical-align:middle;"></span><?php esc_html_e( 'Loading…', 'siteradian' ); ?></p>
	</div>
	<?php else : ?>

	<h2 class="nav-tab-wrapper">
		<a href="<?php echo esc_url( $base_url . '&tab=pending' ); ?>" class="nav-tab <?php echo 'pending' === $active_tab ? 'nav-tab-active' : ''; ?>"<?php echo 'pending' === $active_tab ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Pending', 'siteradian' ); ?></a>
		<a href="<?php echo esc_url( $base_url . '&tab=history' ); ?>" class="nav-tab <?php echo 'history' === $active_tab ? 'nav-tab-active' : ''; ?>"<?php echo 'history' === $active_tab ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Decided', 'siteradian' ); ?></a>
		<a href="<?php echo esc_url( $base_url . '&tab=queue' ); ?>" class="nav-tab <?php echo 'queue' === $active_tab ? 'nav-tab-active' : ''; ?>"<?php echo 'queue' === $active_tab ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Execution', 'siteradian' ); ?></a>
	</h2>
	...
	<div id="wpcc-confirm-modal" class="wpcc-modal-backdrop" style="display:none;" role="dialog" aria-modal="true" aria-labelledby="wpcc-confirm-title" aria-describedby="wpcc-confirm-warning">
		<div class="wpcc-modal">
			<h2 id="wpcc-confirm-title"><?php esc_html_e( 'Confirm destructive approval', 'siteradian' ); ?></h2>
			<div id="wpcc-confirm-warning" class="wpcc-card-destructive" role="alert"></div>
			<p>
				<label for="wpcc-confirm-phrase"><?php esc_html_e( 'Type the confirmation phrase to proceed:', 'siteradian' ); ?> <code id="wpcc-confirm-phrase-hint"></code></label><br>
				<input type="text" id="wpcc-confirm-phrase" class="regular-text" autocomplete="off">
			</p>
			<p>
				<label for="wpcc-confirm-reason"><?php esc_html_e( 'Reason (required):', 'siteradian' ); ?></label><br>
				<textarea id="wpcc-confirm-reason" rows="2" class="large-text"></textarea>
			</p>
			<p>
				<button type="button" class="button button-primary wpcc-reject-btn" id="wpcc-confirm-go" disabled><?php esc_html_e( 'Approve &amp; Run', 'siteradian' ); ?></button>
				<button type="button" class="button" id="wpcc-confirm-cancel"><?php esc_html_e( 'Cancel', 'siteradian' ); ?></button>
			</p>
		</div>
	</div>
</div>
...
(function() {
	...
	var i18n = {
		readOnly:    <?php echo wp_json_encode( __( 'Read Only', 'siteradian' ) ); ?>,
		review:      <?php echo wp_json_encode( __( 'Review', 'siteradian' ) ); ?>,
		approve:     <?php echo wp_json_encode( __( 'Approve', 'siteradian' ) ); ?>,
		reject:      <?php echo wp_json_encode( __( 'Reject', 'siteradian' ) ); ?>,
		approved:    <?php echo wp_json_encode( __( 'Done — your site has been updated.', 'siteradian' ) ); ?>,
		approvedLink:<?php echo wp_json_encode( __( 'See it in Changes', 'siteradian' ) ); ?>,
		/*
		 ...
		 */
		doneTitle:   <?php echo wp_json_encode( __( 'This change has been applied to your site.', 'siteradian' ) ); ?>,
		doneUndo:    <?php echo wp_json_encode( __( 'It is recorded and can be undone from Changes.', 'siteradian' ) ); ?>,
		doneBackAi:  <?php echo wp_json_encode( __( 'Back to Built-in AI', 'siteradian' ) ); ?>,
		...
		doneMore:    <?php echo wp_json_encode( __( 'Review other approvals', 'siteradian' ) ); ?>,
		changesUrl:  <?php echo wp_json_encode( admin_url( 'admin.php?page=wpcc-history' ) ); ?>,
		rejected:    <?php echo wp_json_encode( __( 'Rejected.', 'siteradian' ) ); ?>,
		/*
		 ...
		 */
		approvedErr: <?php echo wp_json_encode( __( 'You approved this, but it could not run — so nothing on your site has changed. It stays in Approvals; you can try it again from the Execution tab.', 'siteradian' ) ); ?>,
		unknownErr:  <?php echo wp_json_encode( __( 'Unknown error.', 'siteradian' ) ); ?>,
		/*
		 ...
		 */
		reqFailed:   <?php echo wp_json_encode( __( 'Could not reach your site, so this decision may not have been saved. Reload the page to see where the request stands before deciding again.', 'siteradian' ) ); ?>,
		destructive: <?php echo wp_json_encode( __( 'DESTRUCTIVE — this permanently deletes data and cannot be undone.', 'siteradian' ) ); ?>,
		auditNote:   <?php echo wp_json_encode( __( 'This action will be logged in the audit trail.', 'siteradian' ) ); ?>,
		noPending:   <?php echo wp_json_encode( __( 'Nothing is waiting for you. Requests that need approval appear here for your decision.', 'siteradian' ) ); ?>,
		clearTitle:  <?php echo wp_json_encode( __( 'Nothing is waiting for you', 'siteradian' ) ); ?>,
		...
		selectAll:   <?php echo wp_json_encode( __( 'Select all on this page', 'siteradian' ) ); ?>,
		selectOne:   <?php echo wp_json_encode( __( 'Select this request', 'siteradian' ) ); ?>,
		/* translators: 1: range such as "1–25", 2: total count */
		showingRange: <?php echo wp_json_encode( /* translators: %1$s: value, %2$s: value */ __( 'Showing %1$s of %2$s', 'siteradian' ) ); ?>,
		/* translators: 1: current page, 2: total pages */
		pageOf:      <?php echo wp_json_encode( /* translators: %1$s: value, %2$s: value */ __( 'Page %1$s of %2$s', 'siteradian' ) ); ?>,
		...
		nSelected:   <?php echo wp_json_encode( /* translators: %d: number */ __( '%d selected', 'siteradian' ) ); ?>,
		approveSelected: <?php echo wp_json_encode( __( 'Approve selected', 'siteradian' ) ); ?>,
		rejectSelected:  <?php echo wp_json_encode( __( 'Reject selected', 'siteradian' ) ); ?>,
		/* translators: %d: number of requests */
		confirmBulkApprove: <?php echo wp_json_encode( /* translators: %d: number */ __( 'Approve %d requests? Each one runs through the same checks as approving it individually. Requests that permanently delete something are skipped and must be approved one at a time.', 'siteradian' ) ); ?>,
		/* translators: %d: number of requests */
		confirmBulkReject:  <?php echo wp_json_encode( /* translators: %d: number */ __( 'Reject %d requests?', 'siteradian' ) ); ?>,
		bulkReason:  <?php echo wp_json_encode( __( 'Rejected in bulk by administrator.', 'siteradian' ) ); ?>,
		/* translators: %d: total number of pending requests */
		moreQueued:  <?php echo wp_json_encode( /* translators: %d: number */ __( '(%d pending in total)', 'siteradian' ) ); ?>,
		noHistory:   <?php echo wp_json_encode( __( 'Nothing decided yet. Once you approve or reject something, it is kept here as a record.', 'siteradian' ) ); ?>,
		noQueue:     <?php echo wp_json_encode( __( 'The execution queue is empty.', 'siteradian' ) ); ?>,
		...
		loadFailed:  <?php echo wp_json_encode( __( 'Could not load this list — nothing has changed on your site. Your admin session may have expired; reload the page to try again.', 'siteradian' ) ); ?>,
		loadMore:    <?php echo wp_json_encode( __( 'Load more', 'siteradian' ) ); ?>,
		...
		colResolved: <?php echo wp_json_encode( __( 'Resolved by', 'siteradian' ) ); ?>,
		colWhen:     <?php echo wp_json_encode( __( 'Requested', 'siteradian' ) ); ?>,
		colQueueId:  <?php echo wp_json_encode( __( 'Queue item', 'siteradian' ) ); ?>,
		colAttempts: <?php echo wp_json_encode( __( 'Attempts', 'siteradian' ) ); ?>,
		colError:    <?php echo wp_json_encode( __( 'Error', 'siteradian' ) ); ?>,
		unavailable: <?php echo wp_json_encode( __( 'unavailable', 'siteradian' ) ); ?>,
		chipPending: <?php echo wp_json_encode( __( 'Pending', 'siteradian' ) ); ?>,
		chipCrit:    <?php echo wp_json_encode( __( 'Critical pending', 'siteradian' ) ); ?>,
		chipResolved:<?php echo wp_json_encode( __( 'Resolved (all-time)', 'siteradian' ) ); ?>,
		chipFailed:  <?php echo wp_json_encode( __( 'Failed in queue', 'siteradian' ) ); ?>,
		details:     <?php echo wp_json_encode( __( 'Details', 'siteradian' ) ); ?>,
		secReason:   <?php echo wp_json_encode( __( 'Reason', 'siteradian' ) ); ?>,
		secRequest:  <?php echo wp_json_encode( __( 'Request', 'siteradian' ) ); ?>,
		secChangeset:<?php echo wp_json_encode( __( 'Change set', 'siteradian' ) ); ?>,
		secDiff:     <?php echo wp_json_encode( __( 'Diff', 'siteradian' ) ); ?>,
		secWhatChanges: <?php echo wp_json_encode( __( 'What will change', 'siteradian' ) ); ?>,
		...
		secWhatChanged: <?php echo wp_json_encode( __( 'What changed', 'siteradian' ) ); ?>,
		secWhatWouldHave: <?php echo wp_json_encode( __( 'What this would have changed', 'siteradian' ) ); ?>,
		lblUndoes:      <?php echo wp_json_encode( __( 'Undoes', 'siteradian' ) ); ?>,
		undoUnknown:    <?php echo wp_json_encode( __( 'A change that is no longer in your history', 'siteradian' ) ); ?>,
		lblChangeId:    <?php echo wp_json_encode( __( 'Change id', 'siteradian' ) ); ?>,
		secPayload:  <?php echo wp_json_encode( __( 'Technical details', 'siteradian' ) ); ?>,
		yes:         <?php echo wp_json_encode( __( 'Yes', 'siteradian' ) ); ?>,
		no:          <?php echo wp_json_encode( __( 'No', 'siteradian' ) ); ?>,
		emptyValue:  <?php echo wp_json_encode( __( '(empty)', 'siteradian' ) ); ?>,
		/*
		...
		fieldNames:  <?php echo wp_json_encode( array_merge( \WPCommandCenter\Admin\ActionLabels::option_labels(), [
			'post_title'      => __( 'Title', 'siteradian' ),
			'post_content'    => __( 'Content', 'siteradian' ),
			'post_status'     => __( 'Status', 'siteradian' ),
			'post_excerpt'    => __( 'Excerpt', 'siteradian' ),
			'post_id'         => __( 'Item', 'siteradian' ),
			'option_id'       => __( 'Setting', 'siteradian' ),
			'user_id'         => __( 'User', 'siteradian' ),
			'comment_id'      => __( 'Comment', 'siteradian' ),
			'display_name'    => __( 'Display name', 'siteradian' ),
			'user_email'      => __( 'Email address', 'siteradian' ),
			'role'            => __( 'Role', 'siteradian' ),
			'slug'            => __( 'Slug', 'siteradian' ),
			'value'           => __( 'New value', 'siteradian' ),
			'ids'             => __( 'Items', 'siteradian' ),
			'alt_text'        => __( 'Alt text', 'siteradian' ),
			'caption'         => __( 'Caption', 'siteradian' ),
			'start_url'       => __( 'Site address', 'siteradian' ),
		] ) ); ?>,
		...
		auditNames:  <?php echo wp_json_encode( [
			...
			'operation.approval.approved'       => __( 'Approved', 'siteradian' ),
			'operation.approval.rejected'       => __( 'Rejected', 'siteradian' ),
			'operation.approval.cancelled'      => __( 'Cancelled', 'siteradian' ),
			'operation.queued'                  => __( 'Added to the queue', 'siteradian' ),
			'operation.executed'                => __( 'Carried out', 'siteradian' ),
			'operation.failed'                  => __( 'Failed to run', 'siteradian' ),
		] ); ?>,
		secQueue:    <?php echo wp_json_encode( __( 'Queue', 'siteradian' ) ); ?>,
		secResults:  <?php echo wp_json_encode( __( 'Execution result', 'siteradian' ) ); ?>,
		secAudit:    <?php echo wp_json_encode( __( 'Audit trail', 'siteradian' ) ); ?>,
		lblResolved: <?php echo wp_json_encode( __( 'Resolved by', 'siteradian' ) ); ?>,
		lblArea:     <?php echo wp_json_encode( __( 'Area', 'siteradian' ) ); ?>,
		lblRequested:<?php echo wp_json_encode( __( 'Requested', 'siteradian' ) ); ?>,
		lblApproved: <?php echo wp_json_encode( __( 'Approved', 'siteradian' ) ); ?>,
		lblRejected: <?php echo wp_json_encode( __( 'Rejected', 'siteradian' ) ); ?>,
		lblExecuted: <?php echo wp_json_encode( __( 'Executed', 'siteradian' ) ); ?>,
		lblFailedAt: <?php echo wp_json_encode( __( 'Failed', 'siteradian' ) ); ?>,
		lblCancelled:<?php echo wp_json_encode( __( 'Cancelled', 'siteradian' ) ); ?>,
		lblFiles:    <?php echo wp_json_encode( __( 'files', 'siteradian' ) ); ?>,
		lblHighRisk: <?php echo wp_json_encode( __( 'HIGH-RISK paths included', 'siteradian' ) ); ?>,
		diffUnavail: <?php echo wp_json_encode( __( 'No diff stored for this request.', 'siteradian' ) ); ?>,
		noResult:    <?php echo wp_json_encode( __( 'This has not run yet.', 'siteradian' ) ); ?>,
		noAudit:     <?php echo wp_json_encode( __( 'No audit events recorded for this request.', 'siteradian' ) ); ?>,
		notFound:    <?php echo wp_json_encode( __( 'Request not found.', 'siteradian' ) ); ?>,
		counts:      <?php echo wp_json_encode( /* translators: %1$s: value, %2$s: value, %3$s: value, %4$s: value */ __( 'Created %1$s · Updated %2$s · Skipped %3$s · Errors %4$s', 'siteradian' ) ); ?>,
		retry:       <?php echo wp_json_encode( __( 'Retry', 'siteradian' ) ); ?>,
		retrying:    <?php echo wp_json_encode( __( 'Retrying…', 'siteradian' ) ); ?>,
		retryConfirm:<?php echo wp_json_encode( __( 'Re-queue this failed item for another attempt?', 'siteradian' ) ); ?>,
		retryYes:    <?php echo wp_json_encode( __( 'Try again', 'siteradian' ) ); ?>,
		cancel:      <?php echo wp_json_encode( __( 'Cancel', 'siteradian' ) ); ?>,
		/* translators: 1: current item number, 2: total items */
		bulkProgress: <?php echo wp_json_encode( /* translators: %1$s: value, %2$s: value */ __( 'Working — %1$s of %2$s', 'siteradian' ) ); ?>,
		bulkApproveBody: <?php echo wp_json_encode( __( 'Each one runs through the engine exactly as if you had approved it on its own. Anything needing its own destructive confirmation is skipped.', 'siteradian' ) ); ?>,
		bulkRejectBody:  <?php echo wp_json_encode( __( 'Nothing will run. The requests are recorded as rejected and your site is left unchanged.', 'siteradian' ) ); ?>,
		retryFailed: <?php echo wp_json_encode( __( 'Retry failed: ', 'siteradian' ) ); ?>,
		nonceExpired:<?php echo wp_json_encode( __( 'Your session expired. Please reload the page and try again.', 'siteradian' ) ); ?>,
		detailsFor:  <?php echo wp_json_encode( __( 'View details for this request', 'siteradian' ) ); ?>
	};

	var riskLabels = {
		critical:   <?php echo wp_json_encode( __( 'Critical', 'siteradian' ) ); ?>,
		high:       <?php echo wp_json_encode( __( 'High Risk', 'siteradian' ) ); ?>,
		medium:     <?php echo wp_json_encode( __( 'Medium Risk', 'siteradian' ) ); ?>,
		low:        <?php echo wp_json_encode( __( 'Low Risk', 'siteradian' ) ); ?>,
	 ...
	 */
	var statusLabels = {
		pending_review: <?php echo wp_json_encode( __( 'Waiting for you', 'siteradian' ) ); ?>,
		approved:       <?php echo wp_json_encode( __( 'Approved', 'siteradian' ) ); ?>,
		rejected:       <?php echo wp_json_encode( __( 'Rejected', 'siteradian' ) ); ?>,
		executed:       <?php echo wp_json_encode( __( 'Applied', 'siteradian' ) ); ?>,
		failed:         <?php echo wp_json_encode( __( 'Did not run', 'siteradian' ) ); ?>,
		cancelled:      <?php echo wp_json_encode( __( 'Cancelled', 'siteradian' ) ); ?>,
		queued:         <?php echo wp_json_encode( __( 'Waiting to run', 'siteradian' ) ); ?>,
		running:        <?php echo wp_json_encode( __( 'Running', 'siteradian' ) ); ?>,
		completed:      <?php echo wp_json_encode( __( 'Applied', 'siteradian' ) ); ?>
	};
	// Queue-item overrides: same tokens, different meaning inside the queue.
	var queueStatusLabels = {
		completed:      <?php echo wp_json_encode( __( 'Applied', 'siteradian' ) ); ?>,
		queued:         <?php echo wp_json_encode( __( 'Waiting to run', 'siteradian' ) ); ?>,
		running:        <?php echo wp_json_encode( __( 'Running now', 'siteradian' ) ); ?>,
		failed:         <?php echo wp_json_encode( __( 'This attempt did not run', 'siteradian' ) ); ?>,
		...
		cancelled:      <?php echo wp_json_encode( __( 'Attempt stopped', 'siteradian' ) ); ?>
	};
