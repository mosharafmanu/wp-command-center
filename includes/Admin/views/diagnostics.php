...
$tabs = [
	'performance' => __( 'Performance', 'siteradian' ),
	'security'    => __( 'Security', 'siteradian' ),
	'woocommerce' => __( 'WooCommerce', 'siteradian' ),
	'debug-log'   => __( 'Debug Log', 'siteradian' ),
];

$status_labels = [
	'good'        => __( 'Good', 'siteradian' ),
	'recommended' => __( 'Recommended', 'siteradian' ),
	'critical'    => __( 'Critical', 'siteradian' ),
	'info'        => __( 'Info', 'siteradian' ),
];
...
$render_checks = static function ( array $checks ) use ( $status_badge ): void {
	...
	<table class="widefat striped wpcc-cds-table wpcc-table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Check', 'siteradian' ); ?></th>
				<th><?php esc_html_e( 'Status', 'siteradian' ); ?></th>
				<th><?php esc_html_e( 'Details', 'siteradian' ); ?></th>
			</tr>
		</thead>
		<tbody>
...
<div class="wrap wpcc-wrap">
	<h1><?php esc_html_e( 'Diagnostics', 'siteradian' ); ?></h1>
	<p><?php esc_html_e( 'Performance, security, and WooCommerce diagnostics, plus the debug log viewer.', 'siteradian' ); ?></p>
	...
	<?php elseif ( 'debug-log' === $tab ) : ?>
		...
		if ( isset( $_POST['wpcc_action'] ) && 'clear_debug_log' === sanitize_text_field( wp_unslash( $_POST['wpcc_action'] ) ) ) {
			...
			if ( is_wp_error( $cleared ) ) {
				...
				printf( '<div class="wpcc-cds-notice wpcc-cds-notice--success"><p>%s</p></div>', esc_html__( 'Debug log cleared.', 'siteradian' ) );
			}
		}
		...
		<form method="get" class="wpcc-debug-log-controls">
			...
			<label for="wpcc-lines"><?php esc_html_e( 'Lines:', 'siteradian' ); ?></label>
			...
			<?php submit_button( __( 'Refresh', 'siteradian' ), 'secondary', '', false ); ?>
		</form>
		...
		<?php else : ?>
			...
			<p class="wpcc-scan-meta">
				...
				if ( $result['truncated'] ) {
					echo ' &mdash; ' . esc_html__( 'showing the most recent portion of a large log file.', 'siteradian' );
				}
				?>
			</p>
			...
			<?php if ( ! $wpcc_log_has_content ) : ?>
				<div class="wpcc-cds-empty"><div class="wpcc-cds-empty__title"><?php esc_html_e( 'The debug log is empty.', 'siteradian' ); ?></div></div>
			<?php else : ?>
			...
			<form method="post">
				...
				<?php submit_button( __( 'Clear Log', 'siteradian' ), 'delete', 'submit', false ); ?>
			</form>
