...
$wpcc_ready_steps = [
	[ 'done' => ! empty( $wpcc_conns ),                                      'label' => __( 'A connection added', 'siteradian' ) ],
	[ 'done' => '' !== $wpcc_default,                                        'label' => __( 'A default chosen', 'siteradian' ) ],
	[ 'done' => $wpcc_has_healthy,                                           'label' => __( 'Tested healthy', 'siteradian' ) ],
	[ 'done' => ( $wpcc_health['attention'] === 0 && ! empty( $wpcc_conns ) ), 'label' => __( 'No connection issues', 'siteradian' ) ],
];
...
$wpcc_default_name = '' !== $wpcc_default && isset( $wpcc_conns[ $wpcc_default ] ) ? $wpcc_conns[ $wpcc_default ]['name'] : __( 'none yet', 'siteradian' );
...
if ( isset( $wpcc_tools_on['content'] ) ) {
	$wpcc_editor_actions[] = __( 'Title', 'siteradian' );
	$wpcc_editor_actions[] = __( 'Excerpt', 'siteradian' );
}
if ( isset( $wpcc_tools_on['seo'] ) ) {
	$wpcc_editor_actions[] = __( 'SEO title and description', 'siteradian' );
}
...
<div class="wrap wpcc-aip">
	...
	<?php if ( $wpcc_outcome ) : ?>
		...
		$wpcc_o_cta  = '';
		if ( ! $wpcc_o_haskey ) {
			$wpcc_o_step = __( 'It has no API key yet, so it cannot generate anything. Add one to finish setup.', 'siteradian' );
			$wpcc_o_cta  = 'addkey';
			...
			$wpcc_o_step = __( 'Nothing is switched on by saving a key. Test it once to confirm the key works — the test only reads, and changes nothing on your site.', 'siteradian' );
			$wpcc_o_cta  = 'test';
		} elseif ( in_array( $wpcc_o_health['state'], [ 'healthy', 'slow' ], true ) && ! $wpcc_ai_tools_on ) {
			$wpcc_o_step = __( 'The key works. The SEO, Alt Text and Content tools are still switched off for this site — turn one on and it will generate through this connection.', 'siteradian' );
			$wpcc_o_cta  = 'tools';
		} elseif ( in_array( $wpcc_o_health['state'], [ 'healthy', 'slow' ], true ) ) {
			$wpcc_o_step = __( 'The key works and your built-in AI tools will generate through this connection. Nothing else to set up.', 'siteradian' );
		} else {
		...
		if ( 'test' === $wpcc_outcome_action ) {
			...
			$wpcc_o_head = __( '“%1$s” is working — %2$s', 'siteradian' );
		} elseif ( 'update_key' === $wpcc_outcome_action ) {
			/* translators: 1: connection name chosen by the customer, 2: provider label, e.g. "Anthropic". */
			$wpcc_o_head = __( '“%1$s” has its API key — %2$s', 'siteradian' );
		} else {
			/* translators: 1: connection name chosen by the customer, 2: provider label, e.g. "Anthropic". */
			$wpcc_o_head = __( '“%1$s” is saved — %2$s', 'siteradian' );
		}
		...
		<div class="wpcc-aip-outcome" role="status">
			<p class="wpcc-aip-outcome__title">
				...
				?>
				<?php if ( $wpcc_o_isdef ) : ?>
					<span class="wpcc-aip-badge" style="background:#e7f0fb;color:#1d62b0;margin-left:6px;"><?php esc_html_e( 'DEFAULT', 'siteradian' ); ?></span>
				<?php endif; ?>
			</p>
			...
			<div class="wpcc-aip-outcome__actions">
				<?php if ( 'test' === $wpcc_o_cta ) : ?>
					...
					<form method="post" style="margin:0;">
						...
						<button type="submit" name="wpcc_conn_action" value="test" class="button button-primary"><?php esc_html_e( 'Test this connection', 'siteradian' ); ?></button>
					</form>
				<?php elseif ( 'addkey' === $wpcc_o_cta ) : ?>
					<?php // Opens this connection's own key field and puts the cursor in it. ?>
					<a class="button button-primary wpcc-aip-jump" href="#wpcc-conn-<?php echo esc_attr( $wpcc_outcome_id ); ?>" data-open-key="1"><?php esc_html_e( 'Add your API key', 'siteradian' ); ?></a>
				<?php elseif ( 'tools' === $wpcc_o_cta ) : ?>
					<a class="button button-primary wpcc-aip-jump" href="#wpcc-bai-tools-h"><?php esc_html_e( 'Choose a tool to turn on', 'siteradian' ); ?></a>
				<?php endif; ?>
				<a class="wpcc-aip-outcome__link wpcc-aip-jump" href="#wpcc-conn-<?php echo esc_attr( $wpcc_outcome_id ); ?>"><?php esc_html_e( 'Show the connection', 'siteradian' ); ?></a>
			</div>
			...
			<?php if ( $wpcc_o_proven ) : ?>
				...
				<div class="wpcc-aip-uses">
					<p class="wpcc-aip-uses__h"><?php esc_html_e( 'What you can do now', 'siteradian' ); ?></p>
					...
					<?php if ( $wpcc_o_runtime ) : ?>
							...
							<div class="wpcc-aip-use__body">
								...
								<span>
									...
									if ( $wpcc_tools_on ) {
										...
										esc_html_e( 'Generate SEO descriptions, image alt text and draft content from inside WordPress. All three tools are switched off for this site, so nothing generates yet.', 'siteradian' );
									}
									?>
								</span>
							</div>
							<?php if ( $wpcc_tools_on ) : ?>
								<a class="button button-small wpcc-aip-jump" href="#wpcc-bai-tools-h"><?php esc_html_e( 'Open Built-in AI', 'siteradian' ); ?></a>
							<?php else : ?>
								<a class="button button-small button-primary wpcc-aip-jump" href="#wpcc-bai-tools-h"><?php esc_html_e( 'Turn one on', 'siteradian' ); ?></a>
							<?php endif; ?>
						</div>
						...
						<div class="wpcc-aip-use">
							...
							<?php if ( $wpcc_editor_actions ) : ?>
								<a class="button button-small" href="<?php echo esc_url( admin_url( 'edit.php' ) ); ?>"><?php esc_html_e( 'Open Posts', 'siteradian' ); ?></a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
					...
					<div class="wpcc-aip-use">
						<span class="wpcc-aip-use__icon" aria-hidden="true">◇</span>
						<div class="wpcc-aip-use__body">
							<strong><?php esc_html_e( 'Claude, ChatGPT and other assistants', 'siteradian' ); ?></strong>
							<span>
								<?php
								if ( \WPCommandCenter\Admin\ConnectionStatus::STATE_NO_TOKEN === $wpcc_assistant['state'] ) {
									esc_html_e( 'Let an assistant work on this site directly. It needs its own access token, not this provider key. Changes follow your selected protection mode, including any required human approval.', 'siteradian' );
									...
									echo esc_html(
										sprintf(
											...
											__( 'An assistant is already connected. %s Changes it asks for follow your selected protection mode.', 'siteradian' ),
						...
						<a class="button button-small<?php echo \WPCommandCenter\Admin\ConnectionStatus::STATE_NO_TOKEN === $wpcc_assistant['state'] ? ' button-primary' : ''; ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-settings&wpcc_tab=connections&cpane=assistants' ) ); ?>">
							...
							echo \WPCommandCenter\Admin\ConnectionStatus::STATE_NO_TOKEN === $wpcc_assistant['state']
								? esc_html__( 'Connect an assistant', 'siteradian' )
								: esc_html__( 'Assistants', 'siteradian' );
							?>
						</a>
					</div>
				</div>
	...
	<?php if ( $wpcc_health['attention'] > 0 ) : ?>
		<div class="wpcc-aip-warn" role="status">⚠ <?php printf( esc_html( /* translators: %d: number */ _n( '%d connection needs attention. Open it below for the recommended fix.', '%d connections need attention. Open them below for the recommended fix.', $wpcc_health['attention'], 'siteradian' ) ), (int) $wpcc_health['attention'] ); ?></div>
	<?php elseif ( '' === $wpcc_default && ! empty( $wpcc_conns ) ) : ?>
		<div class="wpcc-aip-warn" role="status"><?php esc_html_e( 'No default connection yet. Add a key to a connection and set it as default so AI features have something to use.', 'siteradian' ); ?></div>
	<?php endif; ?>
	...
	<!-- ===== Recent AI activity ===== -->
	<h2><?php esc_html_e( 'Recent AI activity', 'siteradian' ); ?></h2>
		...
		<div style="background:#fff;border:1px solid #dcdfe3;border-radius:12px;padding:16px 18px;">
			...
			<div style="display:flex;justify-content:flex-end;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px;">
				<?php if ( (int) $wpcc_act['pending_approvals'] > 0 ) : ?>
					<a class="button button-small" href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-activity&wpcc_tab=approvals' ) ); ?>"><?php printf( esc_html( /* translators: %d: number */ _n( '%d pending approval', '%d pending approvals', (int) $wpcc_act['pending_approvals'], 'siteradian' ) ), (int) $wpcc_act['pending_approvals'] ); ?></a>
				<?php endif; ?>
			</div>
			<?php if ( empty( $wpcc_feed ) ) : ?>
				<p class="muted" style="font-size:13px;margin:0;"><?php esc_html_e( 'No activity yet. When AI or an agent acts on this site, the governed history appears here — every action recorded, reversible where supported.', 'siteradian' ); ?></p>
			<?php else : ?>
				<?php
				...
				$wpcc_bucket = '';
				?>
				<ul class="wpcc-aip-timeline" aria-label="<?php esc_attr_e( 'Recent AI activity', 'siteradian' ); ?>">
					<?php foreach ( $wpcc_feed as $ev ) :
						$b = ( $ev['time'] && $ev['time'] >= $wpcc_today ) ? 'today' : 'earlier';
						if ( $b !== $wpcc_bucket ) :
							$wpcc_bucket = $b;
							?>
							<li class="grp"><?php echo 'today' === $b ? esc_html__( 'Today', 'siteradian' ) : esc_html__( 'Earlier', 'siteradian' ); ?></li>
						<?php endif; ?>
						<li class="ev">
							...
							<span class="tm muted"><?php echo $ev['time'] ? esc_html( sprintf( /* translators: %s ago */ __( '%s ago', 'siteradian' ), human_time_diff( $ev['time'], time() ) ) ) : ''; ?></span>
						</li>
					<?php endforeach; ?>
				</ul>
			<?php endif; ?>
			<p style="margin:12px 0 0;display:flex;gap:8px;flex-wrap:wrap;">
				<a class="button button-small" href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-history&wpcc_tab=changes' ) ); ?>"><?php esc_html_e( 'Review changes & undo', 'siteradian' ); ?></a>
				<a class="button button-small" href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-activity&wpcc_tab=approvals' ) ); ?>"><?php esc_html_e( 'Approvals', 'siteradian' ); ?></a>
				<a class="button button-small" href="<?php echo esc_url( admin_url( 'admin.php?page=wpcc-settings&wpcc_tab=connections&cpane=assistants' ) ); ?>"><?php esc_html_e( 'Connect an assistant', 'siteradian' ); ?></a>
			</p>
		</div>
		<div style="display:grid;gap:10px;">
			<div class="wpcc-aip-kpi"><div class="v"><?php echo (int) $wpcc_act['events']; ?></div><div class="l"><?php esc_html_e( 'Recent events', 'siteradian' ); ?></div></div>
			<div class="wpcc-aip-kpi"><div class="v" style="color:<?php echo (int) $wpcc_act['pending_approvals'] ? '#d63638' : '#0a7a33'; ?>;"><?php echo (int) $wpcc_act['pending_approvals']; ?></div><div class="l"><?php esc_html_e( 'Pending approvals', 'siteradian' ); ?></div></div>
			<?php
			/*
			 * Token usage.
			 *
			...
			<div class="wpcc-aip-kpi">
				<?php if ( $wpcc_usage['tracked'] ) : ?>
					...
					<div class="l"><?php esc_html_e( 'Tokens used', 'siteradian' ); ?></div>
					<div class="muted" style="font-size:11px;margin-top:6px;line-height:1.5;">
						...
						printf(
							...
							esc_html__( '%1$s in · %2$s out', 'siteradian' ),
						...
						printf(
							...
							esc_html( _n( 'across %s generation', 'across %s generations', $wpcc_usage['calls'], 'siteradian' ) ),
						...
						if ( $wpcc_usage['partial'] ) {
							...
							printf(
								...
								esc_html( _n( '%s call reported no usage, so this is a minimum.', '%s calls reported no usage, so this is a minimum.', $wpcc_usage['unreported_calls'], 'siteradian' ) ),
								esc_html( number_format_i18n( $wpcc_usage['unreported_calls'] ) )
							);
						}
						?>
						<br /><?php esc_html_e( 'Cost is not shown — no price list ships with the plugin, and an estimate could be wrong.', 'siteradian' ); ?>
					</div>
				<?php else : ?>
					<div class="v" style="font-size:15px;color:#646970;"><?php esc_html_e( 'Nothing generated yet', 'siteradian' ); ?></div>
					<div class="l"><?php esc_html_e( 'Tokens used', 'siteradian' ); ?></div>
					<div class="muted" style="font-size:11px;margin-top:6px;line-height:1.5;">
						<?php esc_html_e( 'Token counts appear here after your first Built-in AI generation. Cost is not shown — no price list ships with the plugin.', 'siteradian' ); ?>
					</div>
				<?php endif; ?>
			</div>
		</div>
	...
	<?php if ( $wpcc_usage['tracked'] ) : ?>
		...
		<details style="max-width:760px;margin:0 0 4px;">
			<summary style="cursor:pointer;font-size:12px;font-weight:600;color:#2271b1;"><?php esc_html_e( 'Token usage by feature and model', 'siteradian' ); ?></summary>
			<div style="overflow-x:auto;">
				<table class="widefat striped" style="margin-top:8px;">
					<caption class="screen-reader-text"><?php esc_html_e( 'Token usage broken down by feature, provider and model', 'siteradian' ); ?></caption>
					<thead>
						<tr>
							<th scope="col"><?php esc_html_e( 'Feature', 'siteradian' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Provider', 'siteradian' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Model', 'siteradian' ); ?></th>
							<th scope="col" style="text-align:right;"><?php esc_html_e( 'Calls', 'siteradian' ); ?></th>
							<th scope="col" style="text-align:right;"><?php esc_html_e( 'In', 'siteradian' ); ?></th>
							<th scope="col" style="text-align:right;"><?php esc_html_e( 'Out', 'siteradian' ); ?></th>
							<th scope="col" style="text-align:right;"><?php esc_html_e( 'Total', 'siteradian' ); ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<p class="muted" style="font-size:11px;margin:6px 0 0;"><?php esc_html_e( 'Counts as the provider reported them. Token counts only — no prompt or generated text is stored to produce this.', 'siteradian' ); ?></p>
		</details>
	<?php endif; ?>

	<?php endif; ?>
	<!-- ===== Quick action ===== -->
	<p style="margin:18px 0;"><button type="button" class="button button-primary button-hero" id="wpcc-aip-new" aria-expanded="false" aria-controls="wpcc-aip-wizard">+ <?php esc_html_e( 'New connection', 'siteradian' ); ?></button></p>

	<!-- ===== Connection wizard (progressive; degrades to a full form without JS) ===== -->
	<form method="post" class="wpcc-aip-wizard" id="wpcc-aip-wizard" aria-label="<?php esc_attr_e( 'New connection wizard', 'siteradian' ); ?>">
		<?php wp_nonce_field( ConnectionController::NONCE ); ?>
		...
		<div class="wpcc-aip-step active" data-step="1">
			...
			<div class="wpcc-aip-field">
				...
				<select name="wpcc_provider" id="wpcc-w-provider">
					<?php foreach ( [ 'cloud' => __( 'Cloud', 'siteradian' ), 'local' => __( 'Local', 'siteradian' ), 'gateway' => __( 'Gateway / Custom', 'siteradian' ) ] as $gk => $glabel ) : ?>
						<?php if ( ! empty( $wpcc_groups[ $gk ] ) ) : ?>
							<optgroup label="<?php echo esc_attr( $glabel ); ?>">
								<?php foreach ( $wpcc_groups[ $gk ] as $pid => $plabel ) : ?>
									<option value="<?php echo esc_attr( $pid ); ?>"><?php echo esc_html( $plabel ); ?> — <?php echo esc_html( ProviderCatalog::runtime_usable( $pid ) ? __( 'used by runtime', 'siteradian' ) : ( ProviderCatalog::test_supported( $pid ) ? __( 'testable', 'siteradian' ) : __( 'stored only', 'siteradian' ) ) ); ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endif; ?>
					<?php endforeach; ?>
		...
		<div class="wpcc-aip-step" data-step="2">
			<h3><?php esc_html_e( 'Step 2 — Name & where it runs', 'siteradian' ); ?></h3>
			<div class="wpcc-aip-field"><label for="wpcc-w-name"><?php esc_html_e( 'Connection name', 'siteradian' ); ?></label><input type="text" id="wpcc-w-name" name="wpcc_name" placeholder="<?php esc_attr_e( 'e.g. Production Claude', 'siteradian' ); ?>" /></div>
			<div class="wpcc-aip-field" id="wpcc-w-endpoint-field"><label for="wpcc-w-endpoint"><?php esc_html_e( 'Base URL', 'siteradian' ); ?> <span class="muted">(<?php esc_html_e( 'required for this provider', 'siteradian' ); ?>)</span></label><input type="url" id="wpcc-w-endpoint" name="wpcc_endpoint" style="font-family:monospace;" placeholder="https://…" /><p class="muted" style="font-size:12px;margin:4px 0 0;"><?php esc_html_e( 'Cloud providers use their official URL automatically — you only set this for local, Azure, gateway, or custom endpoints.', 'siteradian' ); ?></p></div>
		</div>

		<div class="wpcc-aip-step" data-step="3">
			<h3><?php esc_html_e( 'Step 3 — Credentials', 'siteradian' ); ?></h3>
			<p class="muted" style="font-size:13px;"><?php esc_html_e( 'Paste your API key. It is stored on this site and never shown again. Local models usually need no key.', 'siteradian' ); ?></p>
			<?php
			...
			<div role="note" style="margin:0 0 12px;padding:12px 14px;background:#f0f6fc;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;font-size:13px;color:#1d2327;max-width:640px;">
				<strong style="display:block;margin-bottom:4px;"><?php esc_html_e( 'What this key does', 'siteradian' ); ?></strong>
			...
			<div class="wpcc-aip-field"><label for="wpcc-w-key"><?php esc_html_e( 'API key', 'siteradian' ); ?></label><input type="password" id="wpcc-w-key" name="wpcc_key" autocomplete="off" spellcheck="false" style="font-family:monospace;" placeholder="<?php esc_attr_e( 'Paste your API key (optional for local)', 'siteradian' ); ?>" /></div>
		</div>
		...
		<div class="wpcc-aip-step" data-step="4">
			...
			<details class="wpcc-aip-advanced" style="margin-top:4px;">
				<summary style="cursor:pointer;font-size:13px;"><?php esc_html_e( 'Advanced options', 'siteradian' ); ?></summary>
				...
				<div class="wpcc-aip-field" id="wpcc-w-deployment-field" style="display:none;margin-top:10px;">
					...
					<p class="muted" style="font-size:12px;margin:4px 0 0;"><?php esc_html_e( 'Azure OpenAI only: the deployment name you created for this model in your Azure resource.', 'siteradian' ); ?></p>
				</div>
			</details>
		</div>

		<div class="wpcc-aip-step" data-step="5">
			<h3><?php esc_html_e( 'Step 5 — Create & test', 'siteradian' ); ?></h3>
			<p class="muted" style="font-size:13px;"><?php esc_html_e( 'We’ll create the connection now. Then use “Test” on its card to verify the key works — no changes are made to your site.', 'siteradian' ); ?></p>
			<p style="font-size:13px;"><?php esc_html_e( 'Adding a key does not turn AI features on by itself — built-in AI screens are enabled per site.', 'siteradian' ); ?></p>
		</div>

		<div class="wpcc-aip-wnav">
			<button type="button" class="button" id="wpcc-w-back" style="visibility:hidden;"><?php esc_html_e( 'Back', 'siteradian' ); ?></button>
			<span>
				<button type="button" class="button" id="wpcc-w-cancel"><?php esc_html_e( 'Cancel', 'siteradian' ); ?></button>
				<button type="button" class="button button-primary" id="wpcc-w-next"><?php esc_html_e( 'Next', 'siteradian' ); ?></button>
				<button type="submit" class="button button-primary" id="wpcc-w-finish" style="display:none;"><?php esc_html_e( 'Create connection', 'siteradian' ); ?></button>
			</span>
		</div>
	</form>

	<!-- ===== Connections ===== -->
	<h2><?php esc_html_e( 'Your connections', 'siteradian' ); ?></h2>
	<?php if ( empty( $wpcc_conns ) ) : ?>
			...
			<?php foreach ( $wpcc_conns as $cid => $c ) :
				...
				<div class="wpcc-aip-card <?php echo $c['enabled'] ? '' : 'dim'; ?>" id="wpcc-conn-<?php echo esc_attr( $cid ); ?>">
					...
					<div class="wpcc-aip-meta">
						<?php if ( Dialect::endpoint_editable( $c['dialect'] ) ) : ?><div><?php esc_html_e( 'Endpoint', 'siteradian' ); ?>: <code><?php echo esc_html( $c['endpoint'] ?: '—' ); ?></code></div><?php endif; ?>
						<div><?php esc_html_e( 'Model', 'siteradian' ); ?>: <code><?php echo esc_html( $c['model'] ?: ( $def['default_model'] ?? '—' ) ); ?></code></div>
						<?php
						/*
						...
						$wpcc_use_bits[] = $wpcc_routed
							? sprintf(
								...
								__( 'Powers %s', 'siteradian' ),
								implode( ', ', $wpcc_routed )
							)
							: __( 'Not powering any tool', 'siteradian' );
						if ( $wpcc_used['calls'] > 0 && $wpcc_used['last'] > 0 ) {
							$wpcc_use_bits[] = sprintf(
								...
								_n( 'last used %1$s ago (%2$s generation)', 'last used %1$s ago (%2$s generations)', (int) $wpcc_used['calls'], 'siteradian' ),
								human_time_diff( (int) $wpcc_used['last'], time() ),
								number_format_i18n( (int) $wpcc_used['calls'] )
							);
						} else {
							$wpcc_use_bits[] = __( 'not used yet', 'siteradian' );
						}
						?>
						...
						<?php if ( is_array( $lt ) && isset( $lt['time'] ) ) : ?>
							<div class="muted">
								<?php
								$bits = [];
								$bits[] = sprintf( /* translators: %s ago */ __( 'Last test %s ago', 'siteradian' ), human_time_diff( (int) $lt['time'], time() ) );
								if ( ! empty( $lt['latency_ms'] ) ) { $bits[] = sprintf( /* translators: %d ms */ __( '%d ms', 'siteradian' ), (int) $lt['latency_ms'] ); }
								if ( ! empty( $lt['models'] ) ) { $bits[] = sprintf( /* translators: %d models */ _n( '%d model', '%d models', (int) $lt['models'], 'siteradian' ), (int) $lt['models'] ); }
					...
					<details>
						<summary style="cursor:pointer;font-size:12px;font-weight:600;color:#2271b1;"><?php esc_html_e( 'Capabilities (declared)', 'siteradian' ); ?></summary>
						...
						<p class="muted" style="font-size:11px;margin:6px 0 0;"><?php esc_html_e( 'Declared from the provider’s API — not live-tested.', 'siteradian' ); ?></p>
					</details>

					<!-- Inline edit -->
					<details>
						<summary style="cursor:pointer;font-size:12px;font-weight:600;color:#2271b1;"><?php esc_html_e( 'Edit', 'siteradian' ); ?></summary>
						<form method="post" style="margin-top:8px;display:grid;gap:8px;">
							...
							<label style="font-size:12px;"><?php esc_html_e( 'Name', 'siteradian' ); ?><input type="text" name="wpcc_name" value="<?php echo esc_attr( $c['name'] ); ?>" style="width:100%;" /></label>
							<?php if ( Dialect::endpoint_editable( $c['dialect'] ) ) : ?><label style="font-size:12px;"><?php esc_html_e( 'Base URL', 'siteradian' ); ?><input type="url" name="wpcc_endpoint" value="<?php echo esc_attr( $c['endpoint'] ); ?>" style="width:100%;font-family:monospace;" /></label><?php endif; ?>
							<?php
								...
								?>
								<label style="font-size:12px;"><?php esc_html_e( 'Model', 'siteradian' ); ?>
									<select name="wpcc_model" class="wpcc-edit-model" style="width:100%;">
										<?php if ( $wpcc_rec ) : ?><optgroup label="<?php esc_attr_e( 'Recommended', 'siteradian' ); ?>"><?php foreach ( $wpcc_rec as $wpcc_mid => $wpcc_mlabel ) : ?><option value="<?php echo esc_attr( $wpcc_mid ); ?>" <?php selected( $wpcc_cur === (string) $wpcc_mid ); ?>><?php echo esc_html( $wpcc_mlabel ); ?></option><?php endforeach; ?></optgroup><?php endif; ?>
										<?php if ( $wpcc_disc ) : ?><optgroup label="<?php echo esc_attr( sprintf( /* translators: %d: number */ __( 'Discovered from your account (%d)', 'siteradian' ), count( $wpcc_disc ) ) ); ?>"><?php foreach ( $wpcc_disc as $wpcc_did ) : if ( isset( $wpcc_rec[ $wpcc_did ] ) ) { continue; } ?><option value="<?php echo esc_attr( $wpcc_did ); ?>" <?php selected( $wpcc_cur === (string) $wpcc_did ); ?>><?php echo esc_html( $wpcc_did ); ?></option><?php endforeach; ?></optgroup><?php endif; ?>
										<option value="custom" <?php selected( ! $wpcc_known ); ?>><?php esc_html_e( 'Custom model ID…', 'siteradian' ); ?></option>
									</select>
								</label>
								...
								<?php if ( ! empty( $wpcc_disc ) ) : ?>
									<p class="muted" style="font-size:11px;margin:0;"><?php esc_html_e( 'Recommended = our defaults. Discovered from your account = pulled live from your last connection test. Custom = enter any model id.', 'siteradian' ); ?></p>
								<?php elseif ( $wpcc_lists ) : ?>
									<p class="muted" style="font-size:11px;margin:0;"><?php esc_html_e( 'Test this connection once to discover the additional models available to your account.', 'siteradian' ); ?></p>
								<?php else : ?>
									<p class="muted" style="font-size:11px;margin:0;"><?php esc_html_e( 'This provider offers the recommended models only — it doesn’t publish an account model list to discover. Nothing is broken; use Custom to enter any model id.', 'siteradian' ); ?></p>
								<?php endif; ?>
							<label style="font-size:12px;"><?php esc_html_e( 'Tags', 'siteradian' ); ?><input type="text" name="wpcc_tags" value="<?php echo esc_attr( implode( ', ', $c['tags'] ) ); ?>" style="width:100%;" placeholder="prod, cheap" /></label>
							<div><button type="submit" name="wpcc_conn_action" value="update" class="button button-small"><?php esc_html_e( 'Save changes', 'siteradian' ); ?></button></div>
						</form>
						<?php if ( ! $is_const ) : ?>
						<form method="post" style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
							<?php wp_nonce_field( ConnectionController::NONCE ); ?>
							<input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" />
							<input type="password" name="wpcc_key" autocomplete="off" spellcheck="false" style="font-family:monospace;max-width:240px;" placeholder="<?php echo $has_key ? esc_attr__( '•••••• (replace key)', 'siteradian' ) : esc_attr__( 'API key', 'siteradian' ); ?>" />
							<button type="submit" name="wpcc_conn_action" value="update_key" class="button button-small"><?php echo $has_key ? esc_html__( 'Update key', 'siteradian' ) : esc_html__( 'Save key', 'siteradian' ); ?></button>
							<?php if ( $has_key ) : ?><button type="submit" name="wpcc_conn_action" value="clear_key" class="button button-small button-link-delete wpcc-conn-confirm" data-confirm="clear_key" data-conn-name="<?php echo esc_attr( $c['name'] ); ?>"><?php esc_html_e( 'Remove key', 'siteradian' ); ?></button><?php endif; ?>
						</form>
						<?php else : ?><p class="muted" style="font-size:12px;margin-top:8px;"><?php esc_html_e( 'Key defined in wp-config.php (constant) — read-only.', 'siteradian' ); ?></p><?php endif; ?>
					</details>
					...
					<div class="wpcc-aip-actions">
						<form method="post"><?php wp_nonce_field( ConnectionController::NONCE ); ?><input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" /><button type="submit" name="wpcc_conn_action" value="test" class="button button-small <?php echo $wpcc_needs_test ? 'button-primary' : ''; ?>" <?php disabled( ! $testable || ! $has_key ); ?>><?php esc_html_e( 'Test', 'siteradian' ); ?></button></form>
						<?php if ( $wpcc_needs_key ) : ?>
							<?php // The only thing this connection can usefully do next: it has no key. ?>
							<a class="button button-small button-primary wpcc-aip-jump" href="#wpcc-conn-<?php echo esc_attr( $cid ); ?>" data-open-key="1"><?php esc_html_e( 'Add API key', 'siteradian' ); ?></a>
						<?php endif; ?>
						<?php if ( $runtime && ! $is_def && $has_key ) : ?><form method="post"><?php wp_nonce_field( ConnectionController::NONCE ); ?><input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" /><button type="submit" name="wpcc_conn_action" value="set_default" class="button button-small"><?php esc_html_e( 'Set default', 'siteradian' ); ?></button></form><?php endif; ?>
						<form method="post"><?php wp_nonce_field( ConnectionController::NONCE ); ?><input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" /><input type="hidden" name="wpcc_enabled" value="<?php echo $c['enabled'] ? '0' : '1'; ?>" /><button type="submit" name="wpcc_conn_action" value="set_enabled" class="button button-small"><?php echo $c['enabled'] ? esc_html__( 'Disable', 'siteradian' ) : esc_html__( 'Enable', 'siteradian' ); ?></button></form>
						<details class="wpcc-aip-more">
							<summary><?php esc_html_e( 'More', 'siteradian' ); ?></summary>
							<div class="wpcc-aip-more__body">
								<form method="post"><?php wp_nonce_field( ConnectionController::NONCE ); ?><input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" /><button type="submit" name="wpcc_conn_action" value="duplicate" class="button button-small"><?php esc_html_e( 'Duplicate', 'siteradian' ); ?></button></form>
								<?php
								/*
								 ...
								 */
								?>
								<form method="post"><?php wp_nonce_field( ConnectionController::NONCE ); ?><input type="hidden" name="wpcc_conn_id" value="<?php echo esc_attr( $cid ); ?>" /><button type="submit" name="wpcc_conn_action" value="delete" class="button button-small button-link-delete wpcc-conn-confirm" data-confirm="delete" data-conn-name="<?php echo esc_attr( $c['name'] ); ?>" data-has-key="<?php echo $has_key ? '1' : '0'; ?>" data-routed="<?php echo esc_attr( implode( ', ', $wpcc_routed ) ); ?>"><?php esc_html_e( 'Delete', 'siteradian' ); ?></button></form>
							</div>
						</details>
					</div>
				</div>
	...
	<?php if ( empty( $wpcc_runtime_conns ) ) : ?>
		...
		<?php else : ?>
			<p class="muted" style="font-size:13px;"><?php esc_html_e( 'Add a key to an Anthropic connection to choose feature routing.', 'siteradian' ); ?></p>
		<?php endif; ?>
	<?php else : ?>
			...
			<?php foreach ( ConnectionStore::FEATURES as $fk => $flabel ) : ?>
				<div class="wpcc-aip-route">
					<span class="f"><?php echo esc_html( $flabel ); ?><span style="display:block;font-weight:400;font-size:11.5px;color:#8a93a0;"><?php
						$wpcc_fdesc = [ 'seo_meta' => __( 'Powers AI-written SEO titles & descriptions', 'siteradian' ), 'alt_text' => __( 'Powers AI image alt text for accessibility & SEO', 'siteradian' ), 'ai_content' => __( 'Powers AI title & excerpt suggestions', 'siteradian' ) ];
						echo esc_html( $wpcc_fdesc[ $fk ] ?? '' );
					?></span></span>
					<span class="arrow" aria-hidden="true">→</span>
					<label class="screen-reader-text" for="wpcc-route-<?php echo esc_attr( $fk ); ?>"><?php printf( /* translators: %s: value */ esc_html__( 'Connection for %s', 'siteradian' ), esc_html( $flabel ) ); ?></label>
	...
	<?php if ( $wpcc_store->is_configured( $wpcc_conns[ $wpcc_default ] ?? [] ) ) : ?>
		<div style="margin:18px 0 0;padding:12px 14px;background:#f0f6fc;border:1px solid #c3c4c7;border-radius:8px;max-width:720px;">
			<strong style="font-size:13px;"><?php esc_html_e( 'Your provider is ready. What happens next?', 'siteradian' ); ?></strong>
			<ol style="margin:8px 0 0;padding-left:20px;color:#50575e;font-size:13px;line-height:1.6;">
				<li><?php esc_html_e( 'Use “Test” on a connection to confirm the key works.', 'siteradian' ); ?></li>
				<?php if ( $wpcc_ai_tools_on ) : ?>
					<li><?php esc_html_e( 'Choose SEO, Alt Text or Content above to generate suggestions. Built-in AI uses this provider directly — you do not need to connect an external assistant.', 'siteradian' ); ?></li>
				<?php else : ?>
					<li><?php esc_html_e( 'Switch on SEO, Alt Text or Content above to start generating. Adding a key does not turn the tools on by itself.', 'siteradian' ); ?></li>
				<?php endif; ?>
				<li><?php esc_html_e( 'Nothing is published automatically. Every suggestion is a draft you review, and applying one follows the same approval and undo rules as any other change.', 'siteradian' ); ?></li>
			</ol>
			<p style="margin:10px 0 0;font-size:12px;color:#646970;">
				...
				printf(
					...
					esc_html__( 'Connecting Claude, Cursor or another assistant is a separate, optional path that needs no provider key — see %1$sAssistants%2$s.', 'siteradian' ),
...
(function () {
	...
	(function () {
		...
		var CUSTOM_LABEL = <?php echo wp_json_encode( __( 'Custom model ID…', 'siteradian' ) ); ?>;
		var FREE_HELP = <?php echo wp_json_encode( __( 'Enter the model id your endpoint serves (free text).', 'siteradian' ) ); ?>;
		var DISC_HELP = <?php echo wp_json_encode( __( 'Discovering models…', 'siteradian' ) ); ?>;
...
( function () {
	...
	var i18n = <?php echo wp_json_encode( [
		'deleteTitle'  => __( 'Delete this connection?', 'siteradian' ),
		/* translators: %s: the connection name. */
		'deleteBody'   => __( '“%s” will be removed and its stored API key deleted. This cannot be undone — you would need to paste the key again to restore it.', 'siteradian' ),
		/* translators: %s: comma-separated list of tool names. */
		'deleteRouted' => __( 'These tools generate through it and will stop until you point them at another connection: %s.', 'siteradian' ),
		'deleteKeeps'  => __( 'Your existing suggestions, applied changes and audit history are not affected — nothing already recorded is erased.', 'siteradian' ),
		'deleteGo'     => __( 'Delete connection', 'siteradian' ),
		'keyTitle'     => __( 'Remove this key?', 'siteradian' ),
		/* translators: %s: the connection name. */
		'keyBody'      => __( 'The stored API key for “%s” will be deleted. The connection stays, but it cannot generate anything until you add a key again.', 'siteradian' ),
		'keyGo'        => __( 'Remove key', 'siteradian' ),
		'cancel'       => __( 'Cancel', 'siteradian' ),
	] ); ?>;
