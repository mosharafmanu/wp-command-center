...
if ( in_array( $wpcc_selected_client, [ 'codex', 'claude_code', 'antigravity', 'command_code' ], true ) ) {
	$wpcc_verify_first = __( 'Run /mcp in the app and confirm wp-command-center is loaded.', 'siteradian' );
} elseif ( 'cursor' === $wpcc_selected_client ) {
	$wpcc_verify_first = __( 'Open Cursor → Customize → MCPs and confirm wp-command-center is enabled.', 'siteradian' );
