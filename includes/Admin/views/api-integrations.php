...
<div class="wpcc-apiint" style="max-width:880px;">
	...
	<?php if ( ! $wpcc_have_tok ) : ?>
		<div class="wpcc-cds-empty" role="status" style="background:#fff;border:1px solid #c3c4c7;border-left:4px solid #2271b1;border-radius:6px;padding:18px 20px;margin:16px 0;">
			...
			<a class="button button-primary" href="<?php echo esc_url( $wpcc_access_url ); ?>"><?php esc_html_e( 'Create a token in Settings → Connections', 'wp-command-center' ); ?></a>
		</div>
	<?php else : ?>
		<p style="margin:14px 0;">
			...
			printf(
				...
				esc_html( _n( 'You have %d active token. Manage tokens and scopes in Settings → Connections.', 'You have %d active tokens. Manage tokens and scopes in Settings → Connections.', count( $wpcc_active ), 'wp-command-center' ) ),
	...
	<table class="widefat striped" style="max-width:680px;">
		<tbody>
			...
			<tr>
				...
				<td><a href="<?php echo esc_url( $wpcc_access_url ); ?>"><?php esc_html_e( 'Created & managed in Settings → Connections', 'wp-command-center' ); ?></a></td>
			</tr>
		</tbody>
	</table>
	...
	<div style="margin-top:20px;padding:14px 16px;background:#f6f7f7;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;max-width:680px;">
		...
		<p style="margin:0;font-size:13px;color:#50575e;">
			<?php esc_html_e( 'Read requests return immediately. Write requests run through the same engine as every other door: they are capability-scoped to the token, wait for approval when your security mode requires it, are recorded under Changes, and can be undone where reversible.', 'wp-command-center' ); ?>
		</p>
	</div>
