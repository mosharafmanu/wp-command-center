...
<div class="wpcc-apiint" style="max-width:880px;">
	...
	<?php if ( ! $wpcc_have_tok ) : ?>
		<div class="wpcc-cds-empty" role="status" style="background:#fff;border:1px solid #c3c4c7;border-left:4px solid #2271b1;border-radius:6px;padding:18px 20px;margin:16px 0;">
			<p style="margin:0 0 4px;font-size:15px;"><strong><?php esc_html_e( 'Create an access token to connect an app.', 'siteradian' ); ?></strong></p>
			<p class="description" style="margin:0 0 12px;"><?php esc_html_e( 'Apps authenticate with a Base URL and a token. Reads stay instant; changes stay governed.', 'siteradian' ); ?></p>
			<a class="button button-primary" href="<?php echo esc_url( $wpcc_access_url ); ?>"><?php esc_html_e( 'Go to Access tokens', 'siteradian' ); ?></a>
		</div>
	<?php else : ?>
		<p style="margin:14px 0;">
			...
			printf(
				...
				esc_html( _n( 'You have %d active token. Manage tokens and scopes in Settings → Connections.', 'You have %d active tokens. Manage tokens and scopes in Settings → Connections.', count( $wpcc_active ), 'siteradian' ) ),
				(int) count( $wpcc_active )
			);
			?>
			<a href="<?php echo esc_url( $wpcc_access_url ); ?>"><?php esc_html_e( 'Access tokens →', 'siteradian' ); ?></a>
		</p>
	<?php endif; ?>

	<h2 style="margin-top:24px;"><?php esc_html_e( 'Connection details', 'siteradian' ); ?></h2>
	<table class="widefat striped" style="max-width:680px;">
		<tbody>
			<tr>
				<th scope="row" style="width:140px;"><?php esc_html_e( 'Base URL', 'siteradian' ); ?></th>
				<td><code style="font-size:13px;"><?php echo esc_html( $wpcc_base ); ?></code></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Authentication', 'siteradian' ); ?></th>
				<td><code style="font-size:13px;">Authorization: Bearer &lt;token&gt;</code></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Token', 'siteradian' ); ?></th>
				<td><a href="<?php echo esc_url( $wpcc_access_url ); ?>"><?php esc_html_e( 'Created and managed under Access tokens', 'siteradian' ); ?></a></td>
			</tr>
		</tbody>
	</table>
	...
	<div style="margin-top:20px;padding:14px 16px;background:#f6f7f7;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;max-width:680px;">
		<strong style="display:block;margin-bottom:6px;"><?php esc_html_e( 'How changes stay safe', 'siteradian' ); ?></strong>
		<p style="margin:0;font-size:13px;color:#50575e;">
			<?php esc_html_e( 'Read requests return immediately. Write requests run through the same engine as every other door: they are capability-scoped to the token, wait for approval when your security mode requires it, are recorded under Changes, and can be undone where reversible.', 'siteradian' ); ?>
		</p>
	</div>
	...
	<p style="margin-top:18px;font-size:13px;">
		...
		printf(
			...
			esc_html__( 'Connecting a desktop AI assistant (Claude, Cursor, Codex…) instead? Use %1$sAssistants%2$s.', 'siteradian' ),
