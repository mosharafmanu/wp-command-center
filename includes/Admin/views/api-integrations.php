...
<div class="wpcc-apiint" style="max-width:880px;">
	<h1><?php esc_html_e( 'API & Integrations', 'ai-command-center' ); ?></h1>
	<p class="description" style="max-width:680px;font-size:14px;">
		<?php esc_html_e( 'Let your own app, service, or automation drive this WordPress site over a governed REST API. Your software sends requests with an access token; reads are instant, and any change is approved, audited, and reversible — exactly like every other door into WP Command Center.', 'ai-command-center' ); ?>
	</p>
	...
	<?php else : ?>
		<p style="margin:14px 0;">
			...
			printf(
				...
				esc_html( _n( 'You have %d active token. Manage tokens and scopes in Settings → Connections.', 'You have %d active tokens. Manage tokens and scopes in Settings → Connections.', count( $wpcc_active ), 'ai-command-center' ) ),
	...
	<h2 style="margin-top:24px;"><?php esc_html_e( 'Connection details', 'ai-command-center' ); ?></h2>
	<table class="widefat striped" style="max-width:680px;">
		<tbody>
			<tr>
				<th scope="row" style="width:140px;"><?php esc_html_e( 'Base URL', 'ai-command-center' ); ?></th>
				<td><code style="font-size:13px;"><?php echo esc_html( $wpcc_base ); ?></code></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Authentication', 'ai-command-center' ); ?></th>
	...
	<h2 style="margin-top:24px;"><?php esc_html_e( 'Try a read-only call', 'ai-command-center' ); ?></h2>
	<p class="description" style="max-width:680px;">
		<?php esc_html_e( 'This lists the operations the API exposes — a safe, read-only request that changes nothing. Set WPCC_TOKEN to an active token first.', 'ai-command-center' ); ?>
	</p>
	...
	<div style="margin-top:20px;padding:14px 16px;background:#f6f7f7;border-left:3px solid #2271b1;border-radius:0 4px 4px 0;max-width:680px;">
		<strong style="display:block;margin-bottom:6px;"><?php esc_html_e( 'How changes stay safe', 'ai-command-center' ); ?></strong>
		<p style="margin:0;font-size:13px;color:#50575e;">
			<?php esc_html_e( 'Read requests return immediately. Write requests run through the same engine as every other door: they are capability-scoped to the token, wait for approval when your security mode requires it, are recorded under Changes, and can be undone where reversible.', 'ai-command-center' ); ?>
		</p>
	</div>
