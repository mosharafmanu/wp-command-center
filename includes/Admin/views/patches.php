...
if ( isset( $_POST['wpcc_action'] ) ) {
	...
	if ( 'create_patch' === $action ) {
		...
		$notice = is_wp_error( $result )
			...
			: [ 'type' => 'success', 'message' => __( 'Patch created and pending approval.', 'siteradian' ) ];
		...
		$result = match ( $action ) {
			...
			'delete_patch'   => ( static function () use ( $patch_manager, $id ) {
				...
				if ( PatchManager::STATUS_APPLIED === $patch['status'] ) {
					return new \WP_Error( 'wpcc_invalid_status', __( 'Roll back this patch before deleting it.', 'siteradian' ) );
				}
		...
		if ( is_wp_error( $result ) ) {
			...
			$notice = [ 'type' => 'error', 'message' => __( 'Patch failed verification and the affected file(s) were automatically reverted.', 'siteradian' ) ];
		} else {
			$messages = [
				'approve_patch'  => __( 'Patch approved.', 'siteradian' ),
				'reject_patch'   => __( 'Patch rejected.', 'siteradian' ),
				'apply_patch'    => __( 'Patch applied successfully. A snapshot was taken automatically.', 'siteradian' ),
				'rollback_patch' => __( 'Patch rolled back. The affected file(s) were restored.', 'siteradian' ),
				'delete_patch'   => __( 'Patch deleted.', 'siteradian' ),
			];
...
<div class="wrap wpcc-wrap">
	<h1><?php esc_html_e( 'Patches', 'siteradian' ); ?></h1>
	<p><?php esc_html_e( 'AI-generated and manual patches — review the diff, explanation, and risk level, then approve, apply, or roll back.', 'siteradian' ); ?></p>
	...
	<?php if ( '' !== $view_id ) : ?>
		...
		<?php else : ?>
			...
			<h2><?php esc_html_e( 'Patch Details', 'siteradian' ); ?></h2>

			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Patch ID', 'siteradian' ); ?></th>
					<td><code><?php echo esc_html( $patch['id'] ); ?></code></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Source', 'siteradian' ); ?></th>
					<td><?php echo esc_html( PatchManager::source_label( $patch['source'] ) ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Status', 'siteradian' ); ?></th>
					<td><?php echo wp_kses_post( PatchManager::status_badge( $patch['status'] ) ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Risk Level', 'siteradian' ); ?></th>
					<td><?php echo wp_kses_post( PatchManager::risk_badge( $patch['risk_level'] ) ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Target File(s)', 'siteradian' ); ?></th>
					<td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Created', 'siteradian' ); ?></th>
			...
			<?php if ( ! empty( $patch['verification'] ) ) : ?>
				<h2><?php esc_html_e( 'Verification', 'siteradian' ); ?></h2>
				<ul class="ul-disc">
			...
			<h2><?php esc_html_e( 'Diff Preview', 'siteradian' ); ?></h2>
			<?php
			...
			<h2><?php esc_html_e( 'Actions', 'siteradian' ); ?></h2>
			<p class="wpcc-actions">
				<?php if ( in_array( $patch['status'], [ PatchManager::STATUS_DRAFT, PatchManager::STATUS_PENDING_APPROVAL ], true ) ) : ?>
					<form method="post" class="wpcc-inline-form">
						...
						<?php submit_button( __( 'Approve Patch', 'siteradian' ), 'primary', '', false ); ?>
					</form>
				<?php endif; ?>
				...
				<?php if ( PatchManager::STATUS_APPROVED === $patch['status'] ) : ?>
					<form method="post" class="wpcc-inline-form">
						...
						<input type="hidden" name="id" value="<?php echo esc_attr( $patch['id'] ); ?>" />
						<?php submit_button( __( 'Apply Patch', 'siteradian' ), 'primary', '', false ); ?>
					</form>
				<?php endif; ?>
				...
				<?php if ( in_array( $patch['status'], [ PatchManager::STATUS_DRAFT, PatchManager::STATUS_PENDING_APPROVAL, PatchManager::STATUS_APPROVED ], true ) ) : ?>
					<form method="post" class="wpcc-inline-form">
						...
						<input type="hidden" name="id" value="<?php echo esc_attr( $patch['id'] ); ?>" />
						<?php submit_button( __( 'Reject Patch', 'siteradian' ), 'secondary', '', false ); ?>
					</form>
				<?php endif; ?>

				<?php if ( PatchManager::STATUS_APPLIED === $patch['status'] ) : ?>
					<form method="post" class="wpcc-inline-form" onsubmit="return confirm('<?php echo esc_js( __( 'Roll back this patch and restore the affected file(s)?', 'siteradian' ) ); ?>');">
						<?php wp_nonce_field( 'wpcc_patches' ); ?>
						<input type="hidden" name="wpcc_action" value="rollback_patch" />
						<input type="hidden" name="id" value="<?php echo esc_attr( $patch['id'] ); ?>" />
						<?php submit_button( __( 'Roll Back Patch', 'siteradian' ), 'secondary', '', false ); ?>
					</form>
				<?php endif; ?>

				<?php if ( PatchManager::STATUS_APPLIED !== $patch['status'] ) : ?>
					<form method="post" class="wpcc-inline-form" onsubmit="return confirm('<?php echo esc_js( __( 'Delete this patch? This cannot be undone.', 'siteradian' ) ); ?>');">
						<?php wp_nonce_field( 'wpcc_patches' ); ?>
						<input type="hidden" name="wpcc_action" value="delete_patch" />
						<input type="hidden" name="id" value="<?php echo esc_attr( $patch['id'] ); ?>" />
						<?php submit_button( __( 'Delete Patch', 'siteradian' ), 'delete', '', false ); ?>
					</form>
				<?php endif; ?>
			</p>

			<p><a href="<?php echo esc_url( $page_url() ); ?>"><?php esc_html_e( '← Back to all patches', 'siteradian' ); ?></a></p>
	...
	<?php else : ?>
		...
		<table class="widefat striped wpcc-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Patch ID', 'siteradian' ); ?></th>
					<th><?php esc_html_e( 'Source', 'siteradian' ); ?></th>
					<th><?php esc_html_e( 'Target File(s)', 'siteradian' ); ?></th>
					<th><?php esc_html_e( 'Date Created', 'siteradian' ); ?></th>
			...
			<?php if ( empty( $patches ) ) : ?>
				<tr>
					<td colspan="6"><?php esc_html_e( 'No patches yet.', 'siteradian' ); ?></td>
				</tr>
			<?php endif; ?>
			<?php foreach ( $patches as $summary ) : ?>
				<tr>
					...
					<td class="wpcc-actions">
						<a class="button button-small" href="<?php echo esc_url( $page_url( [ 'view' => $summary['id'] ] ) ); ?>"><?php esc_html_e( 'View', 'siteradian' ); ?></a>
						...
						<?php if ( in_array( $summary['status'], [ PatchManager::STATUS_DRAFT, PatchManager::STATUS_PENDING_APPROVAL ], true ) ) : ?>
							<form method="post" class="wpcc-inline-form">
								...
								<?php submit_button( __( 'Approve', 'siteradian' ), 'small primary', '', false ); ?>
							</form>
						<?php endif; ?>
						...
						<?php if ( PatchManager::STATUS_APPROVED === $summary['status'] ) : ?>
							<form method="post" class="wpcc-inline-form">
								...
								<input type="hidden" name="wpcc_action" value="apply_patch" />
								<input type="hidden" name="id" value="<?php echo esc_attr( $summary['id'] ); ?>" />
								<?php submit_button( __( 'Apply', 'siteradian' ), 'small primary', '', false ); ?>
							</form>
						<?php endif; ?>
						...
						<?php if ( PatchManager::STATUS_APPLIED === $summary['status'] ) : ?>
							<form method="post" class="wpcc-inline-form" onsubmit="return confirm('<?php echo esc_js( __( 'Roll back this patch and restore the affected file(s)?', 'siteradian' ) ); ?>');">
								...
								<input type="hidden" name="wpcc_action" value="rollback_patch" />
								<input type="hidden" name="id" value="<?php echo esc_attr( $summary['id'] ); ?>" />
								<?php submit_button( __( 'Roll Back', 'siteradian' ), 'small', '', false ); ?>
							</form>
						<?php endif; ?>
					</td>
				</tr>
		...
		<details id="create-patch"<?php echo '' !== $prefill_path ? ' open' : ''; ?>>
			<summary><?php esc_html_e( 'Create Patch Manually', 'siteradian' ); ?></summary>

			<form method="get" class="wpcc-search-form">
				<input type="hidden" name="page" value="wpcc-patches" />
				<label for="wpcc-patch-load-path" class="screen-reader-text"><?php esc_html_e( 'File path', 'siteradian' ); ?></label>
				...
				<?php submit_button( __( 'Load File', 'siteradian' ), 'secondary', '', false ); ?>
				<p class="description"><?php esc_html_e( 'Load the current contents of a file (relative to wp-content/), edit it below, and submit to create a patch for review.', 'siteradian' ); ?></p>
			</form>
			...
			<?php if ( '' !== $prefill_path ) : ?>
				...
				<?php else : ?>
					<form method="post">
						...
						<table class="form-table" role="presentation">
							<tr>
								<th scope="row"><?php esc_html_e( 'File', 'siteradian' ); ?></th>
								<td><code><?php echo esc_html( $loaded['path'] ); ?></code></td>
							</tr>
							<tr>
								<th scope="row"><label for="wpcc-patch-modified"><?php esc_html_e( 'New Contents', 'siteradian' ); ?></label></th>
							...
							<tr>
								<th scope="row"><label for="wpcc-patch-explanation"><?php esc_html_e( 'Explanation', 'siteradian' ); ?></label></th>
								<td><textarea name="explanation" id="wpcc-patch-explanation" class="large-text" rows="3" placeholder="<?php esc_attr_e( 'What does this change do and why?', 'siteradian' ); ?>"></textarea></td>
							</tr>
							<tr>
								<th scope="row"><label for="wpcc-patch-risk"><?php esc_html_e( 'Risk Level', 'siteradian' ); ?></label></th>
								<td>
						...
						<?php submit_button( __( 'Create Patch', 'siteradian' ) ); ?>
					</form>
				<?php endif; ?>
			<?php endif; ?>
		</details>
