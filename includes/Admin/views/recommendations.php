...
if ( isset( $_POST['wpcc_rec_action'] ) && check_admin_referer( 'wpcc_recommendations' ) && current_user_can( 'manage_options' ) ) {
	...
	if ( 'scan' === $wpcc_rec_act ) {
		...
		if ( is_wp_error( $scan ) ) {
			...
			$wpcc_rec_notice = __( 'Scan complete. Findings are listed below.', 'action-steward' );
		}
		...
		if ( is_wp_error( $res ) ) {
			...
			$wpcc_rec_notice = 'dismissed' === $status
				? __( 'Recommendation dismissed.', 'action-steward' )
				: __( 'Recommendation marked resolved.', 'action-steward' );
		}
		...
		$wpcc_rec_notice = 'approved' === $new
			? __( 'Suggested fix approved.', 'action-steward' )
			: __( 'Suggested fix rejected.', 'action-steward' );
	}
}
...
<div class="wpcc-rec-wrap">
	<h1><?php esc_html_e( 'Recommendations', 'action-steward' ); ?></h1>
	<p class="description" style="max-width:720px;">
		<?php esc_html_e( 'Things worth your attention on this site — detected by deterministic checks, never invented. Review a finding, dismiss it, or mark it resolved. Suggested fixes that need your sign-off appear at the bottom.', 'action-steward' ); ?>
	</p>
	...
	<div class="wpcc-rec-cards">
		<div class="wpcc-rec-card"><div class="v"><?php echo esc_html( (string) $wpcc_c_open ); ?></div><div class="l"><?php esc_html_e( 'Open', 'action-steward' ); ?></div></div>
		<div class="wpcc-rec-card is-critical"><div class="v"><?php echo esc_html( (string) $wpcc_c_critical ); ?></div><div class="l"><?php esc_html_e( 'Critical', 'action-steward' ); ?></div></div>
		<div class="wpcc-rec-card"><div class="v"><?php echo esc_html( (string) $wpcc_c_resolved ); ?></div><div class="l"><?php esc_html_e( 'Resolved', 'action-steward' ); ?></div></div>
	</div>

	<form method="post" style="margin:0 0 18px;">
		<?php wp_nonce_field( 'wpcc_recommendations' ); ?>
		<button type="submit" name="wpcc_rec_action" value="scan" class="button"><?php esc_html_e( 'Run a scan', 'action-steward' ); ?></button>
		<span class="description" style="margin-left:8px;"><?php esc_html_e( 'Re-checks this site for issues. Generates findings only — it never changes your site.', 'action-steward' ); ?></span>
	</form>

	<?php if ( empty( $wpcc_rec_list ) ) : ?>
		<div class="wpcc-rec-empty">
			<p style="margin:0 0 4px;font-size:14px;"><strong><?php esc_html_e( 'No recommendations yet.', 'action-steward' ); ?></strong></p>
			<p style="margin:0;"><?php esc_html_e( 'When a check finds something worth your attention, it appears here. Run a scan to check this site now.', 'action-steward' ); ?></p>
		</div>
	<?php else : ?>
		<table class="wp-list-table widefat fixed striped">
			<thead><tr>
				<th><?php esc_html_e( 'Recommendation', 'action-steward' ); ?></th>
				<th style="width:90px;"><?php esc_html_e( 'Severity', 'action-steward' ); ?></th>
				<th style="width:120px;"><?php esc_html_e( 'Status', 'action-steward' ); ?></th>
				<th style="width:180px;"><?php esc_html_e( 'Actions', 'action-steward' ); ?></th>
			</tr></thead>
			<tbody>
						...
						<?php if ( $rid && ( $wpcc_can_dismiss( $status ) || $wpcc_can_resolve( $status ) ) ) : ?>
							<form method="post" style="display:inline;">
								...
								<?php if ( $wpcc_can_resolve( $status ) ) : ?>
									<button type="submit" name="wpcc_rec_action" value="resolve" class="button button-small"><?php esc_html_e( 'Resolve', 'action-steward' ); ?></button>
								<?php endif; ?>
								<?php if ( $wpcc_can_dismiss( $status ) ) : ?>
									<button type="submit" name="wpcc_rec_action" value="dismiss" class="button button-small"><?php esc_html_e( 'Dismiss', 'action-steward' ); ?></button>
								<?php endif; ?>
							</form>
						<?php else : ?>
	...
	<h2 style="margin-top:26px;"><?php esc_html_e( 'Suggested fixes awaiting your approval', 'action-steward' ); ?></h2>
	<?php if ( empty( $wpcc_pending_plans ) ) : ?>
		<div class="wpcc-rec-empty">
			<p style="margin:0;"><?php esc_html_e( 'Nothing waiting. When a recommendation produces a fix that needs your sign-off, it appears here.', 'action-steward' ); ?></p>
		</div>
	<?php else : ?>
		<table class="wp-list-table widefat fixed striped">
			<thead><tr><th><?php esc_html_e( 'Title', 'action-steward' ); ?></th><th><?php esc_html_e( 'Objective', 'action-steward' ); ?></th><th style="width:180px;"><?php esc_html_e( 'Actions', 'action-steward' ); ?></th></tr></thead>
			<tbody>
				...
				<tr>
					...
					<td>
						<form method="post" style="display:inline;">
							...
							<button type="submit" name="wpcc_rec_action" value="approve_plan" class="button button-primary button-small"><?php esc_html_e( 'Approve', 'action-steward' ); ?></button>
							<button type="submit" name="wpcc_rec_action" value="reject_plan" class="button button-small"><?php esc_html_e( 'Reject', 'action-steward' ); ?></button>
						</form>
					</td>
				</tr>
			<?php endforeach; ?>
