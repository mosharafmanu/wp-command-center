...
if ( isset( $_POST['wpcc_rec_action'] ) && check_admin_referer( 'wpcc_recommendations' ) && current_user_can( 'manage_options' ) ) {
	...
	if ( 'scan' === $wpcc_rec_act ) {
		...
		if ( is_wp_error( $scan ) ) {
			...
			$wpcc_rec_notice = __( 'Scan complete. Findings are listed below.', 'siteradian' );
		}
		...
		if ( is_wp_error( $res ) ) {
			...
			$wpcc_rec_notice = 'dismissed' === $status
				? __( 'Recommendation dismissed.', 'siteradian' )
				: __( 'Recommendation marked resolved.', 'siteradian' );
		}
		...
		$wpcc_rec_notice = 'approved' === $new
			? __( 'Suggested fix approved.', 'siteradian' )
			: __( 'Suggested fix rejected.', 'siteradian' );
	}
}
...
<div class="wpcc-rec-wrap">
	<h1><?php esc_html_e( 'Recommendations', 'siteradian' ); ?></h1>
	<p class="description" style="max-width:720px;">
		<?php esc_html_e( 'Things worth your attention on this site — detected by deterministic checks, never invented. Review a finding, dismiss it, or mark it resolved. Suggested fixes that need your sign-off appear at the bottom.', 'siteradian' ); ?>
	</p>
	...
	<div class="wpcc-rec-cards">
		<div class="wpcc-rec-card"><div class="v"><?php echo esc_html( (string) $wpcc_c_open ); ?></div><div class="l"><?php esc_html_e( 'Open', 'siteradian' ); ?></div></div>
		<div class="wpcc-rec-card is-critical"><div class="v"><?php echo esc_html( (string) $wpcc_c_critical ); ?></div><div class="l"><?php esc_html_e( 'Critical', 'siteradian' ); ?></div></div>
		<div class="wpcc-rec-card"><div class="v"><?php echo esc_html( (string) $wpcc_c_resolved ); ?></div><div class="l"><?php esc_html_e( 'Resolved', 'siteradian' ); ?></div></div>
	</div>

	<form method="post" style="margin:0 0 18px;">
		<?php wp_nonce_field( 'wpcc_recommendations' ); ?>
		<button type="submit" name="wpcc_rec_action" value="scan" class="button"><?php esc_html_e( 'Run a scan', 'siteradian' ); ?></button>
		<span class="description" style="margin-left:8px;"><?php esc_html_e( 'Re-checks this site for issues. Generates findings only — it never changes your site.', 'siteradian' ); ?></span>
	</form>

	<?php if ( empty( $wpcc_rec_list ) ) : ?>
		<div class="wpcc-rec-empty">
			<p style="margin:0 0 4px;font-size:14px;"><strong><?php esc_html_e( 'No recommendations yet.', 'siteradian' ); ?></strong></p>
			<p style="margin:0;"><?php esc_html_e( 'When a check finds something worth your attention, it appears here. Run a scan to check this site now.', 'siteradian' ); ?></p>
		</div>
	<?php else : ?>
		<table class="wp-list-table widefat fixed striped">
			<thead><tr>
				<th><?php esc_html_e( 'Recommendation', 'siteradian' ); ?></th>
				<th style="width:90px;"><?php esc_html_e( 'Severity', 'siteradian' ); ?></th>
				<th style="width:120px;"><?php esc_html_e( 'Status', 'siteradian' ); ?></th>
				<th style="width:180px;"><?php esc_html_e( 'Actions', 'siteradian' ); ?></th>
			</tr></thead>
			<tbody>
						...
						<?php if ( $rid && ( $wpcc_can_dismiss( $status ) || $wpcc_can_resolve( $status ) ) ) : ?>
							<form method="post" style="display:inline;">
								...
								<?php if ( $wpcc_can_resolve( $status ) ) : ?>
									<button type="submit" name="wpcc_rec_action" value="resolve" class="button button-small"><?php esc_html_e( 'Resolve', 'siteradian' ); ?></button>
								<?php endif; ?>
								<?php if ( $wpcc_can_dismiss( $status ) ) : ?>
									<button type="submit" name="wpcc_rec_action" value="dismiss" class="button button-small"><?php esc_html_e( 'Dismiss', 'siteradian' ); ?></button>
								<?php endif; ?>
							</form>
						<?php else : ?>
	...
	<h2 style="margin-top:26px;"><?php esc_html_e( 'Suggested fixes awaiting your approval', 'siteradian' ); ?></h2>
	<?php if ( empty( $wpcc_pending_plans ) ) : ?>
		<div class="wpcc-rec-empty">
			<p style="margin:0;"><?php esc_html_e( 'Nothing waiting. When a recommendation produces a fix that needs your sign-off, it appears here.', 'siteradian' ); ?></p>
		</div>
	<?php else : ?>
		<table class="wp-list-table widefat fixed striped">
			<thead><tr><th><?php esc_html_e( 'Title', 'siteradian' ); ?></th><th><?php esc_html_e( 'Objective', 'siteradian' ); ?></th><th style="width:180px;"><?php esc_html_e( 'Actions', 'siteradian' ); ?></th></tr></thead>
			<tbody>
				...
				<tr>
					...
					<td>
						<form method="post" style="display:inline;">
							...
							<button type="submit" name="wpcc_rec_action" value="approve_plan" class="button button-primary button-small"><?php esc_html_e( 'Approve', 'siteradian' ); ?></button>
							<button type="submit" name="wpcc_rec_action" value="reject_plan" class="button button-small"><?php esc_html_e( 'Reject', 'siteradian' ); ?></button>
						</form>
					</td>
				</tr>
			<?php endforeach; ?>
