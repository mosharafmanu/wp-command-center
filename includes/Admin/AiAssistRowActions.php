...
final class AiAssistRowActions {
	 ...
	 * action's Builder). The visible label is "✨ SiteRadian" (the sparkle
	 * is decorative, `aria-hidden`); the `aria-label` gives screen-reader users the full
	 * "SiteRadian" attribution + purpose, and `aria-haspopup="dialog"` signals the
	 * chooser. Branded so it is unmistakably SiteRadian and not a generic competitor "magic wand".
	 *
	 * @param string[] $ids
	 */
	private function anchor( \WP_Post $post, array $ids ): string {
		...
		return '<a href="' . esc_url( $fallback ) . '"'
			...
			. ' aria-label="' . esc_attr__( 'SiteRadian — AI actions', 'siteradian' ) . '">'
			. '<span aria-hidden="true">✨ </span>'
			. esc_html__( 'SiteRadian', 'siteradian' )
			. '</a>';
	}
}
