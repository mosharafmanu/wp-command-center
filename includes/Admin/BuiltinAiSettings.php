	...
	public static function handle_post(): ?array {
		...
		if ( self::is_config_controlled( $key ) ) {
			return [
				...
				'message' => sprintf( __( '%s is controlled by your site configuration and can’t be changed here.', 'siteradian' ), $label ),
			];
		}
		if ( self::set( $key, $on ) ) {
			( new AuditLog() )->record(
		...
		if ( ! $on ) {
			/* translators: %s: tool name */
			return [ 'type' => 'success', 'message' => sprintf( __( '%s is now turned off.', 'siteradian' ), $label ) ];
		}
		if ( ! AdoptionStatus::ai_configured() ) {
			/* translators: %s: tool name */
			return [ 'type' => 'success', 'message' => sprintf( __( '%s is on. Connect an AI provider to start generating.', 'siteradian' ), $label ) ];
		}
		/*
		...
		return [
			...
			'message' => sprintf( __( '%1$s is on and ready — open the %2$s tab above to use it.', 'siteradian' ), $label, $label ),
		];
	}
}
