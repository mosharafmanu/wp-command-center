...
final class DiffRenderer {
	...
	public static function render_summary( array $summary ): string {
		...
		$html .= '<span class="wpcc-diff-stat">' . esc_html( sprintf(
			...
			_n( '%d file changed', '%d files changed', $files_changed, 'action-steward' ),
			$files_changed
		) ) . '</span> ';
	...
	public static function render_file_diff( string $diff ): string {
		if ( '' === trim( $diff ) ) {
			return '<p class="description">' . esc_html__( 'No textual changes.', 'action-steward' ) . '</p>';
		}
		...
		if ( $truncated ) {
			$html .= '<p class="description wpcc-diff-truncated">' . esc_html( sprintf(
				...
				_n( 'Diff truncated — %d more line not shown.', 'Diff truncated — %d more lines not shown.', $total - self::MAX_LINES, 'action-steward' ),
