...
final class ProposalStore {
	...
	public function create( array $args ): array|\WP_Error {
		...
		if ( '' === $operation_id ) {
			return new \WP_Error( 'wpcc_proposal_missing_operation_id', __( 'operation_id is required.', 'siteradian' ) );
		}
		if ( '' === $target_type ) {
			return new \WP_Error( 'wpcc_proposal_missing_target_type', __( 'target_type is required.', 'siteradian' ) );
		}
		if ( ! array_key_exists( 'payload', $args ) || ! is_array( $args['payload'] ) ) {
			return new \WP_Error( 'wpcc_proposal_missing_payload', __( 'payload (array) is required.', 'siteradian' ) );
		}
		...
		if ( false === $inserted ) {
			return new \WP_Error( 'wpcc_proposal_create_failed', __( 'Failed to create proposal.', 'siteradian' ) );
		}

		$row = $this->get( $proposal_id );
		return $row ?: new \WP_Error( 'wpcc_proposal_create_failed', __( 'Proposal created but could not be read back.', 'siteradian' ) );
	}
...
// phpcs:enable WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter,WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery
	...
	public function update_final_payload( string $proposal_id, array $final_payload ): array|\WP_Error {
		...
		$row = $this->get( $proposal_id );
		if ( ! $row ) {
			return new \WP_Error( 'wpcc_proposal_not_found', __( 'Proposal not found.', 'siteradian' ) );
		}
		if ( self::STATUS_DRAFT !== $row['status'] ) {
			return new \WP_Error(
				'wpcc_proposal_not_editable',
				sprintf( /* translators: %s: value */ __( 'final_payload is editable only while draft (current: %s).', 'siteradian' ), (string) $row['status'] )
			);
		}
		...
		if ( false === $updated ) {
			return new \WP_Error( 'wpcc_proposal_update_failed', __( 'Failed to update proposal payload.', 'siteradian' ) );
		}
		return $this->get( $proposal_id ) ?: new \WP_Error( 'wpcc_proposal_not_found', __( 'Proposal not found.', 'siteradian' ) );
	}
	...
	public function mark_pending_approval( string $proposal_id, string $request_id ): array|\WP_Error {
		if ( '' === $request_id ) {
			return new \WP_Error( 'wpcc_proposal_request_id_required', __( 'request_id is required to mark a proposal pending_approval.', 'siteradian' ) );
		}
	...
	public function mark_applied( string $proposal_id, string $change_id, ?array $applied_by = null ): array|\WP_Error {
		if ( '' === $change_id ) {
			return new \WP_Error( 'wpcc_proposal_change_id_required', __( 'change_id is required to mark a proposal applied.', 'siteradian' ) );
		}
	...
	public function mark_failed( string $proposal_id, array|string $error ): array|\WP_Error {
		...
		if ( '' === $error_json || '[]' === $error_json || 'null' === $error_json ) {
			return new \WP_Error( 'wpcc_proposal_error_required', __( 'A non-empty error is required to mark a proposal failed.', 'siteradian' ) );
		}
	...
	private function transition( string $proposal_id, string $to, array $fields ): array|\WP_Error {
		...
		if ( in_array( $from, self::TERMINAL, true ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: 1: current status, 2: requested status */ __( 'Proposal is terminal (%1$s); cannot transition to %2$s.', 'siteradian' ), $from, $to )
			);
		}
		// Legality.
		...
		if ( ! in_array( $to, $allowed, true ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: 1: current status, 2: requested status */ __( 'Illegal transition %1$s -> %2$s.', 'siteradian' ), $from, $to )
			);
		}
		...
		if ( false === $updated ) {
			return new \WP_Error( 'wpcc_proposal_update_failed', __( 'Failed to update proposal status.', 'siteradian' ) );
		}
		if ( 0 === $updated ) {
			...
			return new \WP_Error( 'wpcc_proposal_conflict', __( 'Proposal state changed concurrently; transition not applied.', 'siteradian' ) );
		}
