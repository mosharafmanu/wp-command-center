...
final class ProposalSync {
		...
		if ( $outcome->is_success() ) {
			...
			if ( '' === $change_id ) {
				return $this->store->mark_failed( $proposal_id, [
					...
					'message' => __( 'Request executed but no change record was found to attribute it.', 'siteradian' ),
				] );
			}
	...
	private function durable_error( string $request_id, string $fallback_code ): array {
		...
		return [ 'code' => $fallback_code, 'message' => __( 'The approved request failed during execution.', 'siteradian' ) ];
	}
