...
final class ProposalApplyService {
	...
	public function apply( string $proposal_id, array $context = [] ): array|\WP_Error {
		...
		if ( ProposalStore::STATUS_DRAFT !== $proposal['status'] ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %s: value */ __( 'Only a draft proposal can be applied (current: %s).', 'siteradian' ), (string) $proposal['status'] )
			);
		}
		...
		if ( $outcome->is_gated() ) {
			...
			if ( '' === $request_id ) {
				return new \WP_Error( 'wpcc_apply_gated_no_request', __( 'Apply was gated for approval but no request id was returned.', 'siteradian' ) );
			}
		...
		if ( '' === $change_id ) {
			...
			$error = [
				...
				'message' => __( 'Apply succeeded but no change record was found to attribute it.', 'siteradian' ),
			];
			$this->store->mark_failed( $proposal_id, $error );
	...
	private function resolve_payload( array $proposal ): array|\WP_Error {
		...
		if ( ! is_array( $decoded ) ) {
			return new \WP_Error( 'wpcc_proposal_payload_invalid', __( 'Proposal payload is not a valid object.', 'siteradian' ) );
		}
		return $decoded;
	}
