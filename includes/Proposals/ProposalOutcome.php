	...
	public static function interpret( mixed $envelope ): self {
		if ( ! is_array( $envelope ) ) {
			return new self( self::KIND_HARD_FAILURE, '', [
				...
				'message' => __( 'Apply failed (no result).', 'siteradian' ),
			] );
		}
	...
	private static function error_from_envelope( array $envelope, array $inner ): array {
		...
		if ( ! empty( $first ) ) {
			return [
				...
				'message' => (string) ( $first['message'] ?? __( 'Apply failed.', 'siteradian' ) ),
			];
		}
		...
		if ( ! empty( $inner['error'] ) ) {
			return [
				...
				'message' => (string) ( $inner['message'] ?? __( 'Apply failed.', 'siteradian' ) ),
			];
		}
		return [ 'code' => 'wpcc_apply_failed', 'message' => __( 'Apply failed.', 'siteradian' ) ];
	}
