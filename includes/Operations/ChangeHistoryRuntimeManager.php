...
final class ChangeHistoryRuntimeManager {
	...
	private function operation_status( array $p ): array {
		...
		if ( '' === $key ) {
			return $this->err( 'wpcc_missing_idempotency_key', __( 'idempotency_key is required for operation_status.', 'siteradian' ) );
		}
	...
	private function history_get( array $p ): array {
		...
		if ( '' === $change_id ) {
			return $this->err( 'wpcc_missing_change_id', __( 'change_id is required for history_get.', 'siteradian' ) );
		}
		...
		if ( ! is_array( $row ) ) {
			return $this->err( 'wpcc_change_not_found', sprintf(
				...
				__( 'No change found for change_id %s.', 'siteradian' ),
				$change_id
			) );
		}
	...
	private function rollback_discover( array $p ): array {
		...
		if ( ! $has_selector ) {
			return $this->err( 'wpcc_missing_rollback_selector', __( 'rollback_discover requires one of: target, change_set_id, or change_id.', 'siteradian' ) );
		}
	...
	private function rollback_target( array $p, array $cx ): array {
		...
		if ( AuthTokens::SCOPE_READ_ONLY === (string) ( $cx['token_scope'] ?? '' ) ) {
			return $this->err( 'wpcc_token_read_only', __( 'This API token is read-only and cannot perform rollback_target.', 'siteradian' ) );
		}
		...
		if ( '' === $change_id && '' !== $rollback_id ) {
			...
			if ( '' === $change_id ) {
				return $this->err( 'wpcc_rollback_id_not_found', sprintf(
					...
					__( 'No recorded change carries rollback_id %s. It may belong to a patch — reverse those with patch_manage — or the change may predate change recording.', 'siteradian' ),
					$rollback_id
				) );
			}
		}
		...
		if ( '' === $change_id ) {
			return $this->err( 'wpcc_missing_change_id', __( 'Pass change_id, or rollback_id as returned by the write you want to undo.', 'siteradian' ) );
		}
		...
		if ( 'rolled_back' === (string) $row['status'] ) {
			return $this->err( 'wpcc_already_rolled_back', __( 'This change has already been rolled back.', 'siteradian' ) );
		}
		...
		if ( 1 !== $reversible || 'none' === $kind ) {
			return $this->err( 'wpcc_not_reversible', __( 'This change is not reversible.', 'siteradian' ) );
		}
		...
		if ( 'patch' === $kind ) {
			...
			if ( '' === $patch_id ) {
				return $this->err( 'wpcc_missing_rollback_id', __( 'No patch id is linked to this change.', 'siteradian' ) );
			}
			...
			if ( '' === $rollback_id ) {
				return $this->err( 'wpcc_missing_rollback_id', __( 'No rollback id is linked to this change.', 'siteradian' ) );
			}
			...
			if ( empty( $res['success'] ) ) {
				return $this->err(
					...
					(string) ( $res['message'] ?? __( 'Rollback failed.', 'siteradian' ) )
				);
			}
			$engine_res = $res;
		} else {
			return $this->err( 'wpcc_unsupported_rollback_kind', sprintf(
				...
				__( 'Unsupported rollback kind: %s.', 'siteradian' ),
				$kind
			) );
		}
