...
final class FormsRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		if ( ! isset( $this->providers[ $provider ] ) ) {
			return $this->error( 'wpcc_provider_not_available', sprintf( /* translators: %s: value */ __( 'Form provider "%s" is not available.', 'siteradian' ), $provider ) );
		}
	...
	public function rollback( array $payload, array $context = [] ): array {
		...
		if ( $rec['rollback_applied'] ) return $this->error( 'wpcc_rollback_already_applied', __( 'Already applied.', 'siteradian' ) );
		// Undo create = delete
		$act = $rec['action'];
		$eid = $rec['entity_id'];
	...
	private function mutation( string $action, FormsProvider $p, array $payload, array $context, string $method, string $rollback_action ): array {
		...
		return $result ?: $this->error( 'wpcc_mutation_failed', __( 'Operation failed.', 'siteradian' ) );
	}
