...
final class SnapshotManager {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		return match ( $action ) {
			...
			default => new \WP_Error( 'wpcc_invalid_snapshot_action', __( 'Unknown action.', 'action-steward' ) ),
		};
	}
	...
	private function create( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $path ) {
			return new \WP_Error( 'wpcc_missing_snapshot_path', __( 'File path is required for snapshot creation.', 'action-steward' ) );
		}
	...
	private function details( array $params ): array|\WP_Error {
		...
		if ( '' === $id ) {
			return new \WP_Error( 'wpcc_missing_snapshot_id', __( 'snapshot_id is required.', 'action-steward' ) );
		}
