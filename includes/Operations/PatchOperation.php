...
final class PatchOperation {
	...
	private function preview( array $params, array $context ): array|\WP_Error {
		...
		return array_merge( [
			...
			'verification'       => [
				...
				'note'   => __( 'Preview uses an in-memory tokenizer check; apply re-verifies the written file with php -l when available.', 'siteradian' ),
			],
	...
	private function confirmation_contract( array $dangerous ): array {
		...
		return [
			...
			'confirmation_params'   => [
				'confirm'             => true,
				'confirmation_phrase' => DestructiveGuard::PHRASE_PATCH,
				'reason'              => __( 'a human-readable reason for applying this high-risk change', 'siteradian' ),
			],
			'confirmation_hint'     => sprintf(
				/* translators: 1: comma-separated high-risk paths, 2: required phrase */
				__( 'This change touches high-risk file(s): %1$s. To apply, call patch_apply with confirm=true, confirmation_phrase="%2$s", and a reason.', 'siteradian' ),
				implode( ', ', $dangerous ),
	...
	private function create( array $params, array $context ): array|\WP_Error {
		...
		return array_merge( [
			...
			'deploy_tracked_note' => empty( $deploy_tracked ) ? '' : __( 'One or more target files are inside a version-controlled (git) directory. If this site uses a deploy pipeline, changes applied here may be overwritten by the next deploy — commit them to the source repository as well.', 'siteradian' ),
	...
	private function apply( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $patch_id ) {
			return new \WP_Error( 'wpcc_missing_patch_id', __( 'patch_id is required.', 'siteradian' ) );
		}
	...
	private function normalize_files( $files ): array|\WP_Error {
		if ( ! is_array( $files ) || empty( $files ) ) {
			return new \WP_Error( 'wpcc_no_files', __( 'A patch must include at least one file. Each entry needs a path plus the fields for its mode (default whole_file uses { path, modified }).', 'siteradian' ) );
		}
		...
		foreach ( $files as $file ) {
			if ( ! is_array( $file ) ) {
				return new \WP_Error( 'wpcc_invalid_file', __( 'Each file must be an object with a path and the fields for its patch mode.', 'siteradian' ) );
			}
			...
			if ( '' === $path ) {
				return new \WP_Error( 'wpcc_invalid_path', __( 'Each file must have a path.', 'siteradian' ) );
			}
				...
				if ( 'wpcc_not_found' === $real->get_error_code() ) {
					return new \WP_Error(
						...
						sprintf(
							...
							__( 'There is no file at "%s". A patch edits a file that already exists — it cannot create one. Create the file another way first, then patch it.', 'siteradian' ),
							$path
						)
					);
				}
			...
			if ( ! is_file( $real ) || ! is_readable( $real ) ) {
				return new \WP_Error( 'wpcc_not_readable', sprintf( /* translators: %s: value */ __( '%s is not readable.', 'siteradian' ), $path ) );
			}
				...
				if ( $chained ) {
					return new \WP_Error(
						...
						sprintf(
							...
							__( 'Conflicting edits on "%1$s" in one change set: a later op could not apply on the result of the earlier op(s) — %2$s. The whole change set was rejected so no partial change is written; re-express the edits against the evolving content, or split them into separate change sets.', 'siteradian' ),
							$path,
	...
	private function reject_unknown_params( array $params ): ?\WP_Error {
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Unknown parameter(s): %1$s. Allowed: %2$s. File contents go inside files[] (e.g. { path, modified }).', 'siteradian' ),
				implode( ', ', $unknown ),
	...
	private static function summarize_entries( array $entries, string $base_risk, ?string $change_set_id ): array {
		...
		$summary = sprintf(
			...
			__( '%1$s: %2$d file(s) [%3$s], +%4$d/-%5$d lines, risk: %6$s%7$s.', 'siteradian' ),
			$is_change_set ? __( 'Change set', 'siteradian' ) : __( 'Single-file patch', 'siteradian' ),
			$count,
			implode( ', ', $umodes ),
			$added,
			$removed,
			...
			$has_high_risk ? sprintf(
				...
				__( '; %d high-risk path(s) included', 'siteradian' ),
				count( $dangerous )
			) : ''
		);
