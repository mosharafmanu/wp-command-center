...
final class CacheRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		switch ( $action ) {
			...
			case 'cache_describe':
				return [ 'action' => 'cache_describe', 'runtime' => 'cache_manage', 'actions' => self::ACTIONS, 'notes' => __( 'cache_purge_all clears every detected layer + object cache. cache_purge_url purges one URL where the layer supports it. cache_status only detects. No shell required.', 'siteradian' ) ];
			default:
				return $this->error(
					'wpcc_invalid_cache_action',
					sprintf(
						/* translators: 1: invalid action, 2: valid actions */
						__( 'Invalid cache action "%1$s". Valid actions: %2$s.', 'siteradian' ),
						$action,
	...
	private function purge_url( array $p, array $context ): array {
		...
		if ( '' === $url ) {
			return $this->error( 'wpcc_missing_url', __( "cache_purge_url requires a 'url' parameter (the page URL to purge).", 'siteradian' ) );
		}
