...
final class WorkflowRuntimeManager {
	...
	private function get_workflow(array $p):array{
		...
		return isset($w[$id])?['workflow'=>$w[$id]]:$this->err('nf',__('Workflow not found.','siteradian'));
	}

	private function create(array $p,array $cx):array{
		$name=sanitize_text_field((string)($p['name']??''));if(''===$name)return$this->err('missing_name',__('Name required.','siteradian'));
	...
	private function update(array $p,array $cx):array{
		$id=sanitize_key((string)($p['workflow_id']??''));$w=$this->load();
		if(!isset($w[$id]))return$this->err('nf',__('Not found.','siteradian'));
	...
	private function execute(array $p,array $cx):array{
		...
		foreach($steps as $i=>$step){
			...
			if(!empty($unresolved)){
				$msg=__('Unresolved step reference(s): ','siteradian').implode(', ',$unresolved);
	...
	private function rollback_execution(array $p,array $cx):array{
		...
		if(''===$exec_id)return$this->err('missing_execution_id',__('execution_id required.','siteradian'));
		$history=get_option('wpcc_workflow_history',[]);$idx=null;
		foreach($history as $i=>$h){if(($h['execution_id']??'')===$exec_id){$idx=$i;break;}}
		if(null===$idx)return$this->err('execution_not_found',__('Execution not found.','siteradian'));
		if(!empty($history[$idx]['rolled_back']))return$this->err('already_rolled_back',__('Execution already rolled back.','siteradian'));
		$completed=[];
		...
		if(!$completed)return$this->err('nothing_to_rollback',__('No rollbackable steps in this execution.','siteradian'));
	...
	private function import(array $p,array $cx):array{
		$json=(string)($p['json']??'');if(''===$json)return$this->err('missing',__('JSON required.','siteradian'));
		$data=json_decode($json,true);if(!$data||!isset($data['name']))return$this->err('invalid_json',__('Invalid workflow JSON.','siteradian'));
