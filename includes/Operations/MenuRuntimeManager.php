...
final class MenuRuntimeManager {
	...
	private function menu_get( array $p ): array {
		...
		if ( ! $menu ) return $this->err( 'wpcc_menu_not_found', __( 'Menu not found.', 'action-steward' ) );
	...
	private function menu_create( array $p, array $cx ): array {
		...
		if ( '' === $name ) return $this->err( 'wpcc_missing_name', __( 'Menu name is required.', 'action-steward' ) );
	...
	private function menu_update( array $p, array $cx ): array {
		...
		if ( ! $m ) return $this->err( 'wpcc_menu_not_found', __( 'Menu not found.', 'action-steward' ) );
	...
	private function menu_item_get( array $p ): array {
		...
		if ( ! $item || 'nav_menu_item' !== $item->post_type ) return $this->err( 'wpcc_item_not_found', __( 'Menu item not found.', 'action-steward' ) );
	...
	private function menu_item_add( array $p, array $cx ): array {
		...
		if ( '' === $title ) return $this->err( 'wpcc_missing_title', __( 'Title is required.', 'action-steward' ) );
	...
	private function menu_item_move( array $p, array $cx ): array {
		...
		if ( ! $item ) return $this->err( 'wpcc_item_not_found', __( 'Menu item not found.', 'action-steward' ) );
	...
	public function rollback( array $p, array $cx = [] ): array {
		...
		if ( '' === $rid ) return $this->err( 'wpcc_missing_rb_id', __( 'Rollback ID required.', 'action-steward' ) );
		$rollbacks = get_option( 'wpcc_menu_rollbacks', [] ); $rec = null; $idx = null;
		foreach ( $rollbacks as $i => $r ) { if ( $r['id'] === $rid ) { $rec = $r; $idx = $i; break; } }
		if ( ! $rec ) return $this->err( 'wpcc_rb_not_found', __( 'Not found.', 'action-steward' ) );
		if ( $rec['rollback_applied'] ) return $this->err( 'wpcc_rb_already', __( 'Already applied.', 'action-steward' ) );
