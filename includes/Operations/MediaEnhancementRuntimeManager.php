...
final class MediaEnhancementRuntimeManager {
	...
	private function resolve_image_id( array $p ): int|\WP_Error {
		...
		if ( ! $post || 'attachment' !== $post->post_type ) {
			return new \WP_Error( 'wpcc_media_not_found', __( 'Media not found.', 'siteradian' ) );
		}
		if ( ! wp_attachment_is_image( $id ) ) {
			return new \WP_Error( 'wpcc_not_an_image', __( 'Attachment is not an image.', 'siteradian' ) );
		}
		return $id;
	}
	...
	private function do_regenerate( int $id, string $mode, string $batch_id, array $cx ) {
		...
		if ( ! $original || ! is_file( $original ) ) {
			return new \WP_Error( 'wpcc_media_no_files', __( 'Attachment has no original file on disk.', 'siteradian' ) );
		}
		...
		if ( empty( $targets ) ) {
			return [
				...
				'message'     => __( 'No regeneration required; all applicable sizes are present.', 'siteradian' ),
			];
		}
		...
		if ( is_wp_error( $snapshot ) ) {
			return new \WP_Error( 'wpcc_thumbnail_snapshot_failed', sprintf( /* translators: %s: value */ __( 'Could not snapshot the attachment before regeneration: %s', 'siteradian' ), $snapshot->get_error_message() ) );
		}
		...
		if ( 'all' === $mode ) {
			...
			if ( is_wp_error( $new_meta ) || ! is_array( $new_meta ) ) {
				...
				return new \WP_Error( 'wpcc_thumbnail_regenerate_failed', __( 'Regeneration failed to produce metadata; restored pre-regeneration state.', 'siteradian' ) );
			}
			...
			if ( is_wp_error( $editor ) ) {
				( new MediaSnapshot() )->restore( $snapshot_id );
				( new MediaSnapshot() )->delete( $snapshot_id );
				return new \WP_Error( 'wpcc_thumbnail_regenerate_failed', sprintf( /* translators: %s: value */ __( 'No usable image editor: %s; restored pre-regeneration state.', 'siteradian' ), $editor->get_error_message() ) );
			}
			$to_make = [];
		...
		if ( ! empty( $still_missing ) ) {
			...
			( new MediaSnapshot() )->delete( $snapshot_id );
			return new \WP_Error( 'wpcc_thumbnail_regenerate_failed', sprintf( /* translators: %s: value */ __( 'Regeneration did not produce: %s. Restored pre-regeneration state.', 'siteradian' ), implode( ', ', $still_missing ) ) );
		}
	...
	private function webp_generate( array $p, array $cx = [] ): array|\WP_Error {
		if ( ! $this->webp_encode_available() ) {
			return new \WP_Error( 'wpcc_image_lib_unavailable', __( 'WebP encoding is not available on this server (no GD/Imagick WebP support).', 'siteradian' ) );
		}
		$id = $this->resolve_image_id( $p );
		if ( is_wp_error( $id ) ) {
			return $id;
	...
	private function do_webp_generate( int $id, string $batch_id, array $cx ) {
		...
		if ( ! in_array( $mime, self::WEBP_SOURCE_MIMES, true ) ) {
			return new \WP_Error( 'wpcc_webp_unsupported_mime', sprintf( /* translators: %s: value */ __( 'WebP cannot be generated from this mime type (%s); JPEG/PNG only.', 'siteradian' ), $mime ?: 'unknown' ) );
		}

		$files = $this->attachment_image_files( $id );
		$original = get_attached_file( $id );
		if ( empty( $files ) || ! $original || ! is_file( $original ) ) {
			return new \WP_Error( 'wpcc_media_no_files', __( 'Attachment has no source files on disk to convert.', 'siteradian' ) );
		}
		...
		if ( empty( $targets ) ) {
			return [
				...
				'count_generated'  => 0,
				'message'          => __( 'WebP already present for all image files.', 'siteradian' ),
			];
		}
		...
		if ( is_wp_error( $snapshot ) ) {
			return new \WP_Error( 'wpcc_webp_snapshot_failed', sprintf( /* translators: %s: value */ __( 'Could not snapshot the attachment before WebP generation: %s', 'siteradian' ), $snapshot->get_error_message() ) );
		}
		$snapshot_id = $snapshot['id'];
		...
		if ( empty( $generated ) ) {
			...
			( new MediaSnapshot() )->delete( $snapshot_id );
			return new \WP_Error( 'wpcc_webp_generate_failed', __( 'No WebP files could be generated; pre-generation state restored.', 'siteradian' ) );
		}
	...
	private function image_optimize( array $p, array $cx = [] ): array|\WP_Error {
		if ( ! $this->optimize_available() ) {
			return new \WP_Error( 'wpcc_image_lib_unavailable', __( 'Image optimization is not available on this server (no GD/Imagick).', 'siteradian' ) );
		}
		$id = $this->resolve_image_id( $p );
		if ( is_wp_error( $id ) ) {
			return $id;
	...
	private function do_optimize( int $id, int $quality, string $batch_id, array $cx ) {
		$mime = (string) get_post_mime_type( $id );
		if ( ! in_array( $mime, self::OPTIMIZE_SOURCE_MIMES, true ) ) {
			return new \WP_Error( 'wpcc_optimize_unsupported_mime', sprintf( /* translators: %s: value */ __( 'Image optimization does not support this mime type (%s); JPEG/PNG/WebP only.', 'siteradian' ), $mime ?: 'unknown' ) );
		}
		$files    = $this->attachment_image_files( $id );
		$original = get_attached_file( $id );
		if ( empty( $files ) || ! $original || ! is_file( $original ) ) {
			return new \WP_Error( 'wpcc_media_no_files', __( 'Attachment has no source files on disk to optimize.', 'siteradian' ) );
		}
		...
		if ( empty( $plan ) ) {
			return [
				...
				'failed'      => $failed,
				'bytes_saved' => 0,
				'message'     => __( 'No significant savings; originals left unchanged.', 'siteradian' ),
			];
		}
		...
		if ( is_wp_error( $snapshot ) ) {
			...
			return new \WP_Error( 'wpcc_optimize_snapshot_failed', sprintf( /* translators: %s: value */ __( 'Could not snapshot the attachment before optimization: %s', 'siteradian' ), $snapshot->get_error_message() ) );
		}
		$snapshot_id = $snapshot['id'];
		...
		if ( empty( $optimized ) ) {
			( new MediaSnapshot() )->restore( $snapshot_id );
			( new MediaSnapshot() )->delete( $snapshot_id );
			return new \WP_Error( 'wpcc_optimize_failed', __( 'No files could be optimized; pre-optimization state restored.', 'siteradian' ) );
		}
	...
	private function unused_media_cleanup( array $p, array $cx = [] ): array|\WP_Error {
		...
		if ( null !== $refused ) {
			return new \WP_Error( 'wpcc_media_cleanup_refused', sprintf( /* translators: %s: value */ __( 'Cleanup refused: %s. Nothing was changed.', 'siteradian' ), $refused ) );
		}

		// Snapshot BEFORE any mutation — the real reversibility guarantee.
		$snapshot = ( new MediaSnapshot() )->capture( $id, 'unused_media_cleanup' );
		if ( is_wp_error( $snapshot ) ) {
			return new \WP_Error( 'wpcc_media_cleanup_snapshot_failed', sprintf( /* translators: %s: value */ __( 'Could not snapshot the attachment before cleanup: %s', 'siteradian' ), $snapshot->get_error_message() ) );
		}
		$snapshot_id = $snapshot['id'];
		...
		if ( ! $now || 'trash' !== $now->post_status ) {
			...
			( new MediaSnapshot() )->delete( $snapshot_id );
			return new \WP_Error( 'wpcc_media_cleanup_failed', __( 'Could not trash the attachment; pre-cleanup state restored.', 'siteradian' ) );
		}
