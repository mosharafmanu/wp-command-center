...
final class MediaRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		return match ( $action ) {
			...
			default => $this->error( 'wpcc_unknown_media_action', __( 'Unknown media action.', 'siteradian' ) ),
		};
	}
	...
	private function snapshot_restore( array $payload, array $context ): array {
		...
		if ( '' === $snapshot_id ) {
			return $this->error( 'wpcc_missing_snapshot_id', __( 'snapshot_id is required.', 'siteradian' ) );
		}
	...
	private function replace_verify( array $payload ): array {
		...
		if ( ! $post || 'attachment' !== $post->post_type ) {
			return $this->error( 'wpcc_media_not_found', __( 'Media not found.', 'siteradian' ) );
		}
	...
	private function search_media( array $payload ): array {
		...
		if ( '' === $search ) {
			return $this->error( 'wpcc_media_empty_search', __( "media_search requires a 'search' parameter (the keyword to look for; aliases: query, s).", 'siteradian' ) );
		}
	...
	private function upload_media( array $payload, array $context ): array {
		...
		if ( '' === $source_url ) {
			return $this->error( 'wpcc_missing_url', __( 'Source URL is required.', 'siteradian' ) );
		}
	...
	private function update_media( array $payload, array $context ): array {
		...
		if ( ! $has_field ) {
			return $this->error( 'wpcc_media_no_fields', __( 'Provide at least one of: title, alt, caption, description.', 'siteradian' ) );
		}
	...
	private function replace_media( array $payload, array $context ): array {
		...
		if ( false === getimagesize( $tmp ) ) {
			wp_delete_file( $tmp );
			$this->discard_replace_snapshot( $rollback_id, $snapshot['id'] );
			return $this->error( 'wpcc_replace_not_image', __( 'The source file is not a valid image.', 'siteradian' ) );
		}
		...
		if ( ! $orig_path || ! @copy( $tmp, $orig_path ) ) {
			wp_delete_file( $tmp );
			$this->discard_replace_snapshot( $rollback_id, $snapshot['id'] );
			return $this->error( 'wpcc_replace_failed', __( 'Failed to write the replacement file.', 'siteradian' ) );
		}
		wp_delete_file( $tmp );
	...
	private function delete_media( array $payload, array $context ): array {
		...
		if ( ! $result ) {
			return $this->error( 'wpcc_media_delete_failed', __( 'Failed to delete media.', 'siteradian' ) );
		}
	...
	private function featured_assign( array $payload, array $context ): array {
		...
		if ( ! get_post( $post_id ) ) {
			return $this->error( 'wpcc_post_not_found', __( 'Post not found.', 'siteradian' ) );
		}
