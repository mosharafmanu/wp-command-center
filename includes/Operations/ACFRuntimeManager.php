...
final class ACFRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		if ( ! function_exists( 'acf_get_field_groups' ) ) {
			return $this->error( 'wpcc_acf_inactive', __( 'Advanced Custom Fields is not active.', 'siteradian' ) );
		}
		...
		if ( ! in_array( $a, ACFRegistry::ACTIONS, true ) ) {
			return $this->error(
				...
				sprintf(
					...
					__( 'Invalid ACF action "%1$s". Valid actions: %2$s. Call action="acf_describe" for details.', 'siteradian' ),
					$a,
		...
		return match ( $a ) {
			...
			default => $this->error( 'wpcc_unknown_acf_action', __( 'Unknown ACF action.', 'siteradian' ) ),
		};
	}
	...
	private function group_get( array $p ): array {
		...
		if ( ! $g ) return $this->error( 'wpcc_acf_group_not_found', __( 'Field group not found.', 'siteradian' ) );
	...
	private function group_create( array $p, array $cx ): array {
		...
		if ( '' === $title ) return $this->error( 'wpcc_missing_title', __( 'Title is required.', 'siteradian' ) );
		...
		if ( ! $result ) return $this->error( 'wpcc_group_create_failed', __( 'Failed to create field group.', 'siteradian' ) );
		$id = $g['key'];
	...
	private function group_update( array $p, array $cx ): array {
		...
		if ( ! $result ) return $this->error( 'wpcc_group_update_failed', __( 'Failed to update field group.', 'siteradian' ) );
	...
	private function group_delete( array $p, array $cx ): array {
		...
		if ( $this->group_will_persist( $key, $id, 0 === $stored_id ) ) {
			...
			return $this->error(
				...
				__( 'Field group still exists after delete — it is defined in a read-only local JSON/PHP source (e.g. theme acf-json) and was not removed.', 'siteradian' )
			);
		}
	...
	private function group_duplicate( array $p, array $cx ): array {
		...
		if ( ! is_array( $dup ) || empty( $dup['key'] ) ) {
			return $this->error( 'wpcc_acf_duplicate_failed', __( 'The field group could not be duplicated.', 'siteradian' ) );
		}
	...
	private function field_get( array $p ): array {
		...
		$f = acf_get_field( $key );
		if ( ! $f ) return $this->error( 'wpcc_acf_field_not_found', __( 'Field not found.', 'siteradian' ) );
	...
	private function field_create( array $p, array $cx ): array {
		...
		if ( '' === $parent ) {
			return $this->error( 'wpcc_acf_missing_parent', __( 'group_id or parent is required.', 'siteradian' ) );
		}
		if ( ! $this->parent_exists( $parent ) ) {
			return $this->error( 'wpcc_acf_parent_not_found', __( 'Parent field group or field not found.', 'siteradian' ) );
		}

		$type = sanitize_key( (string) ( $p['type'] ?? 'text' ) );
		if ( ! in_array( $type, ACFRegistry::FIELD_TYPES, true ) ) {
			return $this->error( 'wpcc_acf_unsupported_field_type', sprintf( /* translators: %s: value */ __( 'Unsupported field type: %s', 'siteradian' ), esc_html( $type ) ) );
		}
		...
		if ( 0 === $parent_ref ) {
			return $this->error(
				...
				sprintf(
					...
					__( 'Could not resolve "%s" to a stored field group or field, so the new field would not be attached to anything. Nothing was created.', 'siteradian' ),
					$parent
				)
			);
		}
		...
		if ( ! acf_update_field( $field ) ) {
			return $this->error( 'wpcc_field_create_failed', __( 'Failed to create field.', 'siteradian' ) );
		}
	...
	private function layout_create( array $p, array $cx ): array {
		...
		if ( ! $f ) {
			return $this->error( 'wpcc_acf_field_not_found', __( 'Field not found.', 'siteradian' ) );
		}
		if ( 'flexible_content' !== ( $f['type'] ?? '' ) ) {
			return $this->error( 'wpcc_acf_not_flexible', __( 'Layouts can only be added to a flexible_content field.', 'siteradian' ) );
		}
		...
		if ( '' === $name ) {
			return $this->error( 'wpcc_acf_missing_layout_name', __( 'A layout name or label is required.', 'siteradian' ) );
		}
		...
		if ( ! acf_update_field( $f ) ) {
			return $this->error( 'wpcc_acf_layout_create_failed', __( 'Failed to create layout.', 'siteradian' ) );
		}
	...
	private function layout_update( array $p, array $cx ): array {
		...
		if ( ! $f || 'flexible_content' !== ( $f['type'] ?? '' ) ) {
			return $this->error( 'wpcc_acf_not_flexible', __( 'Flexible_content field not found.', 'siteradian' ) );
		}
		$f = $this->with_stored_identity( $f, $field_key );
		...
		if ( null === $target ) {
			return $this->error( 'wpcc_acf_layout_not_found', __( 'Layout not found on this field.', 'siteradian' ) );
		}
		...
		if ( ! acf_update_field( $f ) ) {
			return $this->error( 'wpcc_acf_layout_update_failed', __( 'Failed to update layout.', 'siteradian' ) );
		}
	...
	private function location_assign( array $p, array $cx ): array {
		...
		if ( empty( $and_group ) ) {
			return $this->error( 'wpcc_acf_invalid_location', __( 'A location rule { param, operator, value } is required.', 'siteradian' ) );
		}
	...
	private function json_export( array $p ): array {
		$id = sanitize_text_field( (string) ( $p['group_id'] ?? '' ) );
		if ( '' === $id ) return $this->error( 'wpcc_missing_id', __( 'Group ID is required for export.', 'siteradian' ) );
		...
		if ( [] === $group ) return $this->error( 'wpcc_acf_group_not_found', __( 'Field group not found.', 'siteradian' ) );
	...
	private function json_import( array $p, array $cx ): array {
		$json = (string) ( $p['json'] ?? '' );
		if ( '' === $json ) return $this->error( 'wpcc_missing_json', __( 'JSON content is required.', 'siteradian' ) );
		$data = json_decode( $json, true );
		if ( ! $data ) return $this->error( 'wpcc_invalid_json', __( 'Invalid JSON.', 'siteradian' ) );
		...
		if ( ! $imported ) return $this->error( 'wpcc_import_failed', __( 'Failed to import field group.', 'siteradian' ) );
	...
	private function json_sync( array $p, array $cx ): array {
		...
		if ( '' === $only && ! $all ) {
			return $this->error(
				...
				__( 'Pass group_id to synchronise one field group, or all_groups: true to synchronise every group on the site. Rewriting every acf-json file by default would touch groups this plugin did not create.', 'siteradian' )
			);
		}
		$groups    = acf_get_field_groups();
		$written   = [];
		...
		if ( ! AcfLocalJson::enabled() ) {
			return $this->error( 'wpcc_acf_json_disabled', __( 'ACF local JSON is not enabled, or its save directory is not writable, so there is nothing to write.', 'siteradian' ) );
		}
	...
	private function json_diff( array $p ): array {
		$id = sanitize_text_field( (string) ( $p['group_id'] ?? '' ) );
		if ( '' === $id ) return $this->error( 'wpcc_missing_id', __( 'Group ID required.', 'siteradian' ) );
		...
		if ( 'group_not_stored' === $st['reason'] ) {
			return $this->error( 'wpcc_acf_group_not_found', __( 'Field group not found.', 'siteradian' ) );
		}
		return [
	...
	private function value_get( array $p ): array {
		...
		if ( '' === $key ) return $this->error( 'wpcc_missing_field', __( 'Field key or name is required.', 'siteradian' ) );
		...
		if ( '' !== $object_type ) {
			...
			if ( $current <= 0 ) {
				return $this->error(
					...
					__( 'No object to read the field from. Pass object_type ("post", "term", "user" or "option") together with object_id — for a post, post_id also works. Without one of those there is no current object in an API request, and an empty answer would be indistinguishable from a field that has no value.', 'siteradian' )
				);
			}
			$selector = false;
		}
		...
		if ( ! $definition ) {
			return $this->error(
				...
				sprintf(
					...
					__( 'No ACF field named "%s" is registered on this site. Use acf_field_list to see the fields of a group; an unknown field is reported here rather than as an empty value.', 'siteradian' ),
					$key
				)
			);
		}
	...
	private function layout_usage( array $p ): array {
		...
		if ( '' === $layout ) return $this->error( 'wpcc_missing_layout', __( 'A layout name is required.', 'siteradian' ) );
	...
	private function value_set( array $p, array $cx ): array {
		...
		if ( empty( $fields ) ) {
			return $this->error( 'wpcc_no_fields', __( 'No fields supplied. Provide fields:{field_key:value} (or field_key + value).', 'siteradian' ) );
		}
		...
		foreach ( array_keys( $fields ) as $rawk ) {
			...
			if ( ! $field_object ) {
				return $this->error( 'wpcc_unknown_acf_field', sprintf( /* translators: %s: value */ __( 'ACF field "%s" not found.', 'siteradian' ), $k ) );
			}
			if ( 'no' === $this->field_targets_object_type( $field_object, $object_type ) ) {
				return $this->error( 'wpcc_acf_location_mismatch', sprintf(
					...
					__( 'ACF field "%1$s" does not apply to a %2$s — its field group location targets a different object type. Refusing to write to avoid corrupting unrelated data.', 'siteradian' ),
					$k,
					$object_type
				) );
			}
	...
	private function resolve_acf_selector( string $type, int $id ) {
		switch ( $type ) {
			case 'post':
				if ( $id <= 0 || ! get_post( $id ) ) {
					return $this->error( 'wpcc_invalid_object', __( 'Invalid or non-existent post ID.', 'siteradian' ) );
				}
				return $id;
			case 'term':
				$term = $id > 0 ? get_term( $id ) : null;
				if ( ! $term || is_wp_error( $term ) ) {
					return $this->error( 'wpcc_invalid_object', __( 'Invalid or non-existent term ID.', 'siteradian' ) );
				}
				return 'term_' . $id;
			case 'user':
				if ( $id <= 0 || ! get_userdata( $id ) ) {
					return $this->error( 'wpcc_invalid_object', __( 'Invalid or non-existent user ID.', 'siteradian' ) );
				}
				return 'user_' . $id;
			case 'option':
				return 'option';
			default:
				return $this->error( 'wpcc_invalid_object_type', __( 'object_type must be one of: post, term, user, option.', 'siteradian' ) );
		}
	}
	...
	public function rollback( array $p, array $cx = [] ): array {
		...
		if ( '' === $rid ) return $this->error( 'wpcc_missing_rollback_id', __( 'Rollback ID required.', 'siteradian' ) );
		...
		if ( ! $rec ) return $this->error( 'wpcc_rollback_not_found', __( 'Rollback not found.', 'siteradian' ) );
		if ( ! empty( $rec['rollback_applied'] ) ) return $this->error( 'wpcc_rollback_already_applied', __( 'Already applied.', 'siteradian' ) );
		$eid   = $rec['entity_id'];
		$act   = $rec['action'];
		...
		if ( 'json_import' === $act ) {
			return $this->rollback_unsupported( __( 'ACF JSON import cannot be automatically rolled back.', 'siteradian' ) );
		}
		...
		if ( in_array( $act, self::FP_GUARDED, true ) && isset( $before['__after_fp'] ) ) {
			if ( $this->definition_fingerprint( (string) $eid, $act ) !== (string) $before['__after_fp'] ) {
				return $this->rollback_conflict( $rid, __( 'ACF definition changed since this update was applied; rollback skipped to avoid clobbering the newer change.', 'siteradian' ) );
			}
		}
		...
		if ( in_array( $act, [ 'group_create', 'field_create' ] ) ) {
			if ( str_starts_with( $act, 'group' ) ) {
				...
				if ( ! $deleted || $this->group_will_persist( $key, (string) $eid, 0 === $stored_id ) ) {
					return $this->error( 'wpcc_acf_group_delete_failed', __( 'Field group still exists after rollback.', 'siteradian' ) );
				}
				...
				if ( ! $deleted ) {
					return $this->error( 'wpcc_acf_field_delete_failed', __( 'Field still exists after rollback.', 'siteradian' ) );
				}
			}
	...
	private function rollback_value_delta( PostMetaRollbackStore $store, string $rid, array $resolved ): array {
		...
		if ( ! empty( $rec['rollback_applied'] ) ) {
			return $this->error( 'wpcc_rollback_already_applied', __( 'Already applied.', 'siteradian' ) );
		}
