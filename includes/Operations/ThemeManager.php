...
final class ThemeManager {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		if ( ThemeRegistry::ACTION_LIST !== $action && 'theme_rollback' !== $action && '' === $slug ) {
			return new \WP_Error( 'wpcc_missing_theme_slug', __( 'Theme slug is required for this action.', 'siteradian' ) );
		}
		...
		return match ( $action ) {
			...
			default => new \WP_Error( 'wpcc_invalid_theme_action', __( 'Unknown theme action.', 'siteradian' ) ),
		};
	}
	...
	private function theme_install( string $slug, array $context ): array|\WP_Error {
		if ( $this->registry->is_installed( $slug ) ) {
			return new \WP_Error( 'wpcc_theme_already_installed', __( 'Theme is already installed.', 'siteradian' ) );
		}
	...
	private function theme_activate( string $slug, array $context ): array|\WP_Error {
		...
		if ( $target['active'] ) {
			return new \WP_Error( 'wpcc_theme_already_active', __( 'Theme is already active.', 'siteradian' ) );
		}
	...
	private function theme_update( string $slug, array $context ): array|\WP_Error {
		...
		if ( ! $info['update_available'] ) {
			return new \WP_Error( 'wpcc_theme_no_update', __( 'No update available for this theme.', 'siteradian' ) );
		}
		...
		return [
			...
			'reversible'      => false,
			'reversible_note' => __( 'Theme updates are not automatically reversible; no rollback is captured. Snapshot before updating if reversibility is required.', 'siteradian' ),
		];
	}
	...
	private function theme_delete( string $slug, array $context ): array|\WP_Error {
		...
		if ( $info['active'] ) {
			return new \WP_Error( 'wpcc_theme_delete_active', __( 'Cannot delete the active theme. Activate another theme first.', 'siteradian' ) );
		}
