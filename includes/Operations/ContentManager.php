...
final class ContentManager {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		if ( ! in_array( $action, [ ContentRegistry::ACTION_LIST, ContentRegistry::ACTION_CREATE ], true ) ) {
			if ( $content_id <= 0 && $action !== 'content_rollback' ) {
				return new \WP_Error( 'wpcc_missing_content_id', __( 'content_id is required.', 'siteradian' ) );
			}
			if ( $action !== ContentRegistry::ACTION_CREATE && ! get_post( $content_id ) && $action !== 'content_rollback' ) {
				return new \WP_Error( 'wpcc_content_not_found', __( 'Content not found.', 'siteradian' ) );
			}
		}

		if ( ! in_array( $type, ContentRegistry::TYPES, true ) ) {
			return new \WP_Error( 'wpcc_invalid_content_type', __( 'Invalid content type. Use post or page.', 'siteradian' ) );
		}
		...
		return match ( $action ) {
			...
			default => new \WP_Error( 'wpcc_invalid_content_action', __( 'Unknown action.', 'siteradian' ) ),
		};
	}
	...
	private function rollback_content( array $params, array $context ): array|\WP_Error {
		...
		if ( ! empty( $record['rollback_applied'] ) ) {
			return new \WP_Error( 'wpcc_rollback_already_applied', __( 'Rollback has already been applied.', 'siteradian' ) );
		}
	...
	private function create_content( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $title ) {
			return new \WP_Error( 'wpcc_missing_content_title', __( 'Title is required.', 'siteradian' ) );
		}
		if ( ! in_array( $type, ContentRegistry::TYPES, true ) ) {
			return new \WP_Error( 'wpcc_invalid_content_type', __( 'Invalid content type.', 'siteradian' ) );
		}
	...
	private function delete_content( int $id, array $context ): array|\WP_Error {
		...
		if ( ! $result ) {
			...
			return new \WP_Error( 'wpcc_content_delete_failed', __( 'Failed to trash content.', 'siteradian' ) );
		}
	...
	private function schedule_content( int $id, array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $publish_at ) {
			return new \WP_Error( 'wpcc_missing_schedule_time', __( 'publish_at is required.', 'siteradian' ) );
		}

		$ts = strtotime( $publish_at );
		if ( false === $ts || $ts <= time() ) {
			return new \WP_Error( 'wpcc_invalid_schedule_time', __( 'publish_at must be a future date/time.', 'siteradian' ) );
		}
	...
	private function taxonomy_assign( int $id, array $params, array $context ): array|\WP_Error {
		...
		if ( ! is_array( $terms ) || empty( $terms ) ) {
			return new \WP_Error( 'wpcc_missing_taxonomy_terms', __( 'terms array is required.', 'siteradian' ) );
		}

		if ( ! taxonomy_exists( $taxonomy ) ) {
			return new \WP_Error( 'wpcc_invalid_taxonomy', __( 'Taxonomy does not exist.', 'siteradian' ) );
		}
	...
	private function featured_image_assign( int $id, array $params, array $context ): array|\WP_Error {
		...
		if ( $attachment_id <= 0 ) {
			return new \WP_Error( 'wpcc_missing_attachment_id', __( 'attachment_id is required.', 'siteradian' ) );
		}

		$attachment = get_post( $attachment_id );
		if ( ! $attachment || 'attachment' !== $attachment->post_type ) {
			return new \WP_Error( 'wpcc_invalid_attachment', __( 'Attachment not found.', 'siteradian' ) );
		}
