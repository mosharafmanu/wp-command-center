...
final class WooCommerceRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return $this->error( 'wpcc_woo_inactive', __( 'WooCommerce is not active.', 'siteradian' ) );
		}
		...
		if ( ! in_array( $action, WooCommerceRegistry::ACTIONS, true ) ) {
			return $this->error(
				...
				sprintf(
					...
					__( 'Invalid WooCommerce action "%1$s". Valid actions: %2$s. Call action="woo_describe" for details.', 'siteradian' ),
					$action,
		...
		return match ( $action ) {
			...
			default => $this->error( 'wpcc_unknown_woo_action', __( 'Unknown WooCommerce action.', 'siteradian' ) ),
		};
	}
	...
	private function product_get( array $payload ): array {
		...
		if ( ! $p ) return $this->error( 'wpcc_product_not_found', __( 'Product not found.', 'siteradian' ) );
	...
	private function product_search( array $payload ): array {
		...
		if ( '' === $s ) return $this->error( 'wpcc_empty_search', __( "product_search requires a 'search' (or 'query') parameter.", 'siteradian' ) );
	...
	private function product_create( array $payload, array $context ): array {
		...
		if ( '' === $name ) return $this->error( 'wpcc_missing_name', __( 'Product name is required.', 'siteradian' ) );
	...
	private function product_duplicate( array $payload, array $context ): array {
		...
		if ( ! $duplicate || ! $duplicate->get_id() ) return $this->error( 'wpcc_duplicate_failed', __( 'Failed to duplicate product.', 'siteradian' ) );
	...
	private function stock_update( array $payload, array $context ): array {
		...
		if ( null === $qty_raw || '' === $qty_raw || ! is_numeric( $qty_raw ) ) {
			return $this->error(
				...
				__( 'A stock quantity is required — pass quantity (or stock_quantity) as a number.', 'siteradian' )
			);
		}
	...
	private function price_update( array $payload, array $context ): array {
		...
		if ( null === $price_raw || '' === trim( (string) $price_raw ) || ! is_numeric( $price_raw ) ) {
			return $this->error(
				...
				__( 'A regular price is required — pass regular_price as a number.', 'siteradian' )
			);
		}
	...
	private function sale_price_update( array $payload, array $context ): array {
		...
		if ( ! array_key_exists( 'sale_price', $payload ) ) {
			return $this->error(
				...
				__( 'A sale price is required — pass sale_price as a number, or an empty value to end the sale.', 'siteradian' )
			);
		}
		...
		if ( '' !== trim( $sale_raw ) && ! is_numeric( $sale_raw ) ) {
			return $this->error(
				...
				__( 'The sale price must be a number, or empty to end the sale.', 'siteradian' )
			);
		}
	...
	private function attribute_assign( array $payload, array $context ): array {
		...
		if ( '' === $name ) return $this->error( 'wpcc_missing_attribute', __( 'Attribute name is required.', 'siteradian' ) );
	...
	private function variation_list( array $payload ): array {
		$p = wc_get_product( (int) ( $payload['product_id'] ?? 0 ) );
		if ( ! $p || ! $p->is_type( 'variable' ) ) return $this->error( 'wpcc_not_variable', __( 'Product is not variable.', 'siteradian' ) );
	...
	private function variation_get( array $payload ): array {
		...
		if ( ! $v || ! $v->is_type( 'variation' ) ) return $this->error( 'wpcc_variation_not_found', __( 'Variation not found.', 'siteradian' ) );
	...
	private function variation_create( array $payload, array $context ): array {
		$p = wc_get_product( (int) ( $payload['product_id'] ?? 0 ) );
		if ( ! $p || ! $p->is_type( 'variable' ) ) return $this->error( 'wpcc_not_variable', __( 'Parent must be a variable product.', 'siteradian' ) );
	...
	private function order_get( array $payload ): array {
		...
		if ( ! $o ) return $this->error( 'wpcc_order_not_found', __( 'Order not found.', 'siteradian' ) );
	...
	private function order_search( array $payload ): array {
		...
		if ( '' === $s ) return $this->error( 'wpcc_empty_search', __( 'Search term is required.', 'siteradian' ) );
	...
	private function order_note_add( array $payload, array $context ): array {
		...
		if ( '' === $note ) return $this->error( 'wpcc_empty_note', __( 'A note is required.', 'siteradian' ) );
	...
	private function order_status_change( array $payload, array $context ): array {
		...
		if ( ! array_key_exists( 'wc-' . $status, wc_get_order_statuses() ) ) {
			return $this->error( 'wpcc_invalid_order_status', sprintf( /* translators: %s: value */ __( 'Invalid order status: %s', 'siteradian' ), esc_html( $status ) ) );
		}
	...
	private function refund_create( array $payload, array $context ): array {
		...
		if ( (float) $amount <= 0 ) return $this->error( 'wpcc_invalid_refund_amount', __( 'Refund amount must be greater than zero.', 'siteradian' ) );
	...
	private function customer_get( array $payload ): array {
		...
		if ( $id <= 0 ) return $this->error( 'wpcc_customer_not_found', __( 'Customer not found.', 'siteradian' ) );
		try { $c = new \WC_Customer( $id ); } catch ( \Exception $e ) { return $this->error( 'wpcc_customer_not_found', __( 'Customer not found.', 'siteradian' ) ); }
		if ( ! $c->get_id() ) return $this->error( 'wpcc_customer_not_found', __( 'Customer not found.', 'siteradian' ) );
	...
	private function coupon_get( array $payload ): array {
		...
		if ( ! $c->get_id() ) return $this->error( 'wpcc_coupon_not_found', __( 'Coupon not found.', 'siteradian' ) );
	...
	private function coupon_create( array $payload, array $context ): array {
		...
		if ( '' === $code ) return $this->error( 'wpcc_missing_code', __( 'Coupon code is required.', 'siteradian' ) );
		$c = new \WC_Coupon();
		$c->set_code( $code );
	...
	public function rollback( array $payload, array $context = [] ): array {
		...
		if ( '' === $rollback_id ) return $this->error( 'wpcc_missing_rollback_id', __( 'Rollback ID required.', 'siteradian' ) );
		$rollbacks = get_option( 'wpcc_woo_rollbacks', [] );
		$rec = null; $idx = null;
		foreach ( $rollbacks as $i => $r ) { if ( $r['id'] === $rollback_id ) { $rec = $r; $idx = $i; break; } }
		if ( ! $rec ) return $this->error( 'wpcc_rollback_not_found', __( 'Rollback record not found.', 'siteradian' ) );
		if ( $rec['rollback_applied'] ) return $this->error( 'wpcc_rollback_already_applied', __( 'Rollback already applied.', 'siteradian' ) );
	...
	private function rollback_product_delta( array $rec, int $idx, array $rollbacks, array $context ): array {
		...
		if ( ! wc_get_product( $id ) ) {
			return $this->error( 'wpcc_product_not_found', __( 'Product not found.', 'siteradian' ) );
		}
