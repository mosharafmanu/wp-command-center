...
final class RollbackOperation {
			...
			if ( null !== $kind && 'patch' !== $kind ) {
				return new \WP_Error(
					...
					sprintf(
						...
						__( 'This is a %1$s change, not a file patch, so rollback_manage cannot undo it — and this was refused before asking anyone to approve it. Undo it with change_history {action: "rollback_target", rollback_id: "%2$s"}, which routes each change to the engine that made it.', 'siteradian' ),
						$kind,
						$patch_id
					)
				);
	...
	private static function missing_patch_id( array $params ): \WP_Error {
		...
		if ( '' !== $offered ) {
			return new \WP_Error(
				'wpcc_not_a_patch_rollback',
				__( 'rollback_manage undoes patches only, and takes patch_id. A rollback_id returned by another operation (ACF, SEO, WooCommerce, settings, media) is undone with change_history {action: "rollback_target", change_id: "..."} — use change_history {action: "rollback_discover"} to find the change_id.', 'siteradian' )
			);
		}

		return new \WP_Error( 'wpcc_missing_patch_id', __( 'patch_id is required. rollback_manage undoes patches only; to undo any other change use change_history {action: "rollback_target"}.', 'siteradian' ) );
	}
