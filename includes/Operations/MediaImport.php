...
final class MediaImport {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		if ( empty( $url ) || ! wp_http_validate_url( $url ) ) {
			return new \WP_Error( 'wpcc_invalid_url', __( 'Invalid or unsafe source URL.', 'siteradian' ) );
		}

		$parsed = wp_parse_url( $url );
		if ( ! in_array( $parsed['scheme'] ?? '', [ 'http', 'https' ], true ) ) {
			return new \WP_Error( 'wpcc_invalid_url_scheme', __( 'Only HTTP and HTTPS URLs are supported.', 'siteradian' ) );
		}
		...
		if ( ! in_array( $ext, self::ALLOWED_EXTS, true ) ) {
			return new \WP_Error( 'wpcc_unsupported_file_extension', sprintf( /* translators: %s: value */ __( 'Unsupported file extension: %s.', 'siteradian' ), $ext ) );
		}

		if ( ! current_user_can( 'upload_files' ) ) {
			return new \WP_Error( 'wpcc_insufficient_permissions', __( 'You do not have permission to upload files.', 'siteradian' ) );
		}
		...
		if ( $filesize > self::MAX_FILE_SIZE ) {
			...
			return new \WP_Error( 'wpcc_file_too_large', __( 'File exceeds the maximum allowed size of 10MB.', 'siteradian' ) );
		}
		...
		if ( function_exists( 'mime_content_type' ) ) {
			...
			if ( ! $real_mime || ( ! str_starts_with( $real_mime, 'image/' ) && 'application/pdf' !== $real_mime ) ) {
				wp_delete_file( $tmp_file );
				return new \WP_Error( 'wpcc_invalid_mime_type', __( 'Invalid or unsafe file content detected.', 'siteradian' ) );
			}
		}
