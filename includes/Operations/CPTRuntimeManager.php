...
final class CPTRuntimeManager {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		return match ( $action ) {
			...
			default                             => new \WP_Error( 'wpcc_invalid_cpt_action', __( 'Unknown CPT action.', 'siteradian' ) ),
		};
	}
	...
	private function cpt_get( array $params ): array|\WP_Error {
		...
		if ( '' === $name ) {
			return new \WP_Error( 'wpcc_missing_cpt_name', __( 'CPT name is required.', 'siteradian' ) );
		}

		$pt = $this->registry->get_post_type( $name );
		if ( null === $pt ) {
			return new \WP_Error( 'wpcc_cpt_not_found', __( 'Post type not found.', 'siteradian' ) );
		}
	...
	private function cpt_create( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $label ) {
			return new \WP_Error( 'wpcc_missing_cpt_label', __( 'CPT label is required.', 'siteradian' ) );
		}

		if ( ! preg_match( '/^[a-z][a-z0-9_]*$/', $name ) ) {
			return new \WP_Error( 'wpcc_invalid_cpt_name', __( 'CPT name must be lowercase alphanumeric with underscores only.', 'siteradian' ) );
		}

		if ( post_type_exists( $name ) ) {
			return new \WP_Error( 'wpcc_cpt_exists', __( 'Post type already exists.', 'siteradian' ) );
		}
	...
	private function taxonomy_create( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $name ) {
			return new \WP_Error( 'wpcc_missing_taxonomy_name', __( 'Taxonomy name is required.', 'siteradian' ) );
		}
		if ( '' === $label ) {
			return new \WP_Error( 'wpcc_missing_taxonomy_label', __( 'Taxonomy label is required.', 'siteradian' ) );
		}

		if ( ! preg_match( '/^[a-z][a-z0-9_]*$/', $name ) ) {
			return new \WP_Error( 'wpcc_invalid_taxonomy_name', __( 'Taxonomy name must be lowercase alphanumeric with underscores only.', 'siteradian' ) );
		}

		if ( taxonomy_exists( $name ) ) {
			return new \WP_Error( 'wpcc_taxonomy_exists', __( 'Taxonomy already exists.', 'siteradian' ) );
		}
	...
	private function cpt_rollback( array $params, array $context ): array|\WP_Error {
		...
		switch ( $action ) {
			...
			default:
				return new \WP_Error( 'wpcc_rollback_unsupported', __( 'Rollback not supported for this action.', 'siteradian' ) );
		}
