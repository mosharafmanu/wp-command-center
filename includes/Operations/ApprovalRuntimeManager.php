...
final class ApprovalRuntimeManager {
			...
			if ( $wp_user_id <= 0 ) {
				return new \WP_Error(
					...
					sprintf(
						...
						__( 'Approvals must be granted by a WordPress administrator in %s. Use the WordPress admin approval interface to approve or reject this request.', 'siteradian' ),
	...
	private function request_create( array $p, array $actor ): array|\WP_Error {
		...
		if ( '' === $operation_id ) {
			return new \WP_Error( 'wpcc_missing_operation_id', __( 'operation_id is required.', 'siteradian' ) );
		}
	...
	private function require_request( string $id ): array|\WP_Error {
		if ( '' === $id ) {
			return new \WP_Error( 'wpcc_missing_request_id', __( 'request_id is required.', 'siteradian' ) );
		}
	...
	private function require_queue_item( string $id ): array|\WP_Error {
		if ( '' === $id ) {
			return new \WP_Error( 'wpcc_missing_queue_id', __( 'queue_id is required.', 'siteradian' ) );
		}
	...
	private function results_get( array $p ): array|\WP_Error {
		...
		if ( '' === $id ) {
			return new \WP_Error( 'wpcc_missing_result_id', __( 'result_id is required.', 'siteradian' ) );
		}
		$row = ( new OperationResults() )->get_result( $id );
		if ( ! $row ) {
			return new \WP_Error( 'wpcc_result_not_found', __( 'Operation result not found.', 'siteradian' ) );
		}
