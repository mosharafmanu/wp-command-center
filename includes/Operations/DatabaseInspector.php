...
final class DatabaseInspector {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		if ( false !== $input_str && $this->registry->contains_write_keywords( $input_str ) ) {
			...
			return new \WP_Error( 'wpcc_db_write_blocked', __( 'Write keywords are not allowed in database inspection.', 'siteradian' ) );
		}
		...
		$result = match ( $action ) {
			...
			default => new \WP_Error( 'wpcc_invalid_db_action', __( 'Unknown action.', 'siteradian' ) ),
		};
	...
	private function table_stats( ?string $table ): array|\WP_Error {
		if ( ! $table ) {
			return new \WP_Error( 'wpcc_missing_db_table', __( 'table is required.', 'siteradian' ) );
		}
		global $wpdb;
		...
		if ( ! $row ) {
			return new \WP_Error( 'wpcc_db_table_not_found', __( 'Table not found.', 'siteradian' ) );
		}
		return [
