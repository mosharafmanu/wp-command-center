...
final class Cf7Seed {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		if ( ! class_exists( 'WPCF7_ContactForm' ) ) {
			return new \WP_Error( 'wpcc_cf7_inactive', __( 'Contact Form 7 is not active.', 'siteradian' ) );
		}
		...
		if ( ! isset( self::TEMPLATES[ $template ] ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %s: value */ __( 'Invalid CF7 template "%s". Supported: contact_basic, newsletter, quote_request.', 'siteradian' ), $template )
			);
		}
		...
		if ( ! $contact_form ) {
			return new \WP_Error( 'wpcc_cf7_init_failed', __( 'Failed to initialize CF7 form instance.', 'siteradian' ) );
		}
		...
		if ( ! $id ) {
			return new \WP_Error( 'wpcc_cf7_save_failed', __( 'Failed to save Contact Form 7.', 'siteradian' ) );
		}
