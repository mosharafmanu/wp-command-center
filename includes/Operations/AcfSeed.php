...
final class AcfSeed {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		if ( ! function_exists( 'acf_get_field' ) || ! function_exists( 'update_field' ) ) {
			return new \WP_Error( 'wpcc_acf_inactive', __( 'Advanced Custom Fields is not active.', 'siteradian' ) );
		}
		...
		if ( 0 === $post_id || ! get_post( $post_id ) ) {
			return new \WP_Error( 'wpcc_invalid_post_id', __( 'Invalid post ID.', 'siteradian' ) );
		}

		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new \WP_Error( 'wpcc_insufficient_permissions', __( 'You do not have permission to edit this post.', 'siteradian' ) );
		}

		if ( empty( $fields ) ) {
			return new \WP_Error( 'wpcc_no_fields_supplied', __( 'No fields supplied for seeding.', 'siteradian' ) );
		}
		...
		foreach ( $fields as $field_key => $value ) {
			...
			if ( ! $field_object ) {
				return new \WP_Error( 'wpcc_unknown_acf_field', sprintf( /* translators: %s: value */ __( 'ACF field "%s" not found.', 'siteradian' ), $field_key ) );
			}

			if ( ! in_array( $field_object['type'], self::ALLOWED_FIELD_TYPES, true ) ) {
				return new \WP_Error(
					'wpcc_unsupported_acf_field_type',
					sprintf( /* translators: %s: value */ __( 'ACF field type "%s" is not supported for seeding.', 'siteradian' ), $field_object['type'] )
				);
			}
			...
			if ( false === $updated ) {
				...
				if ( $current_value !== $sanitized_value ) {
					return new \WP_Error( 'wpcc_acf_update_failed', sprintf( /* translators: %s: value */ __( 'Failed to update ACF field "%s".', 'siteradian' ), $field_key ) );
				}
			}
