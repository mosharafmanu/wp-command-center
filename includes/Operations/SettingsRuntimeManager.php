...
final class SettingsRuntimeManager {
	...
	/**
	 * Apply one update action's fields, driven by option_field_map().
	 *
	 * Each *_update method used to carry its OWN inline copy of the option→payload-key
	 * map, and two of those copies were wrong in a way that destroyed data.
	 *
	 * The pattern was `['admin_email'=>null, 'date_format'=>null, …]` with
	 * `if (isset($p[$key ?? $opt])) update_option($opt, (string) $p[$key])`. The isset()
	 * resolved the null key to the option name and so passed, but the write then read
	 * `$p[null]` — which PHP turns into `$p['']`, an undefined index. So asking to set
	 * `date_format` wrote an EMPTY STRING over it and returned `updated: true`.
	 * Reproduced on a live site: date_format "F j, Y" → "". `time_format` behaved the
	 * same way, `posts_per_page` was written as 0, and `admin_email` could never be set
	 * at all because `is_email(null)` always failed it.
	 *
	 * option_field_map() is already documented as "the single source of truth that
	 * mirrors each *_update method's write set", and touched_options() and the rollback
	 * capture have always used it — which is why the rollback recorded the right options
	 * while the write corrupted them. The updates use it now too, so there is one map.
	 *
	 * Returns the payload keys actually applied, so the caller can refuse to report a
	 * change it did not make.
	 *
	 * @return array<int,string>|array{error:true,code:string,message:string}
	 */
	private function apply_field_map(string $action,array $p):array{
		$applied=[];
		foreach($this->option_field_map($action) as $opt=>$pkey){
			if(!isset($p[$pkey])){continue;}
			$value=$p[$pkey];
			if('admin_email'===$opt&&!is_email((string)$value)){
				return $this->err('wpcc_invalid_email',__('Invalid email.','siteradian'));
			}
	...
	/** Options whose stored value is an integer, so a string cast would corrupt them. */
	private const INT_OPTIONS=['page_on_front','page_for_posts','posts_per_page','posts_per_rss','blog_public','wp_page_for_privacy_policy','thumbnail_size_w','thumbnail_size_h','thumbnail_crop','medium_size_w','medium_size_h','large_size_w','large_size_h','start_of_week','comment_moderation','require_name_email','comment_registration','thread_comments'];

	/**
	 * A write action that recognised none of its payload must say so.
	 *
	 * It previously returned `['updated'=>true]` whatever happened, so a request that
	 * named no field at all was queued, consumed a human approval, executed, reported
	 * success and recorded a rollback point — for a change that never occurred. That is
	 * exactly what a schema-following assistant produces today, because these actions
	 * advertise no value parameters: observed end to end with Claude Code against a live
	 * site. Failing here costs the caller one error message instead of one approval and
	 * a false entry in the change log, and the message names the fields it accepts.
	 *
	 * @param array<int,string>|array{error:true,code:string,message:string} $applied
	 */
	private function updated_result(string $action,array $applied):array{
		if(isset($applied['error'])){return $applied;}
		if([]===$applied){
			...
			return $this->err('wpcc_no_settings_supplied',sprintf(
				...
				__('No settings were supplied for "%1$s", so nothing was changed. Provide at least one of: %2$s.','siteradian'),
				$action,
				implode(', ',$accepted)
			));
		}
		return['updated'=>true,'fields_updated'=>$applied];
	}

	private function general_update(array $p):array{return $this->updated_result(SettingsRegistry::A_GENERAL_UPDATE,$this->apply_field_map(SettingsRegistry::A_GENERAL_UPDATE,$p));}
	...
	private function reading_update(array $p):array{return $this->updated_result(SettingsRegistry::A_READING_UPDATE,$this->apply_field_map(SettingsRegistry::A_READING_UPDATE,$p));}
	...
	private function discussion_update(array $p):array{return $this->updated_result(SettingsRegistry::A_DISCUSSION_UPDATE,$this->apply_field_map(SettingsRegistry::A_DISCUSSION_UPDATE,$p));}
	...
	private function media_update(array $p):array{return $this->updated_result(SettingsRegistry::A_MEDIA_UPDATE,$this->apply_field_map(SettingsRegistry::A_MEDIA_UPDATE,$p));}
	...
	private function permalink_update(array $p):array{if(!isset($p['structure'])){return $this->updated_result(SettingsRegistry::A_PERMALINK_UPDATE,[]);}$s=sanitize_text_field((string)$p['structure']);global $wp_rewrite;$wp_rewrite->set_permalink_structure($s);flush_rewrite_rules();return['updated'=>true,'fields_updated'=>['structure']];}
	private function privacy_get(array $p):array{return['settings'=>['privacy_page'=>(int)get_option('wp_page_for_privacy_policy')]];}
	private function privacy_update(array $p):array{return $this->updated_result(SettingsRegistry::A_PRIVACY_UPDATE,$this->apply_field_map(SettingsRegistry::A_PRIVACY_UPDATE,$p));}
	...
	public function rollback(array $p,array $cx=[]):array{
		...
		if(''===$rid)return $this->err('wpcc_missing_rb',__('Rollback ID required.','siteradian'));
		...
		if(!$resolved)return $this->err('wpcc_rb_nf',__('Not found.','siteradian'));
		$rec=$resolved['record'];
		if(!empty($rec['rollback_applied']))return $this->err('wpcc_rb_done',__('Already applied.','siteradian'));
	...
	/** Options a given update action writes, keyed option_name => payload_key.
	 *  Delegates to SettingsRegistry::field_map(), which is the one place this is
	 *  stated — the operation catalogue derives its declared parameters from the same
	 *  map, so what the schema advertises and what the write accepts cannot drift. */
	private function option_field_map(string $action):array{
		return SettingsRegistry::field_map()[$action] ?? [];
	}
