...
final class CapabilityRegistry {
	...
	public function assign( string $subject, string $subject_id, string $capability ): ?\WP_Error {
		if ( ! in_array( $capability, self::ALL_CAPABILITIES, true ) ) {
			return new \WP_Error( 'wpcc_invalid_capability', __( 'Unknown capability.', 'siteradian' ) );
		}
		if ( self::CAP_SYSTEM_ADMIN === $capability ) {
			return new \WP_Error( 'wpcc_cannot_assign_admin', __( 'system.admin can only be assigned via direct configuration.', 'siteradian' ) );
		}
	...
	public function remove( string $subject, string $subject_id, string $capability ): ?\WP_Error {
		...
		$key = $subject . ':' . $subject_id;
		if ( ! isset( $all[ $key ] ) ) {
			return new \WP_Error( 'wpcc_capability_not_assigned', __( 'No capabilities assigned to this subject.', 'siteradian' ) );
		}
