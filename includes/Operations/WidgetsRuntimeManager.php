...
final class WidgetsRuntimeManager {
	...
	public function run( array $params, array $context = [] ): array|\WP_Error {
		...
		return match ( $action ) {
			...
			default                                 => new \WP_Error( 'wpcc_invalid_widgets_action', __( 'Unknown widgets action.', 'siteradian' ) ),
		};
	}
	...
	private function widget_get( array $params ): array|\WP_Error {
		...
		if ( '' === $widget_id ) {
			return new \WP_Error( 'wpcc_missing_widget_id', __( 'widget_id is required.', 'siteradian' ) );
		}

		$widget = $this->registry->get_widget( $widget_id );
		if ( null === $widget ) {
			return new \WP_Error( 'wpcc_widget_not_found', __( 'Widget not found.', 'siteradian' ) );
		}
	...
	private function widget_add( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $widget_type ) {
			return new \WP_Error( 'wpcc_missing_widget_type', __( 'widget_type is required.', 'siteradian' ) );
		}
		if ( '' === $sidebar_id ) {
			return new \WP_Error( 'wpcc_missing_sidebar_id', __( 'sidebar_id is required.', 'siteradian' ) );
		}
	...
	private function widget_update( array $params, array $context ): array|\WP_Error {
		...
		if ( ! $found ) {
			return new \WP_Error( 'wpcc_widget_not_found', __( 'Widget not found in any sidebar.', 'siteradian' ) );
		}
	...
	private function sidebar_remove( array $params, array $context ): array|\WP_Error {
		...
		if ( '' === $widget_id || '' === $sidebar_id ) {
			return new \WP_Error( 'wpcc_missing_params', __( 'widget_id and sidebar_id are required.', 'siteradian' ) );
		}

		$sidebars_widgets = WidgetsRegistry::sidebars_widgets();
		if ( ! isset( $sidebars_widgets[ $sidebar_id ] ) ) {
			return new \WP_Error( 'wpcc_sidebar_not_found', __( 'Sidebar not found.', 'siteradian' ) );
		}

		if ( ! in_array( $widget_id, $sidebars_widgets[ $sidebar_id ], true ) ) {
			return new \WP_Error( 'wpcc_widget_not_in_sidebar', __( 'Widget not found in this sidebar.', 'siteradian' ) );
		}
