...
final class SiteBuilderRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		return match ( $action ) {
			...
			default => $this->error( 'wpcc_invalid_site_builder_action', __( 'Invalid site builder action.', 'siteradian' ) ),
		};
	}
	...
	private function page_get( array $p ): array {
		...
		if ( is_string( $page ) ) return $this->error( $page, __( 'Page not found.', 'siteradian' ) );
	...
	private function page_create( array $p, array $cx ): array {
		...
		if ( '' === $title ) return $this->error( 'wpcc_missing_title', __( 'Page title is required.', 'siteradian' ) );
	...
	private function template_list(): array {
		...
		return [ 'action' => 'template_list', 'templates' => array_merge( [ 'default' => __( 'Default Template', 'siteradian' ) ], $templates ) ];
	}
	...
	private function template_assign( array $p, array $cx ): array {
		...
		if ( '' === $template ) return $this->error( 'wpcc_missing_template', __( 'A template is required.', 'siteradian' ) );

		$available = array_merge( [ 'default' ], array_keys( wp_get_theme()->get_page_templates( null, 'page' ) ) );
		if ( ! in_array( $template, $available, true ) ) {
			return $this->error( 'wpcc_invalid_template', sprintf( /* translators: %s: value */ __( 'Template not available in the active theme: %s', 'siteradian' ), esc_html( $template ) ) );
		}
	...
	private function pattern_create( array $p, array $cx ): array {
		$title = sanitize_text_field( (string) ( $p['title'] ?? '' ) );
		if ( '' === $title ) return $this->error( 'wpcc_missing_title', __( 'Pattern title is required.', 'siteradian' ) );
	...
	private function navigation_manage( array $p, array $cx ): array {
		...
		if ( 'get' === $op ) {
			...
			if ( ! $nav || 'wp_navigation' !== $nav->post_type ) return $this->error( 'wpcc_navigation_not_found', __( 'Navigation not found.', 'siteradian' ) );
	...
	public function rollback( array $payload, array $context = [] ): array {
		...
		if ( null === $idx ) return $this->error( 'wpcc_rollback_not_found', __( 'Rollback not found.', 'siteradian' ) );
		if ( ! empty( $rollbacks[ $idx ]['rollback_applied'] ) ) return $this->error( 'wpcc_rollback_already_applied', __( 'Already applied.', 'siteradian' ) );
