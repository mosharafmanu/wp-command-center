	...
	public function get_commands(): array {
		return [
			...
			'plugin_list' => [
				...
				'title'                 => __( 'List Plugins', 'action-steward' ),
				'description'           => __( 'List all installed plugins with status and version.', 'action-steward' ),
			...
			'theme_list' => [
				...
				'title'                 => __( 'List Themes', 'action-steward' ),
				'description'           => __( 'List all installed themes with status and version.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'option_get_siteurl' => [
				...
				'title'                 => __( 'Get Site URL', 'action-steward' ),
				'description'           => __( 'Retrieve the WordPress siteurl option.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'option_get_home' => [
				...
				'title'                 => __( 'Get Home URL', 'action-steward' ),
				'description'           => __( 'Retrieve the WordPress home option.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'cron_event_list' => [
				...
				'title'                 => __( 'List Cron Events', 'action-steward' ),
				'description'           => __( 'List all scheduled WP-Cron events.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'transient_delete_expired' => [
				...
				'title'                 => __( 'Delete Expired Transients', 'action-steward' ),
				'description'           => __( 'Delete all expired transients from the database.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'rewrite_list' => [
				...
				'title'                 => __( 'List Rewrite Rules', 'action-steward' ),
				'description'           => __( 'List current WordPress rewrite rules.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
			...
			'cache_flush' => [
				...
				'title'                 => __( 'Flush Cache', 'action-steward' ),
				'description'           => __( 'Flush the WordPress object cache.', 'action-steward' ),
			...
			'rewrite_flush' => [
				...
				'title'                 => __( 'Flush Rewrite Rules', 'action-steward' ),
				'description'           => __( 'Flush WordPress rewrite rules (regenerates .htaccess).', 'action-steward' ),
				'risk_level'            => self::RISK_MEDIUM,
			...
			'cron_event_run_due_now' => [
				...
				'title'                 => __( 'Run Due Cron Events', 'action-steward' ),
				'description'           => __( 'Run all due WP-Cron events now.', 'action-steward' ),
				'risk_level'            => self::RISK_MEDIUM,
			...
			'option_update_blogdescription' => [
				...
				'title'                 => __( 'Update Blog Description', 'action-steward' ),
				'description'           => __( 'Update the WordPress blogdescription (tagline) option.', 'action-steward' ),
				'risk_level'            => self::RISK_MEDIUM,
			...
			'option_update_blogname' => [
				...
				'title'                 => __( 'Update Blog Name', 'action-steward' ),
				'description'           => __( 'Update the WordPress blogname (site title) option.', 'action-steward' ),
				'risk_level'            => self::RISK_MEDIUM,
			...
			'plugin_update_single' => [
				...
				'title'                 => __( 'Update Single Plugin', 'action-steward' ),
				'description'           => __( 'Update a single plugin to its latest version.', 'action-steward' ),
			...
			'theme_update_single' => [
				...
				'title'                 => __( 'Update Single Theme', 'action-steward' ),
				'description'           => __( 'Update a single theme to its latest version.', 'action-steward' ),
				'risk_level'            => self::RISK_HIGH,
			...
			'search_replace_dry_run' => [
				...
				'title'                 => __( 'Search & Replace (Dry Run)', 'action-steward' ),
				'description'           => __( 'Preview a database search and replace without making changes.', 'action-steward' ),
				'risk_level'            => self::RISK_HIGH,
			...
			'search_replace_execute' => [
				...
				'title'                 => __( 'Search & Replace (Execute)', 'action-steward' ),
				'description'           => __( 'Execute a live database search and replace.', 'action-steward' ),
				'risk_level'            => self::RISK_HIGH,
				'command_parts'         => [ 'wp', 'search-replace' ],
				'allowed_args_schema'   => [
					'search'         => [ 'type' => 'string', 'required' => true, 'max_length' => 1000 ],
			...
			'db_export' => [
				...
				'title'                 => __( 'Database Export', 'action-steward' ),
				'description'           => __( 'Export the WordPress database via WP-CLI.', 'action-steward' ),
			...
			'db_optimize' => [
				...
				'title'                 => __( 'Optimize Database', 'action-steward' ),
				'description'           => __( 'Optimize WordPress database tables.', 'action-steward' ),
				'risk_level'            => self::RISK_CRITICAL,
			...
			'db_repair' => [
				...
				'title'                 => __( 'Repair Database', 'action-steward' ),
				'description'           => __( 'Repair WordPress database tables.', 'action-steward' ),
				'risk_level'            => self::RISK_CRITICAL,
			...
			'db_size_check' => [
				...
				'title'                 => __( 'Database Size', 'action-steward' ),
				'description'           => __( 'Check the WordPress database size.', 'action-steward' ),
				'risk_level'            => self::RISK_LOW,
		...
		foreach ( $args as $key => $value ) {
			if ( ! in_array( $key, $allowed_keys, true ) ) {
				return new \WP_Error(
					...
					sprintf( /* translators: %s: value */ __( 'Unknown argument: %s', 'action-steward' ), esc_html( $key ) )
				);
			}
			...
			if ( ! empty( $def['required'] ) && ( null === $value || '' === $value ) ) {
				return new \WP_Error(
					...
					sprintf( /* translators: %s: value */ __( 'Missing required argument: %s', 'action-steward' ), esc_html( $key ) )
				);
			}
			...
			if ( is_string( $value ) && $this->contains_shell_metacharacters( $value ) ) {
				return new \WP_Error(
					...
					sprintf( /* translators: %s: value */ __( 'Arg %s contains unsafe shell characters.', 'action-steward' ), esc_html( $key ) )
				);
			}
			...
			if ( isset( $def['enum'] ) && ! in_array( $value, $def['enum'], true ) ) {
				return new \WP_Error(
					...
					sprintf( /* translators: 1: parameter name, 2: comma-separated list of allowed values */ __( 'Invalid value for %1$s. Allowed: %2$s', 'action-steward' ), esc_html( $key ), implode( ', ', $def['enum'] ) )
				);
			}
			...
			if ( isset( $def['pattern'] ) && ! preg_match( $def['pattern'], (string) $value ) ) {
				return new \WP_Error(
					...
					sprintf( /* translators: %s: value */ __( 'Invalid format for %s', 'action-steward' ), esc_html( $key ) )
				);
			}
			...
			if ( isset( $def['max_length'] ) && is_string( $value ) && mb_strlen( $value ) > $def['max_length'] ) {
				return new \WP_Error(
					...
					sprintf( /* translators: 1: parameter name, 2: maximum length */ __( 'Value for %1$s exceeds maximum length of %2$d', 'action-steward' ), esc_html( $key ), $def['max_length'] )
				);
			}
		}
