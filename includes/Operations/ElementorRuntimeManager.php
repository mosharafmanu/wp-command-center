...
final class ElementorRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		if ( ! defined( 'ELEMENTOR_VERSION' ) ) {
			return $this->error( 'wpcc_elementor_inactive', __( 'Elementor is not active.', 'siteradian' ) );
		}
		...
		return match ( $action ) {
			...
			default => $this->error( 'wpcc_invalid_elementor_action', __( 'Invalid Elementor action.', 'siteradian' ) ),
		};
	}
	...
	private function get_page( array $p ): array {
		...
		if ( is_string( $data ) ) return $this->error( $data, __( 'Not an Elementor page.', 'siteradian' ) );
	...
	private function update_image( array $p, array $cx ): array {
		...
		if ( '' === $url && empty( $p['image_id'] ) ) {
			return $this->error( 'wpcc_missing_image', __( 'image_url or image_id is required.', 'siteradian' ) );
		}
	...
	private function update_button( array $p, array $cx ): array {
		if ( ! isset( $p['text'] ) && ! isset( $p['url'] ) ) {
			return $this->error( 'wpcc_missing_button_fields', __( 'Provide text and/or url for the button.', 'siteradian' ) );
		}
	...
	private function edit_widget( array $p, array $cx, string $action, callable $mutator ): array {
		...
		if ( '' === $widget_id ) return $this->error( 'wpcc_missing_widget_id', __( 'widget_id is required.', 'siteradian' ) );
		...
		if ( ! $found ) return $this->error( 'wpcc_widget_not_found', sprintf( /* translators: %s: value */ __( 'Widget %s not found on this page.', 'siteradian' ), esc_html( $widget_id ) ) );
