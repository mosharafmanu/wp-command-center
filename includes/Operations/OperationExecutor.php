...
final class OperationExecutor {
	...
	public function run( string $operation_id, array $payload, array $context = [] ): array {
		...
		if ( ! $operation ) {
			return $this->fail( $operation_id, 'operation_not_found', __( 'Operation not found in registry.', 'siteradian' ) );
		}
		...
		if ( $is_read_token && $cap_registry->requires_full_scope( $operation_id, $payload ) ) {
			return $this->fail( $operation_id, 'wpcc_token_read_only', __( 'This token is read-only. Mutations and approval requests are not allowed.', 'siteradian' ) );
		}
		...
		if ( get_option( 'wpcc_enforce_capabilities', true ) ) {
			...
			if ( '' !== $token_id ) {
				...
				if ( ! $validation['allowed'] ) {
					...
					return $this->fail( $operation_id, 'wpcc_capability_denied', sprintf( /* translators: %s: value */ __( 'Missing capability: %s', 'siteradian' ), $validation['required_capability'] ) );
				}
			}
		}
		...
		if ( null !== $vocabulary['conflict'] ) {
			return $this->fail(
				...
				sprintf(
					...
					__( 'Both %1$s and %2$s were supplied with different values, and they mean the same thing. Send one. Nothing was changed.', 'siteradian' ),
		...
		if ( ! $is_queued && ! $is_requested ) {
				...
				return $this->fail(
					...
					[] !== $declared_actions
						? sprintf(
							...
							__( 'An action is required for %1$s. Valid actions: %2$s.', 'siteradian' ),
							$operation_id,
							implode( ', ', $declared_actions )
						)
						: sprintf(
							/* translators: %s: operation id */
							__( 'An action is required for %s.', 'siteradian' ),
							$operation_id
						)
				);
			}
			...
			if ( 'bulk_manage' === $operation_id
				...
				return $this->fail(
					...
					__( 'ids is required — provide the IDs to update.', 'siteradian' )
					. ' ' . __( 'Nothing was queued for approval — a request that cannot execute should not need your decision.', 'siteradian' )
				);
			}
		}
				...
				if ( [] !== $missing ) {
					...
					return $this->fail(
						...
						sprintf(
							...
							__( '%1$s cannot run without %2$s. It requires: %3$s. Nothing was queued for approval — a request that cannot execute should not need your decision.', 'siteradian' ),
							$operation_id,
							implode( ', ', $missing ),
			...
			return $this->fail(
				...
				'' !== $needs
					? sprintf(
						...
						__( '%1$s requires %2$s, which is not active on this site.', 'siteradian' ),
						$operation_id,
						$needs
					)
					: __( 'Operation is not available in the current environment.', 'siteradian' )
			);
		}
		...
		if ( ! $handler ) {
			return $this->fail( $operation_id, 'execution_failed', __( 'Execution logic not yet implemented for this operation.', 'siteradian' ) );
		}
		...
		if ( is_array( $result ) && ! empty( $result['error'] ) && isset( $result['code'] ) && is_string( $result['code'] ) ) {
			...
			$error_message = (string) ( $result['message'] ?? __( 'Operation failed.', 'siteradian' ) );
		...
		$res_id = $res_manager->create( array_merge( $res_base, [
			...
			'error_json'    => $apply_failed ? wp_json_encode( [
				...
				'message' => __( 'The change did not apply and was rolled back. The site is unchanged.', 'siteradian' ),
			] ) : null,
	...
	public function rollback( string $operation_id, array $payload, array $context = [] ): array {
		...
		if ( ! $handler || ! method_exists( $handler, 'rollback' ) ) {
			...
			return [
				...
				'message'      => __( 'Operation does not support rollback.', 'siteradian' ),
			];
		}
	...
	private function pending_approval( string $operation_id, array $request, string $risk_level, string $action, ?array $destructive = null, array $extra = [] ): array {
		...
		$result['message'] = sprintf(
			...
			__( 'Approval required (%1$s). A site administrator must approve this request at: %3$s — or poll status with: approval_manage {action: "request_get", request_id: "%2$s"}', 'siteradian' ),
			/*
		...
		if ( ! empty( $extra['change_set'] ) && is_array( $extra['change_set'] ) ) {
			...
			$result['message'] .= ' ' . sprintf(
				...
				__( 'This single approval applies a change set of %1$d file(s) atomically: %2$s | modes: %3$s | +%4$d/-%5$d lines | risk: %6$s%7$s. All files apply together or none do; one combined rollback is available.', 'siteradian' ),
				(int) $cs['file_count'],
				...
				! empty( $cs['has_high_risk_paths'] ) ? __( ' (HIGH-RISK paths included)', 'siteradian' ) : ''
			);
		}
	...
	private function confirmation_required( string $operation_id, array $payload, array $descriptor, array $missing ): array {
		...
		$required_parameters = [
			...
			'reason'                  => __( 'a human-readable reason for the deletion', 'siteradian' ),
			$descriptor['target_key'] => __( 'the identifier of the target to delete', 'siteradian' ),
		];
		...
		return [
			...
			'result'       => [
				...
				'message'               => sprintf(
					...
					__( 'This is a CRITICAL destructive operation. To proceed, resend the request with confirm=true, confirmation_phrase="%1$s", a non-empty reason, and the %2$s of the target. Missing: %3$s.', 'siteradian' ),
					$descriptor['phrase'],
