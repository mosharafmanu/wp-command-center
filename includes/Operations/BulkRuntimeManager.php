...
final class BulkRuntimeManager {
	...
	private function bulk_content(array $p,array $cx):array{
		...
		if(!$ids)return$this->err('wpcc_missing_bulk_ids',__('ids is required — provide the IDs to update.','siteradian'));
		if(!$fields)return$this->err('wpcc_missing_bulk_fields',__('fields is required — provide what to change (post_title and/or post_content).','siteradian'));
	...
	private function bulk_acf(array $p,array $cx):array{
		...
		if(''===$field)return$this->err('missing_field',__('Field key required.','siteradian'));
		if(count($ids)>self::MAX_ITEMS)return$this->err('too_many_items',$this->cap_msg());
	...
	private function batch_execute(array $p,array $cx):array{
		...
		if(count($ops)>self::MAX_ITEMS)return$this->err('too_many_items',sprintf(/* translators: %d: number */ __('Cannot process more than %d operations in a single batch.','siteradian'),self::MAX_ITEMS));
	...
	private function dep_message(string $type):string{
		if('woo'===$type)return __('WooCommerce is not active; cannot reverse this bulk operation.','siteradian');
		if('acf'===$type)return __('ACF is not active; cannot reverse this bulk operation.','siteradian');
		return __('This bulk rollback record type cannot be reversed.','siteradian');
	}
	...
	private function legacy_rollback(string $rid):array{
		...
		if(in_array($action,['bulk_content','bulk_publish','bulk_draft','bulk_media'],true)){
			...
			$fk=(string)($bs['field_key']??'');if(''===$fk)return$this->unsupported(__('Rollback record is missing its ACF field key.','siteradian'));
	...
	private function cap_msg():string{ return sprintf(/* translators: %d: number */ __('Cannot process more than %d items in a single bulk operation.','siteradian'),self::MAX_ITEMS); }
