...
final class SeoRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		if ( SeoProvider::NONE === $provider && SeoRegistry::ACTION_VALIDATE !== $action ) {
			return $this->error( 'wpcc_seo_no_provider', __( 'No supported SEO plugin (Rank Math or Yoast SEO) is active.', 'siteradian' ) );
		}
		...
		return match ( $action ) {
			...
			default                      => $this->error( 'wpcc_invalid_seo_action', __( 'Invalid SEO action.', 'siteradian' ) ),
		};
	}
	...
	private function seo_get( array $payload, string $provider ): array {
		...
		if ( is_string( $post ) ) {
			return $this->error( $post, __( 'Content not found.', 'siteradian' ) );
		}
	...
	private function seo_update( array $payload, string $provider, array $context ): array {
		...
		if ( empty( $fields ) ) {
			return $this->error( 'wpcc_seo_no_fields', __( 'Provide at least one SEO field to update.', 'siteradian' ) );
		}
	...
	private function seo_validate( array $payload, string $provider ): array {
		...
		if ( empty( $fields ) ) {
			$post = $this->resolve_post( $payload );
			if ( is_string( $post ) ) {
				return $this->error( $post, __( 'Provide fields or a valid content_id to validate.', 'siteradian' ) );
			}
	...
	private function seo_analyze( array $payload, string $provider ): array {
		...
		$checks[] = $this->check(
			'title_present',
			'' !== $title,
			__( 'An SEO title is set.', 'siteradian' ),
			__( 'No SEO title is set. Add one so search results do not fall back to the post title.', 'siteradian' )
		);
		$checks[] = $this->check(
			'title_length',
			'' !== $title && $title_len <= self::TITLE_MAX,
			sprintf( /* translators: 1: maximum characters allowed, 2: actual length */ __( 'SEO title is within %1$d characters (is %2$d).', 'siteradian' ), self::TITLE_MAX, $title_len ),
			'' === $title
				? sprintf( /* translators: %d: maximum characters allowed */ __( 'No SEO title to measure; the recommended maximum is %d characters.', 'siteradian' ), self::TITLE_MAX )
				: sprintf( /* translators: 1: actual length, 2: maximum characters allowed */ __( 'SEO title is %1$d characters, longer than the recommended %2$d. Shorten it or search engines may truncate it.', 'siteradian' ), $title_len, self::TITLE_MAX ),
			[ 'measured' => $title_len, 'max' => self::TITLE_MAX, 'unit' => 'characters' ]
		);
		$checks[] = $this->check(
			'description_present',
			'' !== $desc,
			__( 'A meta description is set.', 'siteradian' ),
			__( 'No meta description is set. Add one so search engines do not compose their own.', 'siteradian' )
		);
		$checks[] = $this->check(
			'description_length',
			$desc_len >= self::DESC_MIN && $desc_len <= self::DESC_MAX,
			sprintf( /* translators: 1: minimum characters, 2: maximum characters, 3: actual length */ __( 'Meta description is %1$d–%2$d characters (is %3$d).', 'siteradian' ), self::DESC_MIN, self::DESC_MAX, $desc_len ),
			$desc_len < self::DESC_MIN
				? sprintf( /* translators: 1: actual length, 2: minimum characters */ __( 'Meta description is %1$d characters, shorter than the recommended minimum of %2$d. Add more detail.', 'siteradian' ), $desc_len, self::DESC_MIN )
				: sprintf( /* translators: 1: actual length, 2: maximum characters */ __( 'Meta description is %1$d characters, longer than the recommended %2$d. Shorten it or it may be truncated.', 'siteradian' ), $desc_len, self::DESC_MAX ),
			[ 'measured' => $desc_len, 'min' => self::DESC_MIN, 'max' => self::DESC_MAX, 'unit' => 'characters' ]
		);
		$checks[] = $this->check(
			'focus_keyword_present',
			'' !== $kw,
			__( 'A focus keyword is set.', 'siteradian' ),
			__( 'No focus keyword is set, so the keyword checks below cannot be assessed.', 'siteradian' )
		);
		$checks[] = $this->check(
			'focus_keyword_in_title',
			'' !== $kw && str_contains( $haystk, $kw ),
			__( 'Focus keyword appears in the title.', 'siteradian' ),
			'' === $kw
				? __( 'No focus keyword is set, so this cannot be checked against the title.', 'siteradian' )
				: __( 'Focus keyword does not appear in the SEO title or the post title.', 'siteradian' )
		);
		$checks[] = $this->check(
			'focus_keyword_in_description',
			'' !== $kw && str_contains( strtolower( $desc ), $kw ),
			__( 'Focus keyword appears in the meta description.', 'siteradian' ),
			'' === $kw
				? __( 'No focus keyword is set, so this cannot be checked against the meta description.', 'siteradian' )
				: __( 'Focus keyword does not appear in the meta description.', 'siteradian' )
		);
		$checks[] = $this->check(
			'focus_keyword_in_content',
			'' !== $kw && str_contains( $content, $kw ),
			__( 'Focus keyword appears in the content.', 'siteradian' ),
			'' === $kw
				? __( 'No focus keyword is set, so this cannot be checked against the content.', 'siteradian' )
				: __( 'Focus keyword does not appear in the content.', 'siteradian' )
		);
		$checks[] = $this->check(
			'canonical_set',
			'' !== (string) $seo['canonical'],
			__( 'A canonical URL is set.', 'siteradian' ),
			__( 'No canonical URL is set. This is normal unless this content is duplicated elsewhere.', 'siteradian' )
		);
		$checks[] = $this->check(
			'open_graph_set',
			'' !== (string) $seo['og_title'] || '' !== (string) $seo['og_description'],
			__( 'Open Graph metadata is set.', 'siteradian' ),
			__( 'No Open Graph title or description is set, so social shares fall back to the page content.', 'siteradian' )
		);
	...
	private function restore_delta( int $post_id, string $meta_key, array $record ): array {
		...
		$msg  = 'conflict' === $status
			? sprintf( /* translators: %s: value */ __( 'Rollback skipped: every targeted SEO field (%s) changed since this update was applied. No fields were restored.', 'siteradian' ), implode( ', ', $skipped ) )
			: sprintf( /* translators: %1$s: value, %2$s: value */ __( 'Partial rollback: restored %1$s; skipped %2$s because they changed since this update was applied (drift).', 'siteradian' ), implode( ', ', $restored ), implode( ', ', $skipped ) );
	...
	private function validate_fields( array $fields ): array {
		...
		if ( isset( $fields['title'] ) && mb_strlen( (string) $fields['title'] ) > self::TITLE_MAX ) {
			$issues[] = [ 'field' => 'title', 'severity' => 'warning', 'message' => sprintf( /* translators: %d: number */ __( 'SEO title exceeds %d characters and may be truncated.', 'siteradian' ), self::TITLE_MAX ) ];
		}
		if ( isset( $fields['description'] ) && '' !== (string) $fields['description'] ) {
			$len = mb_strlen( (string) $fields['description'] );
			if ( $len > self::DESC_MAX ) {
				$issues[] = [ 'field' => 'description', 'severity' => 'warning', 'message' => sprintf( /* translators: %d: number */ __( 'Meta description exceeds %d characters and may be truncated.', 'siteradian' ), self::DESC_MAX ) ];
			} elseif ( $len < self::DESC_MIN ) {
				$issues[] = [ 'field' => 'description', 'severity' => 'info', 'message' => sprintf( /* translators: %d: number */ __( 'Meta description is under the recommended %d characters.', 'siteradian' ), self::DESC_MIN ) ];
			}
		}
		if ( isset( $fields['canonical'] ) && '' !== (string) $fields['canonical'] && ! wp_http_validate_url( (string) $fields['canonical'] ) ) {
			$issues[] = [ 'field' => 'canonical', 'severity' => 'error', 'message' => __( 'Canonical URL is not a valid URL.', 'siteradian' ) ];
		}
		if ( isset( $fields['robots'] ) ) {
			foreach ( (array) $fields['robots'] as $d ) {
				if ( ! in_array( strtolower( trim( (string) $d ) ), SeoProvider::ROBOTS_DIRECTIVES, true ) ) {
					$issues[] = [ 'field' => 'robots', 'severity' => 'error', 'message' => sprintf( /* translators: %s: value */ __( 'Unknown robots directive: %s', 'siteradian' ), esc_html( (string) $d ) ) ];
				}
			}
		}
