...
final class TermRuntimeManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		switch ( $action ) {
			...
			case 'term_describe':
				return [ 'action' => 'term_describe', 'runtime' => 'term_manage', 'actions' => self::ACTIONS, 'notes' => __( 'Read-only. term_get accepts term_id OR (slug + taxonomy). term_list/term_search accept taxonomy + filters. Returns term_id, slug, name, taxonomy, parent, count.', 'action-steward' ) ];
			default:
				return $this->error(
					'wpcc_invalid_term_action',
					sprintf(
						/* translators: 1: invalid action, 2: valid actions */
						__( 'Invalid term action "%1$s". Valid actions: %2$s.', 'action-steward' ),
						$action,
	...
	private function term_list( array $p ): array {
		...
		if ( '' !== $taxonomy && ! taxonomy_exists( $taxonomy ) ) {
			return $this->error( 'wpcc_invalid_taxonomy', sprintf( /* translators: %s: value */ __( 'Taxonomy "%s" does not exist.', 'action-steward' ), $taxonomy ), [ 'valid_taxonomies' => get_taxonomies( [], 'names' ) ] );
		}
	...
	private function term_get( array $p ): array {
		...
		if ( $term_id > 0 ) {
			...
			if ( '' === $taxonomy || ( '' === $slug && '' === $name ) ) {
				return $this->error( 'wpcc_missing_term_selector', __( 'Provide term_id, or (taxonomy + slug), or (taxonomy + name).', 'action-steward' ) );
			}
			if ( ! taxonomy_exists( $taxonomy ) ) {
				return $this->error( 'wpcc_invalid_taxonomy', sprintf( /* translators: %s: value */ __( 'Taxonomy "%s" does not exist.', 'action-steward' ), $taxonomy ) );
			}
			$term = '' !== $slug ? get_term_by( 'slug', $slug, $taxonomy ) : get_term_by( 'name', $name, $taxonomy );
		}

		if ( ! $term || is_wp_error( $term ) ) {
			return $this->error( 'wpcc_term_not_found', __( 'Term not found.', 'action-steward' ) );
		}
	...
	private function term_search( array $p ): array {
		...
		if ( '' === $search ) {
			return $this->error( 'wpcc_missing_search', __( "term_search requires a 'search' parameter (matches term name/slug).", 'action-steward' ) );
		}
		$taxonomy = sanitize_key( (string) ( $p['taxonomy'] ?? '' ) );
		if ( '' !== $taxonomy && ! taxonomy_exists( $taxonomy ) ) {
			return $this->error( 'wpcc_invalid_taxonomy', sprintf( /* translators: %s: value */ __( 'Taxonomy "%s" does not exist.', 'action-steward' ), $taxonomy ) );
