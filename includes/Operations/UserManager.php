...
final class UserManager {
	...
	public function run( array $payload, array $context = [] ): array {
		...
		return match ( $action ) {
			...
			default                              => $this->error( 'wpcc_unknown_user_action', __( 'Unknown user action.', 'action-steward' ) ),
		};
	}
	...
	private function get_user( array $payload ): array {
		...
		if ( ! $user ) {
			return $this->error( 'wpcc_user_not_found', __( 'User not found.', 'action-steward' ) );
		}
	...
	private function search_users( array $payload ): array {
		...
		if ( '' === $search ) {
			return $this->error( 'wpcc_missing_search', __( "user_search requires a 'search' (or 'query') parameter — it matches username, email and display name.", 'action-steward' ) );
		}
	...
	private function create_user( array $payload, array $context ): array {
		...
		if ( '' === $username || '' === $email || '' === $password ) {
			return $this->error( 'wpcc_missing_user_fields', __( 'Username, email, and password are required.', 'action-steward' ) );
		}

		if ( username_exists( $username ) ) {
			return $this->error( 'wpcc_username_exists', __( 'Username already exists.', 'action-steward' ) );
		}
		if ( email_exists( $email ) ) {
			return $this->error( 'wpcc_user_email_exists', __( 'Email already in use.', 'action-steward' ) );
		}
	...
	private function update_user( array $payload, array $context ): array {
		...
		if ( empty( $updates ) ) {
			return $this->error( 'wpcc_no_user_updates', __( 'No fields to update.', 'action-steward' ) );
		}
	...
	private function delete_user( array $payload, array $context ): array {
		...
		if ( get_current_user_id() === $user_id ) {
			return $this->error( 'wpcc_cannot_delete_self', __( 'Cannot delete your own account.', 'action-steward' ) );
		}
		...
		if ( ! $result ) {
			return $this->error( 'wpcc_user_delete_failed', __( 'Failed to delete user.', 'action-steward' ) );
		}
	...
	private function reset_password( array $payload, array $context ): array {
		...
		if ( '' === $password ) {
			...
			if ( strlen( $password ) < 8 ) {
				return $this->error( 'wpcc_weak_password', __( 'Password must be at least 8 characters.', 'action-steward' ) );
			}
			$generated = false;
		}
		...
		if ( $generated ) {
			...
			$response['note'] = __( 'Password was auto-generated. Save it now — it will not be shown again.', 'action-steward' );
		}
	...
	private function assign_role( array $payload, array $context ): array {
		...
		if ( ! $wp_roles->is_role( $role ) ) {
			return $this->error( 'wpcc_invalid_role', __( 'Invalid role.', 'action-steward' ) );
		}
	...
	private function remove_role( array $payload, array $context ): array {
		...
		if ( ! in_array( $role, $user->roles, true ) ) {
			return $this->error( 'wpcc_user_role_not_assigned', __( 'User does not have this role.', 'action-steward' ) );
		}

		if ( 1 === count( $user->roles ) ) {
			return $this->error( 'wpcc_cannot_remove_last_role', __( 'Cannot remove the last role.', 'action-steward' ) );
		}
