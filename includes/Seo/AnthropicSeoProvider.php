...
final class AnthropicSeoProvider implements SeoMetaProvider {
	...
	public function suggest_meta( array $content, array $context = [] ): SeoMetaResult {
		...
		if ( ! $this->runtime->is_configured() ) {
			return SeoMetaResult::error( 'not_configured', __( 'No Anthropic API key configured.', 'siteradian' ), $this->id(), $model );
		}
		...
		if ( null === $parsed ) {
			return SeoMetaResult::error( 'invalid_response', __( 'The provider did not return valid SEO meta JSON.', 'siteradian' ), $this->id(), $model );
		}
