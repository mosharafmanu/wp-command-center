...
final class PathGuard {
	...
	public function resolve( string $relative_path ): string|\WP_Error {
		...
		if ( '' === $relative_path || str_contains( $relative_path, '..' ) ) {
			return new \WP_Error( 'wpcc_invalid_path', __( 'Invalid path.', 'siteradian' ) );
		}
		...
		if ( false === $real ) {
			return new \WP_Error( 'wpcc_not_found', __( 'The requested path does not exist.', 'siteradian' ) );
		}

		$real = wp_normalize_path( $real );

		if ( ! $this->is_within_allowed_root( $real ) ) {
			return new \WP_Error( 'wpcc_path_not_allowed', __( 'This path is outside the allowed directories (themes, plugins, mu-plugins).', 'siteradian' ) );
		}

		if ( $this->is_denied( $real ) ) {
			return new \WP_Error( 'wpcc_file_blocked', __( 'Access to this path is blocked for security reasons.', 'siteradian' ) );
		}
