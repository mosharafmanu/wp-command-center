...
final class AuthTokens {
	...
	public static function scope_label( string $scope ): string {
		$labels = [
			self::SCOPE_READ_ONLY => __( 'Read-only', 'siteradian' ),
			self::SCOPE_FULL      => __( 'Full access', 'siteradian' ),
		];
	...
	public static function status_badge( array $token ): string {
		if ( self::STATUS_REVOKED === $token['status'] ) {
			return sprintf( '<span class="wpcc-badge wpcc-badge--neutral">%s</span>', esc_html__( 'Revoked', 'siteradian' ) );
		}

		if ( null !== $token['expires_at'] && $token['expires_at'] < time() ) {
			return sprintf( '<span class="wpcc-badge wpcc-badge--critical">%s</span>', esc_html__( 'Expired', 'siteradian' ) );
		}

		return sprintf( '<span class="wpcc-badge wpcc-badge--good">%s</span>', esc_html__( 'Active', 'siteradian' ) );
	}
	...
	public function create( string $label, string $scope, ?int $expires_at, int $user_id ): array|\WP_Error {
		...
		if ( '' === $label ) {
			return new \WP_Error( 'wpcc_invalid_label', __( 'Please enter a label for this token.', 'siteradian' ) );
		}

		if ( ! in_array( $scope, self::VALID_SCOPES, true ) ) {
			return new \WP_Error( 'wpcc_invalid_scope', __( 'Invalid token scope.', 'siteradian' ) );
		}
	...
	public function delete( string $id ): bool|\WP_Error {
		...
		if ( count( $filtered ) === count( $manifest ) ) {
			return new \WP_Error( 'wpcc_token_not_found', __( 'Token not found.', 'siteradian' ) );
		}
	...
	public function validate( string $raw_token ): array|\WP_Error {
		...
		foreach ( $manifest as &$record ) {
			...
			if ( null !== $record['expires_at'] && $record['expires_at'] < time() ) {
				return new \WP_Error( 'wpcc_token_expired', __( 'This API token has expired.', 'siteradian' ), [ 'status' => 401 ] );
			}
	...
	private function get_storage_dir(): string|\WP_Error {
		return PrivateStore::dir(
			self::DIR_NAME,
			__( 'Failed to create the token storage directory.', 'siteradian' )
		);
	}
