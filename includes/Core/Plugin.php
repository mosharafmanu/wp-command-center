...
final class Plugin {
	...
	public function network_activation_notice(): void {
		...
		printf(
			...
			esc_html__( 'WP Command Center is network activated.', 'wp-command-center' ),
			esc_html__( 'This version supports one site at a time. Its tables and settings exist only for the site it was set up on, so other sites in this network will show an empty Command Center. Deactivate it for the network and activate it on each site that should have one.', 'wp-command-center' )
		);
	}
