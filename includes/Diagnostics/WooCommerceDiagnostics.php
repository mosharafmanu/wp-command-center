...
final class WooCommerceDiagnostics extends AbstractDiagnostics {
		...
		if ( ! class_exists( 'WooCommerce' ) ) {
			return [
				$this->check(
					...
					self::STATUS_INFO,
					__( 'WooCommerce is not active on this site.', 'ai-command-center' )
				),
			];
		}
	...
	private function check_db_version(): array {
		...
		return $this->check(
			...
			__( 'WooCommerce Database Version', 'ai-command-center' ),
			$up_to_date ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$up_to_date
				? sprintf(
					/* translators: %s: database version */
					__( 'WooCommerce database schema is up to date (%s).', 'ai-command-center' ),
					$db_version
				)
				: sprintf(
					/* translators: 1: database version, 2: plugin version */
					__( 'WooCommerce database version (%1$s) does not match the plugin version (%2$s). A database update may be pending.', 'ai-command-center' ),
					$db_version ?: __( 'unknown', 'ai-command-center' ),
					$wc_version ?: __( 'unknown', 'ai-command-center' )
				)
		);
	}
	...
	private function check_payment_gateways(): array {
		...
		if ( 0 === $count ) {
			return $this->check(
				...
				__( 'Payment Gateways', 'ai-command-center' ),
				self::STATUS_CRITICAL,
				__( 'No payment gateways are enabled. Customers will not be able to complete checkout.', 'ai-command-center' )
			);
		}
		...
		return $this->check(
			...
			sprintf(
				...
				__( 'Enabled payment gateways: %s.', 'ai-command-center' ),
	...
	private function check_scheduled_actions(): array {
		if ( ! class_exists( 'ActionScheduler_Store' ) ) {
			return $this->check(
				...
				__( 'Scheduled Actions', 'ai-command-center' ),
				self::STATUS_INFO,
				__( 'Action Scheduler is not available.', 'ai-command-center' )
			);
		}
		...
		if ( $failed > 0 ) {
			return $this->check(
				...
				sprintf(
					...
					__( '%1$d scheduled action(s) have failed, %2$d are pending. Failed actions may indicate a recurring error.', 'ai-command-center' ),
					$failed,
					$pending
				)
			);
		...
		return $this->check(
			...
			sprintf(
				...
				__( 'No failed scheduled actions. %d pending.', 'ai-command-center' ),
				$pending
			)
		);
	}
	...
	private function check_template_overrides(): array {
		...
		if ( empty( $overrides ) ) {
			return $this->check(
				...
				__( 'Template Overrides', 'ai-command-center' ),
				self::STATUS_GOOD,
				__( 'No theme template overrides found.', 'ai-command-center' )
			);
		}
		...
		if ( ! empty( $outdated ) ) {
			return $this->check(
				...
				sprintf(
					...
					__( '%1$d of %2$d overridden WooCommerce template(s) are outdated compared to the installed plugin version.', 'ai-command-center' ),
					count( $outdated ),
					count( $overrides )
				)
			);
		...
		return $this->check(
			...
			sprintf(
				...
				_n( '%d theme template override found, all up to date.', '%d theme template overrides found, all up to date.', count( $overrides ), 'ai-command-center' ),
				count( $overrides )
			)
		);
	}
