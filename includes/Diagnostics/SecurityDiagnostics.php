...
final class SecurityDiagnostics extends AbstractDiagnostics {
	...
	private function check_debug_display( array $debug ): array {
		...
		return $this->check(
			'wp_debug_display',
			__( 'Debug Display', 'siteradian' ),
			$enabled ? self::STATUS_CRITICAL : self::STATUS_GOOD,
			$enabled
				? __( 'WP_DEBUG_DISPLAY is enabled — PHP errors may be shown to site visitors, leaking file paths and code details.', 'siteradian' )
				: __( 'PHP errors are not displayed to visitors.', 'siteradian' )
		);
	}
	...
	private function check_file_edit(): array {
		...
		return $this->check(
			'file_edit',
			__( 'Theme/Plugin File Editor', 'siteradian' ),
			$disabled ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$disabled
				? __( 'The built-in theme and plugin file editor is disabled.', 'siteradian' )
				: __( 'The built-in theme/plugin file editor is enabled. Consider setting DISALLOW_FILE_EDIT to true.', 'siteradian' )
		);
	}
	...
	private function check_wp_config_permissions( array $file_permissions ): array {
		...
		if ( ! $info['exists'] ) {
			return $this->check(
				...
				__( 'wp-config.php Permissions', 'siteradian' ),
				self::STATUS_INFO,
				__( 'wp-config.php was not found at the expected location.', 'siteradian' )
			);
		}
		...
		return $this->check(
			...
			$world_writable
				? sprintf(
					...
					__( 'wp-config.php is world-writable (%s). Restrict permissions to 600 or 644.', 'siteradian' ),
					$info['permissions']
				)
				: sprintf(
					/* translators: %s: file permission octal value */
					__( 'wp-config.php permissions look reasonable (%s).', 'siteradian' ),
					$info['permissions']
				)
		);
	}
	...
	private function check_ssl( array $wordpress ): array {
		...
		return $this->check(
			'ssl',
			__( 'SSL (HTTPS)', 'siteradian' ),
			$enabled ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$enabled
				? __( 'The site is being served over HTTPS.', 'siteradian' )
				: __( 'The site is not using HTTPS. An SSL certificate is strongly recommended.', 'siteradian' )
		);
	}
	...
	private function check_default_admin_account(): array {
		...
		if ( ! $user ) {
			return $this->check(
				...
				__( 'Default "admin" Account', 'siteradian' ),
				self::STATUS_GOOD,
				__( 'No user with the username "admin" exists.', 'siteradian' )
			);
		}
		...
		return $this->check(
			...
			$is_administrator
				? __( 'A user with the username "admin" has the Administrator role — a common brute-force target. Consider renaming or removing it.', 'siteradian' )
				: __( 'A user with the username "admin" exists but is not an administrator.', 'siteradian' )
		);
	}
	...
	private function check_directory_listing(): array {
		...
		return $this->check(
			'directory_listing',
			__( 'Uploads Directory Listing', 'siteradian' ),
			$protected ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$protected
				? __( 'The uploads directory contains an index file that prevents directory listing.', 'siteradian' )
				: __( 'The uploads directory has no index.php/index.html — directory listing may be possible if the server allows it.', 'siteradian' )
		);
	}
	...
	private function check_core_update(): array {
		...
		if ( ! is_array( $updates ) || empty( $updates ) || ! isset( $updates[0]->response ) ) {
			return $this->check(
				'core_update',
				__( 'WordPress Core Updates', 'siteradian' ),
				self::STATUS_INFO,
				__( 'Update status is unknown — WordPress has not checked for updates yet.', 'siteradian' )
			);
		}
		...
		if ( 'latest' === $latest->response || 'development' === $latest->response ) {
			return $this->check(
				...
				__( 'WordPress Core Updates', 'siteradian' ),
				self::STATUS_GOOD,
				__( 'WordPress is up to date.', 'siteradian' )
			);
		}
		...
		return $this->check(
			...
			sprintf(
				...
				__( 'A WordPress core update is available (%s).', 'siteradian' ),
				$latest->version ?? ''
			)
		);
	}
