...
final class PerformanceDiagnostics extends AbstractDiagnostics {
	...
	private function check_memory_limit( array $php ): array {
		...
		if ( $bytes <= 0 ) {
			...
			$description = __( 'No PHP memory limit is set (unlimited).', 'ai-command-center' );
		} elseif ( $bytes < 64 * MB_IN_BYTES ) {
			$status      = self::STATUS_CRITICAL;
			$description = __( 'PHP memory limit is below 64 MB and may cause "allowed memory size exhausted" fatal errors.', 'ai-command-center' );
		} elseif ( $bytes < 128 * MB_IN_BYTES ) {
			$status      = self::STATUS_RECOMMENDED;
			$description = __( 'PHP memory limit is below the recommended 128 MB minimum.', 'ai-command-center' );
		} else {
			$status      = self::STATUS_GOOD;
			$description = __( 'PHP memory limit is sufficient.', 'ai-command-center' );
		}

		return $this->check( 'memory_limit', __( 'PHP Memory Limit', 'ai-command-center' ), $status, sprintf( '%s (%s)', $description, $raw ) );
	}

	private function check_memory_usage(): array {
		return $this->check(
			'memory_usage',
			__( 'Current Request Memory Usage', 'ai-command-center' ),
			self::STATUS_INFO,
			sprintf(
				/* translators: 1: current memory usage, 2: peak memory usage */
				__( 'Current: %1$s, Peak: %2$s', 'ai-command-center' ),
	...
	private function check_object_cache( array $cache ): array {
		...
		return $this->check(
			'object_cache',
			__( 'External Object Cache', 'ai-command-center' ),
			$enabled ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$enabled
				? __( 'A persistent object cache (e.g. Redis/Memcached) is active.', 'ai-command-center' )
				: __( 'No persistent object cache detected. Database-heavy sites benefit from one.', 'ai-command-center' )
		);
	}
	...
	private function check_opcache( array $cache ): array {
		...
		return $this->check(
			'opcache',
			__( 'OPcache', 'ai-command-center' ),
			$enabled ? self::STATUS_GOOD : self::STATUS_RECOMMENDED,
			$enabled
				? __( 'PHP OPcache is enabled.', 'ai-command-center' )
				: __( 'PHP OPcache is not enabled. Enabling it significantly speeds up PHP execution.', 'ai-command-center' )
		);
	}
	...
	private function check_page_cache( array $cache ): array {
		if ( $cache['page_cache_dropin'] || ! empty( $cache['caching_plugins'] ) ) {
			$source = ! empty( $cache['caching_plugins'] )
				...
				: __( 'advanced-cache.php drop-in', 'ai-command-center' );

			return $this->check(
				'page_cache',
				__( 'Page Caching', 'ai-command-center' ),
				self::STATUS_GOOD,
				sprintf(
					/* translators: %s: detected caching plugin(s) or drop-in */
					__( 'Page caching detected: %s.', 'ai-command-center' ),
					$source
				)
			);
		}
		...
		return $this->check(
			...
			__( 'No page caching plugin or drop-in detected.', 'ai-command-center' )
		);
	}
	...
	private function check_autoloaded_options(): array {
		...
		return $this->check(
			'autoloaded_options',
			__( 'Autoloaded Options Size', 'ai-command-center' ),
			$status,
			sprintf(
				/* translators: %s: formatted size of autoloaded options */
				__( 'Total size of autoloaded options: %s. Large autoloaded data is loaded on every page request.', 'ai-command-center' ),
				size_format( $bytes )
			)
		);
	}
	...
	private function check_active_plugins( array $plugins ): array {
		...
		return $this->check(
			...
			__( 'Active Plugins', 'ai-command-center' ),
			$count > 50 ? self::STATUS_RECOMMENDED : self::STATUS_INFO,
			sprintf(
				/* translators: %d: number of active plugins */
				_n( '%d plugin is active.', '%d plugins are active.', $count, 'ai-command-center' ),
				$count
			)
		);
	}
	...
	private function check_cron(): array {
		...
		return $this->check(
			'wp_cron',
			__( 'WP-Cron', 'ai-command-center' ),
			self::STATUS_INFO,
			$disabled
				? __( 'WP-Cron is disabled via DISABLE_WP_CRON. Ensure a real system cron job triggers wp-cron.php.', 'ai-command-center' )
				: __( 'WP-Cron runs on page loads (default behavior).', 'ai-command-center' )
		);
	}
}
