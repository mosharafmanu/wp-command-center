...
TASK=$(api POST /agent/tasks "$(jq -nc --arg sid "$SID" '{session_id:$sid,source:"api",user_prompt:"Validate the SiteRadian V1 beta workflow"}')"); TID=$(echo "$TASK" | jq -r '.task_id // empty')
