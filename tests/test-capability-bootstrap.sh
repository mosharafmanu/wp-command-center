...
assert_eq "D: read_only profile has exactly 4 read capabilities" "4" "$(echo "$CAPS_D" | jq -r 'length')"
...
RESP=$(mcp "$TOKEN_D" '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"plugin_manage","arguments":{"action":"plugin_activate"}},"id":5}')
