...
echo
echo "== 1. ContentRowActions: retains no-JS fallback + bulk (row link consolidated) =="
has  "retains WP bulk actions"                 "bulk_actions-edit-"          "$CRA"
has  "creates drafts via the content generator" "ContentFieldGenerator"      "$CRA"
has  "retains no-JS admin-post handler"        "admin_post_"                 "$CRA"
...
lacks "row link consolidated (no per-kind anchor)" "data-wpcc-action="       "$CRA"
...
echo
echo "== 2. MediaRowActions: retains no-JS fallback + bulk (row link consolidated) =="
has  "retains upload bulk actions"             "bulk_actions-upload"         "$MRA"
has  "creates drafts via the alt generator"    "AltTextGenerator"            "$MRA"
has  "retains no-JS admin-post handler"        "admin_post_"                 "$MRA"
has  "gated by build flag"                     "WPCC_ALT_TEXT_UI"            "$MRA"
has  "gated by FeatureGate"                     "ai_alt_text"                "$MRA"
lacks "row link consolidated (no per-kind anchor)" "data-wpcc-action="       "$MRA"
lacks "never applies"                          "ProposalApplyService"        "$MRA"
lacks "no new REST route"                      "register_rest_route"         "$MRA"

echo
echo "== 3. ActionPanelAssets: one panel, reads the registry, existing routes only =="
has  "reads the AI action registry"           "AiActionRegistry"             "$APA"
has  "localizes via the registry"             "->localized("                 "$APA"
has  "computes enabled actions for screen"    "enabled_for_type"             "$APA"
has  "still enqueues the panel JS"            "assets/js/wpcc-action-panel.js" "$APA"
has  "handles the Media Library screen"       "'upload.php'"                 "$APA"
has  "adds chooser i18n"                       "'chooserTitle'"              "$APA"
has  "registry: content -> proposals route"   "'path' => '/admin/proposals'" "$REG"
has  "registry: content generate branch"      "'generate_kind' => \$kind"    "$REG"
has  "registry: SEO -> seo generate route"    "/admin/seo/generate"          "$REG"
has  "registry: Alt -> alt generate route"    "/admin/alt-text/generate"     "$REG"
has  "registry: content apply -> content_update" "'content_update'"          "$REG"
has  "registry: alt apply -> media_update"    "'media_update'"               "$REG"
...
if ! command -v wp >/dev/null 2>&1; then
	...
	assert_eq "capabilities == 24"  "24" "$(wpe 'echo count(\WPCommandCenter\Operations\CapabilityRegistry::ALL_CAPABILITIES);')"
