...
assert_contains "Continue primary copy is one list entry" "$CONTINUE_ENTRY" "- name: siteradian"
...
assert_contains "Continue entry keeps child indentation" "$CONTINUE_ENTRY" "  - name: siteradian"
...
assert_contains "Codex primary Step 1 is concise" "$VIEW_TEXT" "Make the token available in this terminal"
...
assert_contains "small families begin as one spacious card column" "$VIEW_TEXT" ".wpcc-ai-picks { display:grid;grid-template-columns:minmax(0,1fr)"
...
assert_contains "compact cards override WordPress no-wrap styling" "$VIEW_TEXT" "white-space:normal !important;overflow-wrap:break-word"
assert_contains "cards share consistent row heights" "$VIEW_TEXT" "grid-auto-rows:1fr;align-items:stretch"
assert_contains "card badges align to the bottom" "$VIEW_TEXT" "margin-top:auto;padding:0;border:0;background:transparent"
assert_contains "selected cards reserve room for one confirmation mark" "$VIEW_TEXT" "padding:16px 44px 16px 16px !important"
assert_contains "narrow layout starts with a single card column" "$VIEW_TEXT" "grid-template-columns:minmax(0,1fr);grid-auto-rows:1fr"
assert_contains "large editor groups stop at four cards per row" "$VIEW_TEXT" ".wpcc-ai-family--wide .wpcc-ai-picks { grid-template-columns:repeat(4,minmax(0,1fr)); }"
...
assert_contains "Claude primary flow copies one entry" "$VIEW_TEXT" "Copy SiteRadian entry"
assert_contains "Claude primary flow preserves existing servers" "$VIEW_TEXT" "keep every other entry"
assert_contains "VS Code primary flow names the secure prompt" "$VIEW_TEXT" "paste the SiteRadian token when VS Code asks"
...
assert_contains "VS Code DCR recovery identifies invalid credential" "$VSCODE_NOTES" "saved SiteRadian token was missing or invalid"
