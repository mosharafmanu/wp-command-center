...
#     layer; invariants stay 34 / 23 / 42 / 42 / 2.6.0
#
...
# V1: retired as a section; the slug survives only as a legacy redirect source.
has "legacy slug: Built-in AI"       "BUILTIN_SLUG\s*=\s*'wpcc-built-in-ai'" "$SHELL_PHP"
...
has "map: patches -> advanced" "'wpcc-patches'" "$SHELL_PHP"
has "map: diagnostics -> advanced" "'wpcc-diagnostics'" "$SHELL_PHP"
has "map: site-intelligence -> advanced" "'wpcc-site-intelligence'" "$SHELL_PHP"
has "map: tokens -> connections" "'wpcc-tokens'" "$SHELL_PHP"
...
has "map: ai-integrations -> connections" "'wpcc-ai-integrations'" "$SHELL_PHP"
has "map: ai-setup -> advanced" "'wpcc-ai-setup'" "$SHELL_PHP"
...
lacks "Built-in AI not a top-level menu" "render_builtin"  "$MENU"
lacks "Connect retired from menu" "render_connect"  "$MENU"
...
# V1: Home no longer carries the proposal feed, the engineering counter cards,
# or the localStorage readiness widget. It answers readiness with real state and
# exactly one next action; the removed detail lives on the screens that own it.
lacks "no proposals feed on Home"         "/proposals"    "$HOME_VIEW"
has "uses REST nonce"                      "wp_rest"       "$HOME_VIEW"
has "Needs Attention hero"                "wpcc-cds-attn|renderAttn" "$HOME_VIEW"
lacks "no duplicate readiness widget"     "renderReadiness" "$HOME_VIEW"
has "status strip (protection/assistant/approvals)" "wpcc-home__status" "$HOME_VIEW"
lacks "no engineering counter cards"      "renderCards"   "$HOME_VIEW"
has "recent changes in plain language"    "r.areas"       "$HOME_VIEW"
lacks "no AI workflow feed on Home"       "renderAi"      "$HOME_VIEW"
...
# The "Audited" chip appeared on every row (everything is audited), so it carried
# no signal. The guarantee is stated once, in the guide, instead of per row.
has "invariants kept under Engineer disclosure" "wpcc-engineer-only" "$HOME_VIEW"
