...
- **⚠️ AUTO-DEPLOY WEBHOOK STILL BROKEN.** GitHub Actions → `wpcc-deploy.php` returns 403 (`{"error":"forbidden"}`) because the GH secret `DEPLOY_TOKEN` ≠ the token `wpcc-deploy.php` expects (rotated since the 06:14 success). FIX: realign — read expected token in `~/domains/mosharafmanu.com/public_html/wpcc-deploy.php` (or set a fresh one there) and set the SAME value as GH repo secret `DEPLOY_TOKEN`, then re-run the workflow. Until fixed, deploy manually via the SSH `git reset --hard origin/main` flow above. (Local doc commits b864e16/this one are unpushed and NOT on prod — prod = code only.)
