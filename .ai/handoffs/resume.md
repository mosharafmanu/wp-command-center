...
- **DEPLOY STATUS (2026-06-14): PUSHED but DEPLOY FAILED — production NOT updated.** Serial T2 gate was CLEAN (3533 passed / 24 failed / **net-new 0**, only the chronic-24 baseline). Pushed batch `547bc08` to origin/main (clean fast-forward over `105ecf3`, +15 commits: F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner + docs). **The GitHub Actions "Deploy to Production" run for `547bc08` FAILED** at step "Trigger server-side git pull" (run id `27501261103`). **Production still runs `105ecf3`** (last successful deploy, 2026-06-14 06:14 UTC) — NOT the new batch. Plugin IS active + REST/MCP transports up (prod /health & /mcp return `wpcc_missing_token`, not `rest_no_route`). Root cause of deploy failure is in the auth-gated Actions logs (need GH token) — likely `DEPLOY_TOKEN` expired/rotated, or a server-side git/cache or >60s timeout in `wpcc-deploy.php`. **TO FINISH DEPLOY:** owner checks Actions run `27501261103` logs + `https://mosharafmanu.com/wpcc-deploy.php` response, fixes the hook, then re-runs the deploy (re-run workflow, or trigger the webhook). Do NOT blind-retry repeatedly (deploy hook's `git reset --hard` can race-deactivate the live plugin). Code gate itself is GREEN.
