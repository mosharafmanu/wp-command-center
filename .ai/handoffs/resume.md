...
- **Active remediation sprint** against `WPCC-Acceptance-Test-Report-2026-06-14.md`: **F3.1 ✅ `13bf288`**, **F6.1 ✅ `c3bebbe`**, **F6.2 ✅ `9aa5f81`**, **F2.2 ✅** done. **F2.1 RECLASSIFIED → "requires production reproduction"** (not reproducible on dev; report's blocking-HTTP hypothesis disproven by code — `seo_get` is pure `get_post_meta`, ~1.35s on dev; env gap = prod Rank Math/LiteSpeed vs dev Yoast/Apache). **Next = F3.2 (nested ACF read)** — NOT started.
...
- **F3.2** 🟠 Nested ACF read: `acf_group_get`/`acf_field_get` omit repeater sub-fields + flex-content layouts; no export action. Can create nested structures but not read them back. — **NEXT.**
...
- **NEXT TASK when resumed:** F3.2 (nested ACF read). ACF Pro active on dev → fully reproducible. Use `tests/run.sh --tier T1 --runtime acf` (then `--tier T2` before any deploy). Source of truth: `/Volumes/Tools/MyProductions-Markdown/WPCC/WPCC-Acceptance-Test-Report-2026-06-14.md`.
