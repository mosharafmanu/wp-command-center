...
- **Production:** runs roadmap **STEP 98** (`586f38c` deployed & verified; 38 ops/MCP tools; Developer mode). All work since is **local-only, NOT pushed** (13 unpushed commits; owner deploys). **T2 (full regression) NOT yet run — it is the pre-deploy gate: `tests/run.sh --tier T2 -j6` before pushing.**
