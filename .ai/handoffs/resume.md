...
- **REMEDIATION SPRINT COMPLETE.** All 🔴/🟠 acceptance-report findings are fixed locally and tested; F2.1 deferred to production reproduction. No finding is in progress. Awaiting owner: (a) deploy the batch, and/or (b) choose the next thread (lower-priority finding or STEP 100.3). **Do not auto-start new work.**
- **DEPLOY STATUS (2026-06-14): ✅ LIVE at `547bc08` (deployed MANUALLY via SSH).** Serial T2 gate CLEAN (3533/24/**net-new 0**). Pushed `547bc08` to origin/main, but the GitHub Actions auto-deploy FAILED (403 `DEPLOY_TOKEN` mismatch — `curl -f` exit 22). Deployed manually over SSH (Hostinger, plugin at `~/domains/mosharafmanu.com/public_html/wp-content/plugins/wp-command-center`, wp at `/usr/local/bin/wp`): clean `git reset --hard origin/main` (working tree was clean), `wp cache flush`, plugin stayed ACTIVE. **Verified live:** HEAD=`547bc08`; F2.2 `apply_time_budget` + F3.2 `detail_field` present; `McpServerRuntime::time_budget()`=200; tools/list=38; error_catalog has `wpcc_operation_timeout`; ext `/health`=401, `/mcp`=500 (pre-existing missing-token-no-status quirk, structured body, not a fatal). **Whole batch now live:** F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner.
- **AUTO-DEPLOY WEBHOOK: server side FIXED 2026-06-14 — one GH action pending.** `wpcc-deploy.php` ($token / hash_equals → `git fetch` + `reset --hard origin/main` + `wp cache flush`) had a DEPLOY_TOKEN mismatch (GH secret ≠ file token, drifted after 06:14). **Rotated the server token to a fresh 64-hex value** (backup saved `wpcc-deploy.php.bak.*`); webhook now verified: WITH token → HTTP 200 (`code:0`, reset to 547bc08, cache flushed), WITHOUT → 403. Plugin stayed ACTIVE through the reset. **PENDING (owner): set GH repo secret `DEPLOY_TOKEN` = the new value (provided out-of-band in chat), then re-run the failed workflow / next push auto-deploys.** The token VALUE is intentionally NOT stored in this repo. Manual deploy fallback remains the SSH `git reset --hard origin/main` flow.
- **CODE PARITY VERIFIED:** `includes/` git tree hash `2a730c39…` identical on local, GitHub origin/main, and live server — all at `547bc08`. Local is +2 doc-only commits (resume.md), no code delta. Server `mosharafmanu-automation` folder = LEGACY content seed/fix WP-CLI scripts (148 php, run via `wp eval-file`), NOT a deploy mechanism — superseded by the WPCC plugin runtime; can be archived.
