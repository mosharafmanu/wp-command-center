...
- **Production:** runs roadmap **STEP 98** (`586f38c` deployed & verified; 38 ops/MCP tools; Developer mode). All work since is **local-only, NOT pushed** (14 unpushed commits; owner deploys).
- **DEPLOY STATUS (2026-06-14): NOT pushed.** Code is clean — **0 real net-new regressions** (F2.2/F3.2 + all touched suites pass standalone). BUT the pre-deploy `--tier T2 -j6` run produced **168 FALSE net-new failures** = parallel-isolation artifact (see ⚠️ under Tiered Regression Runner; suites aren't concurrency-safe). **A valid clean gate needs a SERIAL T2 run (`tests/run.sh --tier T2`, ~32m, no -j).** Owner instruction was "stop & report on T2 failure" → did NOT push. To deploy: run serial T2 → if 0 net-new, push main (deploy hook fires) → verify.
