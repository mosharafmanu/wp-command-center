...
- **DEPLOY STATUS (2026-06-14, updated): ✅ PRODUCTION LIVE & CURRENT at `b386092`** (= `origin/main`; SSH-verified `git rev-parse HEAD` + plugin ACTIVE). Pushed `64fa67b..b386092` and the **GitHub Actions auto-deploy webhook SUCCEEDED this run** (next data point after run #19 ✓ / run #20 ✗ — so WAF intermittency is NOT solved, this run just wasn't blocked). The deployed commit is docs-only (handoff resume.md); no code change. Prior batch (F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner) remains live.
...
- **HOW TO DEPLOY (until the auto-deploy decision is made):** Manual SSH is reliable and what I used. SSH = Hostinger `ssh -p 65002 u916998506@72.62.68.183` (pw `DevAdmin#26` — **ROTATE, it's in chat**). Plugin dir `~/domains/mosharafmanu.com/public_html/wp-content/plugins/wp-command-center`; WP root `~/domains/mosharafmanu.com/public_html`; wp at `/usr/local/bin/wp`. Deploy = `cd <plugindir> && git fetch origin && git reset --hard origin/main && wp cache flush --path=<wproot>` (capture `wp plugin is-active wp-command-center` first; reactivate after if needed — reset-race guard). The webhook ALSO works from a non-datacenter IP: `curl -s -X POST -H "Authorization: Bearer <DEPLOY_TOKEN>" -A "wpcc-deploy-bot" https://mosharafmanu.com/wpcc-deploy.php` → 200.
- **AUTO-DEPLOY WEBHOOK — partially fixed, INTERMITTENT (the open decision above).** `wpcc-deploy.php` (web root): `hash_equals($token, …)` → `git fetch` + `reset --hard origin/main` + `wp cache flush`. Fixed: (1) DEPLOY_TOKEN drift — rotated server `$token` to a fresh 64-hex value (backups `wpcc-deploy.php.bak.*`), owner set matching GH repo secret `DEPLOY_TOKEN` (CI-verified: length 64, sha256 MATCH). (2) deploy.yml now sends custom UA `-A "wpcc-deploy-bot"`. **STILL UNRELIABLE:** host WAF intermittently 403s GitHub-runner (datacenter) IPs even with correct token + custom UA (run #19 ✓, run #20 ✗; likely IP-reputation/rate — my burst of test requests may have aggravated it). Token value & sha256 are NOT in repo (token is in chat history → consider rotating). Backups left on server.
