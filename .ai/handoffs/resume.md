...
- **DEPLOY STATUS (2026-06-14, updated): ✅ PRODUCTION LIVE & CURRENT at `b386092`** (= `origin/main`; SSH-verified `git rev-parse HEAD` + plugin ACTIVE). Pushed `64fa67b..b386092` via GitHub Actions auto-deploy. The deployed commit is docs-only (handoff resume.md); no code change. (Superseded same day by the SSH auto-deploy — see top bullet; prod is now further ahead.) Prior batch (F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner) remains live.
