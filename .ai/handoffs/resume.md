...
- **⚠️ AUTO-DEPLOY CI IS CURRENTLY BROKEN.** The SSH auto-deploy (GH Actions `deploy.yml`) **FAILED** on the 2026-06-15 push (run `27531805301`, step "Deploy over SSH" = failure; prior runs were green). Deploy was completed via the **documented MANUAL FALLBACK** (SSH `git fetch` + `reset --hard origin/main` + reactivate-guard + `wp cache flush` — all worked). **Until `deploy.yml` is investigated, future `git push` will likely NOT auto-deploy — use the manual fallback (below).** Step logs need a GH token (not available locally); server SSH/git/wp-cli all function manually. Root cause TBD.
