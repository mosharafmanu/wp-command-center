...
- **▶ AUTO-DEPLOY DECISION RESOLVED (2026-06-14): owner chose SSH-based auto-deploy.** Implemented: dedicated ed25519 deploy key `wpcc-github-actions-deploy` — PUBLIC half installed in server `~/.ssh/authorized_keys` (additive/idempotent; password login untouched), PRIVATE half = single GH repo secret `DEPLOY_SSH_KEY`. `deploy.yml` rewritten to SSH in (host/port/user hardcoded `72.62.68.183`:`65002`/`u916998506`) and run `git fetch` + `reset --hard origin/main` + reactivate-if-was-active + `wp cache flush` — bypasses the web endpoint/WAF entirely. Key-auth verified end-to-end from a workstation (PasswordAuthentication=no, BatchMode). **PENDING:** owner adds the `DEPLOY_SSH_KEY` secret in GitHub → then push the workflow commit (held locally until secret is set) so the first SSH-deploy run is green. Old webhook (`wpcc-deploy.php`) left in place as a manual fallback. **Do not start any other work** (remediation/STEP 100) unless owner redirects.
