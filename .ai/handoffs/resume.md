...
- **F6.1 workflow on_failure:rollback no-op — FIXED locally.** Root cause (my STEP 97 code): a step was only recorded reversible if its sub-op returned a `rollback_id`; `content_create` returns none, so create steps were skipped on rollback → orphaned posts. Fix: capture each step's `created` resource IDs (from executor `created`), mark such steps `rollbackable`, and on rollback reverse via dispatcher (rollback_id) **and/or** delete created posts, then **verify** they're gone (status `rolled_back` only when all verified, else `rollback_incomplete`). `workflow_rollback` rebuilds reversible set from created IDs too. `test-workflow-rollback-f61.sh` 16/16; step97 36/36, workflow-runtime 53/53. Next = F6.2.
