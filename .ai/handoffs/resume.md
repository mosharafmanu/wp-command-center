...
## ►► SESSION STATE — START HERE (2026-06-15 — STEP 100 batch DEPLOYED to production)

- **✅ STEP 100 (Media Enhancement) CORE ROADMAP 100.1–100.9 COMPLETE + DEPLOYED TO PRODUCTION (2026-06-15).** Production `mosharafmanu.com`, `origin/main`, and local are all at **`5d0efaa`** (server working tree clean, plugin ACTIVE, plugin v0.1.0). The STEP 100 **plugin code** is `d30844f`→`279f6e9` (100.3–100.9 `d30844f`→`da3fcc9` + legacy-gallery-ids resolver fix `279f6e9`); commits after that (`366ce5a` validation docs, `32dfc95`/`aa972f2`/`f6435a6`/`5d0efaa` deploy-tooling + handoff/docs) carry **no plugin behavior change**. Pre-deploy gate = **SERIAL T2 CLEAN** (3533 passed / 24 chronic baseline / **net-new 0**). Post-deploy verified: 39 ops, `media_enhance` present (26 actions incl `unused_media_cleanup`/`media_usage_scan`/`webp_generate`), capability map valid (`media_enhance→media.manage`), ext `/health`=401, MCP `tools/list`=39 with `media_enhance`, external REST read actions all OK (full token), no debug.log errors. `operation_map`=33, 39 ops/39 MCP tools.
- **✅ DEPLOY MODEL CHANGED → PULL-BASED (2026-06-15). Root cause of the push-deploy failure FOUND.** The host (Hostinger) now **blocks inbound SSH from GitHub-hosted runner IPs** — proven: GH Actions `ssh: connect to host 72.62.68.183 port 65002: Connection timed out`; the same host answers a home/allowed IP instantly (`ssh-keyscan` 8 keys <1s, manual SSH works); the deploy key is valid (`ssh-keygen -y` passes). The old push-based `deploy.yml` died silently at `ssh-keyscan` (5s timeout → exit 1 under `set -e`, stderr to `/dev/null`) BEFORE the key was ever used — NOT a key/secret/code issue (the `DEPLOY_SSH_KEY_B64` secret rotated ~15h prior is fine). **NEW deploy = server PULLS from GitHub (outbound not blocked) — LIVE & VERIFIED:** server script `~/wpcc-deploy.sh` (git fetch + `reset --hard origin/main` + reactivate-if-active + `wp cache flush`; idempotent, flock-guarded, logs to `~/wpcc-deploy.log`). hPanel cron **`* * * * *`** (every minute) added by owner 2026-06-15: `/bin/bash /home/u916998506/wpcc-deploy.sh >/dev/null 2>&1` (no `crontab` binary over SSH on this host → hPanel-managed). **End-to-end VERIFIED:** push `5d0efaa` 09:10:45Z → cron auto-pulled it by 09:11:09Z (~24s; `~/wpcc-deploy.log` line `DEPLOYED f6435a6 -> 5d0efaa active=yes`). `deploy.yml` is a **green no-op notice** (pushes no longer fail; do NOT rely on it to deploy). **So the deploy flow is now: just `git push origin main` → live within ~1 min.** Full runbook: `.ai/DEPLOY.md`. MANUAL fallback if ever needed (from an allowed IP): `ssh -p 65002 u916998506@72.62.68.183` then `bash ~/wpcc-deploy.sh`.
- **STEP 100 real-world validation (2026-06-15):** live multi-plugin stack (Woo + ACF Pro + Elementor Pro + GD/WebP). 41/41 classification PASS, **0 false positives among protected content**; byte-for-byte reversibility proven (regenerate/webp/optimize/cleanup); DestructiveGuard + read-token rejection + capability fail-closed verified. Docs: `.ai/steps/STEP-100-REAL-WORLD-VALIDATION.md` (plan), `STEP-100-VALIDATION-REPORT.md` (report), `artifacts/step-100-validation/` (61 evidence files). Verdict: **SAFE FOR STAGING; NOT yet safe for UNATTENDED PROD cleanup.**
- **🚧 PROD `unused_media_cleanup` GATING:** 6 structural false-positive blind spots (all reversible — trash+snapshot, never permanent): Woo category term-meta, `woocommerce_placeholder_image` option-ID, ACF Gutenberg-block field, Elementor global kit (`_elementor_page_settings`), file-only hardcoded URL, + legacy gallery (now FIXED in `279f6e9`). Until the resolver covers these sources (BACKLOG), run cleanup only with **human review / `wpcc_media_cleanup_protected`**. All reads + reversible writes are safe to use freely.
- **Observation (not a defect):** read-only tokens correctly get `wpcc_capability_denied` on `media_enhance` (the `read_only` capability profile = `database.inspect` + `search.manage` only; media ops need `media.manage`). Read actions work with a **full** token.
- **NEXT (owner choice — nothing in progress; safe to stop here):** strategic backlog — `report_media` + cross-runtime image audits (`product_image_audit`/`acf_image_audit`/`elementor_image_audit`) + optimizer-interop + close the 6 resolver blind-spot sources (term-meta / option-ID / ACF-block JSON / `_elementor_page_settings` / file-scan / legacy-gallery-done); DEFER AVIF / cross-runtime delegating-writes / `media_duplicate`. Permanent media purge intentionally OUT OF SCOPE. (Deploy + STEP 100 ship are DONE — see above.) Optional housekeeping: rotate the SSH password (no longer needed by CI; manual-fallback only).

---

### Prior session state (2026-06-14, superseded by the deploy above)

- **✅ AUTO-DEPLOY RESOLVED & WORKING (2026-06-14): SSH-based auto-deploy is LIVE and GREEN.** First green run = `bb6de7d` (GH Actions run 27505281401 success; server `git rev-parse HEAD`=bb6de7d = origin/main, plugin ACTIVE). Mechanism: dedicated ed25519 deploy key `wpcc-github-actions-deploy` — PUBLIC half in server `~/.ssh/authorized_keys` (additive; password login untouched), PRIVATE half stored **base64 single-line** as GH repo secret **`DEPLOY_SSH_KEY_B64`** (NOT `DEPLOY_SSH_KEY` — the raw multi-line PEM mangled in the secret → `error in libcrypto`; first SSH run `a74077e` failed for exactly that, fixed in `bb6de7d`). `deploy.yml`: runner SSHes in (host/port/user hardcoded `72.62.68.183`:`65002`/`u916998506`), `base64 -d` the key + `ssh-keygen -y` preflight, then `git fetch` + `reset --hard origin/main` + reactivate-if-was-active + `wp cache flush`. **To deploy now: just `git push origin main`** — no manual SSH needed. The only deploy secret is `DEPLOY_SSH_KEY_B64`. **OLD WEBHOOK FULLY DECOMMISSIONED (2026-06-14):** server `wpcc-deploy.php` + `.bak` backups deleted (endpoint now 404), `DEPLOY_TOKEN` GH secret removed by owner, all repo refs scrubbed — SSH is the sole deploy path, no fallback webhook remains. **Cleanup TODO (owner):** rotate the SSH *password* (still in chat; key auth is now the deploy path so the password isn't needed by CI). **Do not start any other work** (remediation/STEP 100) unless owner redirects.
- **NOTE on `mosharafmanu-automation` (verified 2026-06-14):** legacy content-seed scripts (57 `seed-*.php` + `rank-math-seo.php`, own `.git`, last touched May 17). NOT a WP plugin (no Plugin Name header), NOT in the plugin list, NOT active, NO crontab entries. Cannot affect the plugin or the deploy (deploy only resets inside the plugin dir, which this is outside of). Safe to archive.
- **REMEDIATION SPRINT COMPLETE.** All 🔴/🟠 acceptance-report findings are fixed, tested, committed, and DEPLOYED LIVE; F2.1 deferred to production reproduction. No finding in progress.
- **DEPLOY STATUS (2026-06-14, current): ✅ PRODUCTION LIVE & CURRENT at `c57f251`** (= `origin/main`; SSH auto-deploy green; plugin ACTIVE). All commits since `b386092` are CI/deploy-tooling + handoff docs only (SSH deploy migration `a74077e`→`bb6de7d`, doc `663c9ce`, webhook-scrub `c57f251`) — no plugin code change. Prior batch (F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner) remains live.
- **DEPLOY STATUS (prior baseline): ✅ at `64fa67b`** (= `origin/main`; server `git reset --hard` verified; plugin ACTIVE). Serial T2 gate was CLEAN (3533/24/**net-new 0**). The whole batch is live: F3.1/F6.1/F6.2/F2.2/F3.2 + STEP 100.1/100.2 + tiered runner + CI/doc commits. Verified earlier at 547bc08: F2.2 `apply_time_budget` + F3.2 `detail_field` present, `McpServerRuntime::time_budget()`=200, tools/list=38, error_catalog has `wpcc_operation_timeout`; ext `/health`=401, `/mcp`=500 (pre-existing missing-token-no-status quirk, structured body, not a fatal — candidate future fix: give MCP missing-token error an explicit 401). **Nothing about the live site is broken.**
- **MANUAL DEPLOY FALLBACK (if ever needed):** SSH = Hostinger `ssh -p 65002 u916998506@72.62.68.183` (pw `DevAdmin#26` — **ROTATE, it's in chat**). Plugin dir `~/domains/mosharafmanu.com/public_html/wp-content/plugins/wp-command-center`; WP root `~/domains/mosharafmanu.com/public_html`; wp at `/usr/local/bin/wp`. Deploy = `cd <plugindir> && git fetch origin && git reset --hard origin/main && wp cache flush --path=<wproot>` (capture `wp plugin is-active wp-command-center` first; reactivate after if needed — reset-race guard). Normal path is just `git push origin main` (SSH auto-deploy, above).
- **CODE PARITY VERIFIED:** `includes/` git tree hash `2a730c39…` was identical on local, GitHub, and server at 547bc08; everything since is CI/doc only. Server `mosharafmanu-automation` folder = LEGACY content seed/fix WP-CLI scripts (148 php, run via `wp eval-file`), NOT a deploy mechanism — superseded by the WPCC plugin runtime; can be archived.
- **Regression gate fact:** valid pre-deploy gate = **SERIAL** `tests/run.sh --tier T2` (~32m). `--tier T2 -j6` is INVALID (168 false net-new; see ⚠️ under Tiered Regression Runner).
- **Remediation sprint** against `WPCC-Acceptance-Test-Report-2026-06-14.md` — **DONE:** **F3.1 ✅ `13bf288`**, **F6.1 ✅ `c3bebbe`**, **F6.2 ✅ `9aa5f81`**, **F2.2 ✅ `6ffcf20`**, **F3.2 ✅ `a4b1feb`**. **F2.1 RECLASSIFIED → "requires production reproduction"** (not reproducible on dev; report's blocking-HTTP hypothesis disproven by code — `seo_get` is pure `get_post_meta`, ~1.35s on dev; env gap = prod Rank Math/LiteSpeed vs dev Yoast/Apache). Each fix: audit → root cause → acceptance test (reproduces the gap) → T1 changed regression (0 net-new) → local commit.
...
| **T2** | ONLY before push/deploy | full 88, **SERIAL (no -j)**, net-new vs baseline | full (~32m serial) |

Commands: `tests/run.sh --tier T0 --changed` · `--tier T1 --changed` (or `--runtime NAME`) · `--tier T2` (**serial — do NOT use -j**) · add `--list` to preview selection. Exit code is non-zero only on **net-new** failures or lint failure (the chronic 24 baseline is OK).
- **⚠️ `--tier T2 -j6` IS NOT A VALID GATE (proven 2026-06-14).** A `-j6` T2 produced **168 false net-new failures** across ~45 suites; ALL representative suites passed standalone 0/0 (plugin-runtime 58/0, woocommerce 117/0, seo 24/0, mcp 42/0, bulk 41/0; claude-integration standalone = exactly its baseline 4). The suite set is NOT concurrency-safe against a single shared WP instance — suites mutate shared global state (posts, options, security_mode, theme, tokens, DB rows) and collide. **Run T2 serially.** (If a parallel mode is ever needed, each worker needs an isolated WP/DB.)
...
## Remediation Sprint (WPCC-Acceptance-Test-Report-2026-06-14) — ✅ COMPLETE (2026-06-14)

All 🔴/🟠 findings fixed locally & tested (F3.1, F6.1, F6.2, F2.2, F3.2); F2.1 deferred → "requires production reproduction". No finding in progress. Methodology, one finding at a time; each: audit → root cause → fix → acceptance test (reproduces the exact failure) → regression → local commit. Planned order was F3.1, F6.1, F6.2, F2.1, F2.2, F3.2 (F2.1 pulled out mid-sprint and deferred).
...
- **F2.1 — RECLASSIFIED → "requires production reproduction" (not started, deferred by owner 2026-06-14).** Report's hypothesis ("blocking loopback/external HTTP in the SEO module") is WRONG per code audit: `seo_get` = `SeoRuntimeManager::seo_get` → `resolve_post`(get_post) + `SeoProvider::read` = pure `get_post_meta`; no `wp_remote_*`/loopback/locking. MCP `tools/call` goes straight to `OperationExecutor`, NOT through the lone loopback (`McpServerRuntime::fetch_rest`, verbose resource reads only). `AuditLog::record` uses `file_put_contents(FILE_APPEND|LOCK_EX)` (microsecond lock). LIVE on dev: `seo_get` post 14171 = ~1.35s. Env gap = prod (Rank Math 1.0.272 + LiteSpeed, limited PHP workers) vs dev (Yoast + Apache/MAMP). Reopen ONLY with reproducible prod evidence (PHP slow log, Site Health loopback test, Rank Math request trace). DO NOT implement speculative fixes.
- **F2.2 ✅ — MCP/PHP timeout mismatch — FIXED locally.** Root cause: no server-side execution budget; PHP `max_execution_time` (480s prod) >> MCP client timeout (~240s), so an op in the 240–480s window keeps writing after the client abandoned it = silent partial write + "stuck started" (the F7.3 mechanism). Fix: `McpServerRuntime::time_budget()` (filterable `wpcc_mcp_time_budget`, default 200s, clamped < 240; 0 disables) + `apply_time_budget()` (LOWERS PHP limit to budget only when 0/unlimited or higher — never raises a host's lower cap; registers shutdown handler) + structured `wpcc_operation_timeout` + `mcp.timeout` terminal audit event; wired into `McpRestApi::handle_mcp`; error-catalog entry. `test-mcp-timeout-budget-f22.sh` 18/18. T1 changed = 419/0/0 net-new.
- **F3.2 ✅ — Nested ACF read — FIXED locally.** Root cause: read serializer `summarize_field` emitted only key/label/name/type/required/parent. Sub-fields persist as separate field posts parented to the container; `acf_get_fields()` resolves children only when passed the field ARRAY (it treats a key string as a group → returns []). Confirmed sub-fields DO persist (DB child posts) — pure read bug, not F3.3 persistence. Fix: recursive `detail_field()` (repeater/group → `sub_fields`; flexible_content → `layouts` each with `sub_fields`, grouped by child `parent_layout`; depth-guarded) wired into `acf_group_get`/`acf_field_get`/`acf_field_list`. `summarize_field` LEFT FLAT (rollback before-state feeds `acf_update_field` and must stay a plain field array). `json_export` left as-is (import round-trip risk). `test-acf-nested-read-f32.sh` 18/18 (incl. MCP parity + guard that plain fields gain no nested keys). T1 changed = 211/0/0 net-new.
- (Lower-priority report findings F1.x/F3.4 [field_count:0 — adjacent in summarize_group, left untouched]/F3.5/F6.3/F6.4/F7.x logged in the report but out of this sprint's scope unless re-prioritized.)
- **Sprint 🟠/🔴 findings COMPLETE** (F3.1, F6.1, F6.2, F2.2, F3.2 done; F2.1 deferred to prod repro). **NEXT when resumed:** either owner re-prioritizes a lower-priority report finding, or resume roadmap-continuation **STEP 100.3** (`media_enhance` op). Source of truth: `/Volumes/Tools/MyProductions-Markdown/WPCC/WPCC-Acceptance-Test-Report-2026-06-14.md`.
...
- **ROADMAP COMPLETE (STEPs 89–98)** and DEPLOYED to production (pushed `586f38c`, 2026-06-14; verified 38 ops/MCP tools, plugin active).
- **STEP 100 — Media Enhancement Runtime (in progress, phased):** Design `b77d1c5` + plan `798e1e6` (`.ai/steps/STEP-100-*`). Owner-locked constraints: GD/Imagick only, no shell binaries, WebP first, `keep_original=true`, audit-first. Build order P1 100.1→100.2, P2 100.3→100.4→100.5, P3 100.6→100.7, P4 100.8→100.9 (each a standalone tested local commit).
  - **100.1 MediaSnapshot — COMPLETE locally:** byte-for-byte file snapshot service (`includes/Operations/MediaSnapshot.php`); own uploads store (core snapshot engine is PathGuard-locked to code dirs); `media_snapshot_create/restore/verify/list` actions on `media_manage` (no new op → operation_map stays 32). `test-media-snapshot-step100-1.sh` **23/23**. Doc `.ai/steps/STEP-100.1-MEDIA-SNAPSHOT.md`.
  - **100.2 media_replace fix — COMPLETE locally:** found a 2nd bug via runtime testing — `media_handle_sideload(...,['ID'=>$id])` ignores the ID and creates a new orphan attachment (original replace never replaced). Fixed both: replace now writes new bytes **in place** (preserves attachment ID + URL) after capturing a MediaSnapshot, rejects non-images, aborts if snapshot can't be taken; `rollback()`/`restore_media()` gain a `replace` case → byte-for-byte restore; new `media_replace_verify` (md5/dims/url). `test-media-replace-step100-2.sh` **20/20** (incl. orphan-count guard). Doc `.ai/steps/STEP-100.2-MEDIA-REPLACE-ROLLBACK.md`.
  - **100.3 media_enhance foundation — COMPLETE locally (`d30844f`):** NEW read-only `media_enhance` operation (`includes/Operations/MediaEnhancementRuntimeManager.php` — registry + manager in one file, like Reporting). 5 diagnostic actions: `media_enhance_capabilities` (GD/Imagick + WebP/AVIF encode probe, no shell), `image_sizes_list` (incl. theme `add_image_size`; core vs additional), `image_size_usage_audit` (per-size on-disk coverage; limit 500/max 5000), `image_size_recommendations` (unused/oversized>2048px), `image_size_verify` (present/missing/**not_applicable** when a size exceeds the original). Wiring: OperationExecutor handler, OperationRegistry def (all diagnostic), CapabilityRegistry `media_enhance→media.manage` (**operation_map 32→33; now 39 ops/39 MCP tools**, updated `test-capability-runtime.sh`), RestApi read route `/operations/media_enhance/run` (`require_read`) + handler + manifest + error-catalog `wpcc_invalid_media_enhance_action`. MCP parity automatic. `test-media-enhance-step100-3.sh` **33/33**; T1 `--changed` = 352 passed, **0 net-new**. Doc `.ai/steps/STEP-100.3-MEDIA-ENHANCE-FOUNDATION.md`.
  - **100.4 responsive image audit — COMPLETE locally (`3db625c`):** 4 NEW read-only actions on the **existing** `media_enhance` op (NO new op → operation_map stays 33; MCP parity automatic): `srcset_verify` (WordPress srcset/sizes metadata + candidate count; `has_srcset`=≥2 candidates), `responsive_image_audit` (per-attachment readiness w/ `media_id` = size classification + srcset + metadata completeness + `responsive_ready` verdict + structured recommendations; **library-wide aggregate** without `media_id`), `missing_sizes_audit` (library-wide; attachments missing applicable/regenerable sizes), `image_size_context_audit` (**oversized** = original ≥1.5× largest registered width; **undersized** = smaller than registered sizes it can't fill since WP won't upscale → `unfillable_sizes`). Refactored 100.3 `size_verify` to share `classify_sizes()`/`resolve_image_id()` (100.3 suite still 33/33). No new error codes. **GOTCHA:** WP `big_image_size_threshold` (2560px default) downscales large uploads — the acceptance test disables it via mu-plugin to seed a true oversized (6144px) original, and registers a size POST-upload (mu-plugin) for the deterministic "missing" case. `test-media-enhance-responsive-step100-4.sh` **33/33**; T1 `--changed` = 281 passed, **0 net-new**. Doc `.ai/steps/STEP-100.4-RESPONSIVE-IMAGE-AUDIT.md`.
  - **100.5 thumbnail regeneration — COMPLETE locally (`adc18e6`):** FIRST reversible **WRITE** on `media_enhance` (NO new op → operation_map stays 33; capability `media.manage`). Actions: `thumbnail_regenerate`/`thumbnail_regenerate_attachment` (`mode` missing[default]|all), `thumbnail_regenerate_batch` (cursor + `batch_id`, limit 20/max 50), `thumbnail_verify` (read). Rollback via NEW route `POST /operations/media_enhance/rollback` + public `rollback()` (also `OperationExecutor::rollback` dispatch). **Safety:** `MediaSnapshot::capture` BEFORE any write (abort `wpcc_thumbnail_snapshot_failed`); `mode=missing` no-op when nothing missing (no snapshot/write); never generates not_applicable/upscale sizes; post-verify → on failure restore snapshot + delete created files (`wpcc_thumbnail_regenerate_failed`, no partial state); rollback deletes regen-created files then restores original+sizes+metadata **byte-for-byte**. Own store `wpcc_media_enhance_rollbacks` (cap 100). Per-action risk: regen=`medium` (security-mode approval applies), reads=`diagnostic`; REST route uses per-action permission `require_media_enhance` (writes→full token, reads→read token). **GOTCHAS:** (1) PSR-4 multi-class-per-file — `MediaEnhancementRegistry` isn't autoloadable alone (shares the manager file), so `require_media_enhance` **hardcodes** the write-action list (calling `Registry::get_risk` there 500'd every request until fixed). (2) a regen size that shares a filename with a core size (custom 300×225 == `medium` on a 4:3 image) creates no new file → restored via snapshot (tests must use a UNIQUE dim, e.g. 412×309, to exercise created-file deletion). (3) failure forced in tests via `add_filter('wp_image_editors', fn()=>[])`. `test-media-enhance-regenerate-step100-5.sh` **39/39**; 100.3/100.4 re-run green; T1 `--changed` = 438 passed, **0 net-new**. Doc `.ai/steps/STEP-100.5-THUMBNAIL-REGENERATION.md`.
  - **100.6 WebP audit + additive generation — COMPLETE locally (`23f5774`):** 4 actions on `media_enhance` (NO new op → operation_map stays 33). `webp_audit` (R, library coverage % + capability probe gd/imagick/webp_encode), `webp_verify` (R, per-attachment `.webp` presence + smaller-or-equal + supported-mime), `webp_generate` (RW), `webp_generate_batch` (RW cursor, **partial-success** generated/no_action/unsupported/failed + `batch_id`). **Additive**: writes `<file>.webp` sidecars BESIDE originals (e.g. `image-300x225.jpg.webp`) — never modifies/replaces/deletes originals (md5-asserted). **Safety:** capability-gated fail-closed (`wpcc_image_lib_unavailable`; filterable via `wpcc_media_webp_encode_available` — operator/test kill-switch); `MediaSnapshot::capture` before any write (`wpcc_webp_snapshot_failed`); skips existing `.webp` (no dupes; fully-covered → `no_action`, no write); JPEG/PNG only (`wpcc_webp_unsupported_mime`; batch counts unsupported & continues); missing source → `wpcc_media_no_files`; verifies; **reuses the 100.5 rollback machinery** (`store_rollback` mode `webp_generate`, `created_files`=generated `.webp`; `/media_enhance/rollback` deletes them). `require_media_enhance` write-list += `webp_generate`/`webp_generate_batch`. **GOTCHA (again):** PSR-4 multi-class — a manager-only const (`WEBP_SOURCE_MIMES`) first placed in the Registry class and referenced via `self::` from the manager → fatal Undefined constant; moved to the manager. `test-media-enhance-webp-step100-6.sh` **37/37**; 100.3/100.4/100.5 re-run green; T1 `--changed` = 438 passed, **0 net-new**. Doc `.ai/steps/STEP-100.6-WEBP-AUDIT-GENERATION.md`. *(Env note: AMPPS Apache+MySQL went down mid-session — REST 000 / DB-connection error — owner restarted; transient local-stack outage, not code.)*
  - **100.7 image optimization — COMPLETE locally (`90aa12f`):** 4 actions on `media_enhance` (NO new op → operation_map stays 33). `image_optimize_audit` (R, library stats + heuristic estimate + capability), `image_optimize_verify` (R, per-image eligibility/estimate), `image_optimize` (RW), `image_optimize_batch` (RW cursor, **partial-success**, resume-safe). Re-encodes JPEG/PNG/WebP at `quality` (default 82, clamp 1–100) to cut bytes **WITHOUT changing dimensions** (GD/Imagick via `wp_get_image_editor`; no shell). **Safe-commit:** re-encode each file (original+sizes) to a TEMP first (originals untouched) → `MediaSnapshot::capture` → commit only files with significant savings (≥5% **and** ≥1KB) via atomic `rename`; nothing significant → `no_action` (no snapshot/write). Capability-gated fail-closed (`wpcc_image_lib_unavailable`; filter `wpcc_media_optimize_available`). Unsupported GIF/SVG/AVIF → `wpcc_optimize_unsupported_mime` (SVG → `wpcc_not_an_image`, not a WP image type). **Reuses 100.5 rollback** (mode `image_optimize`; `/media_enhance/rollback` restores original bytes+metadata **byte-for-byte**). `require_media_enhance` write-list += `image_optimize`/`image_optimize_batch`. **SCOPE:** reversible-only per the prompt (always snapshot-backed); did NOT implement the plan doc's optional `keep_original=false` destructive path (deferred). **GOTCHA:** `WP_Image_Editor::save($path,$mime)` derives the output filename from the **mime**, not the path extension (`file.jpg.wpccopt` → `file.jpg.jpg`) — temp now keeps the correct extension and code uses `$saved['path']`. `test-media-enhance-optimize-step100-7.sh` **45/45**; 100.3–100.6 re-run green; T1 `--changed` = 570 passed, **0 net-new**. Doc `.ai/steps/STEP-100.7-IMAGE-OPTIMIZATION.md`.
  - **100.8 usage analysis — COMPLETE locally (`83c5d9f`):** read-only **cleanup intelligence** (the strategic control-plane step, not optimization). NEW `includes/Operations/MediaUsageResolver.php` (own file) — `references($id)`/`classify($id)`/`attachment_ids($limit)`. Scans **9 sources**: WP featured (`_thumbnail_id`), content (`wp-image-N` + file basenames), Gutenberg blocks (`parse_blocks` id/ids/mediaId), Woo (`_thumbnail_id` + `_product_image_gallery` FIND_IN_SET), ACF fields (value/serialized + `_{key}`=`field_…` companion), ACF options (`options_`/`_options_` `field_…`), Elementor (`_elementor_data` `"id":N`), theme_mods (`custom_logo` + URL basenames), options (`site_icon`/`site_logo`/generic URL LIKE). **Classify:** active (live host publish/private/future or live config) / indirect (draft-only or loose URL) / unused (`cleanup_candidate`) / orphaned (file missing). CONSERVATIVE — when in doubt = referenced, never unused (cleanup safety). Actions on `media_enhance` (all diagnostic; NO new op → operation_map stays 33; MCP parity auto): `media_usage_scan`, `media_usage_report`, `unused_media_find`, `orphaned_media_find`. `resolve_attachment_id` accepts ANY mime. Bounded `limit` default 150/max 1000. No mutations/rollback. **GOTCHAS:** (1) content query used alias `p.` while FROM lacked the alias → "Unknown column p.post_content" silently missed content/block/draft refs (false-negative = unsafe) — fixed to `FROM {posts} p`. (2) `wp eval-file` needs a `<?php` opener or it echoes the file as text. `test-media-enhance-usage-step100-8.sh` **40/40**; 100.3–100.7 re-run green; T1 `--changed` = 570 passed, **0 net-new**. Doc `.ai/steps/STEP-100.8-USAGE-ANALYSIS.md`.
  - **100.9 guarded reversible cleanup — COMPLETE locally (`da3fcc9`):** NEW `unused_media_cleanup` on `media_enhance` (NO new op → operation_map stays 33; risk **high**; full-token). **Snapshot → Trash → Verify, NEVER permanent.** Flow: resolve → **re-run `MediaUsageResolver::classify` at execution time** → `cleanup_exclusion` → `MediaSnapshot::capture` → emulate WP trash (`post_status='trash'` + `_wp_trash_meta_*`, MEDIA_TRASH-independent so WP-Admin Restore works) → verify (auto-restore on failure). **DestructiveGuard `CLEANUP_MEDIA`** (confirm+phrase+reason+media_id) in every mode (new `media_enhance` case + `PHRASE_CLEANUP`). **Hard-excludes:** still-referenced (active OR indirect = draft-only + revision), Woo product images (parent product/variation), theme assets (custom_logo/site_icon/site_logo), `wpcc_media_cleanup_protected` filter (unknown/code/CSS refs). **Revision blind spot FIXED:** resolver now scans `post_type=revision` (main query excludes `inherit`) → revision refs = indirect = protected. **Rollback extended** (`store_rollback($extra)`; `rollback()` cleanup branch): restores file bytes + metadata + `_wp_attached_file` + `post_status` + `post_parent` + clears trash meta; idempotent; reconciles with manual WP-Admin restore. **BIG correctness fix (also affects 100.8):** resolver's generic option URL scan matched WPCC's OWN snapshot store (`wpcc_media_file_snapshots` embeds basenames) → any snapshotted attachment looked "referenced" and could never be classed unused (2nd cleanup-after-rollback refused) — excluded `wpcc_*` options from the URL scan. **TEST NOTE:** assert RAW `$post->post_status` (not `get_post_status()`, which maps attachment `inherit`→`publish`). `test-media-enhance-cleanup-step100-9.sh` **31/31**; 100.3–100.8 + destructive-guardrails (21/21) re-run green; T1 `--changed` = 570 passed, **0 net-new**. Doc `.ai/steps/STEP-100.9-GUARDED-CLEANUP.md`.
  - **★ STEP 100 CORE ROADMAP (100.1 → 100.9) COMPLETE — all committed LOCALLY, NOT pushed/deployed (prod still at STEP 98 `586f38c`).** media_enhance carries 25 actions; operation_map = 33; 39 ops/39 MCP tools. **No permanent media purge** (intentionally out of scope — any future purge MUST be a separate op with its own DestructiveGuard + approval). **Next options (owner choice):** (a) **deploy the 100.x batch** (`git push origin main`; run **serial T2** first as the pre-deploy gate); (b) **strategic backlog** — pull forward `report_media` + cross-runtime image audits (`product_image_audit`/`acf_image_audit`/`elementor_image_audit`) + optimizer-interop detection; **defer** AVIF / cross-runtime delegating-writes / media_duplicate.
...
- **Outstanding risks:** (1) the deploy's in-place `git reset --hard` can race-deactivate the live plugin (mitigated: deploy.yml captures + reactivates active state). (2) in-band `{error:true}` manager convention over REST. (3) media delete rollback needs `MEDIA_TRASH`. (4) final-validation flakes transiently back-to-back.
- *(historical) STEPs 93/94 WooCommerce now COMPLETE & deployed — see the roadmap table above.*

---

**RESUME POINTER → see "►► SESSION STATE — START HERE" at the top of this file. (Historical note: the roadmap 89–98 is complete & deployed. The remediation sprint is now COMPLETE — all 🔴/🟠 findings fixed locally; F2.1 deferred to production reproduction; no finding in progress. Awaiting owner direction before any new thread.) Licensing / Free-Pro gating remains separately unscheduled.**
...
STEPs 78–83 are **complete and deployed to mosharafmanu.com**. CI/CD is live: every `git push` to `main` auto-deploys via GitHub Actions SSH deploy (`git fetch` + `reset --hard origin/main` + `wp cache flush`; see top SESSION STATE bullet). No manual deployment needed going forward.
...
- CI/CD live: `git push origin main` → GitHub Actions SSH deploy → `git reset --hard origin/main` + cache flush
