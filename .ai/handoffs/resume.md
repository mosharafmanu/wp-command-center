...
- **▶ RESUME HERE — ONE OPEN DECISION (everything else is DONE & LIVE):** How to make GitHub Actions **auto-deploy** reliable. Production is ALREADY current and healthy (`64fa67b`); this is purely CI convenience, not blocking. The webhook deploy is **intermittently 403'd by the host WAF on GitHub-runner IPs** (token + custom-UA are correct; run #19 `4ca3bd6` ✓ but run #20 `64fa67b` ✗). Owner was asked to pick a path and wants to **clarify first** before choosing. The three options on the table:
  1. **SSH-based auto-deploy (recommended)** — GH Action SSHes in and runs `git pull` directly, bypassing the web/WAF entirely (owner's earlier idea). I'd add a deploy key on the server (additive/reversible) + owner adds ONE GH secret (private key) + I rewrite `deploy.yml`.
  2. **Webhook + retry/backoff** — keep webhook, retry transient 403s. Quick, no new secret, may still fail under a sustained WAF window.
  3. **Keep webhook, deploy manually via SSH when needed** — works reliably; I've done it repeatedly this session.
  When resuming, re-ask the owner's clarifying questions, then implement their choice. **Do not start any other work** (remediation/STEP 100) unless owner redirects.
