...
- **✅ STEP 100 (Media Enhancement) CORE ROADMAP 100.1–100.9 COMPLETE + DEPLOYED TO PRODUCTION (2026-06-15).** Production `mosharafmanu.com`, `origin/main`, and local are all at **`366ce5a`** (server working tree clean, plugin ACTIVE, plugin v0.1.0). Batch pushed = 100.3–100.9 (`d30844f`→`da3fcc9`) + legacy-gallery-ids resolver fix (`279f6e9`) + validation plan/report/artifacts (`366ce5a`). Pre-deploy gate = **SERIAL T2 CLEAN** (3533 passed / 24 chronic baseline / **net-new 0**). Post-deploy verified: 39 ops, `media_enhance` present (26 actions incl `unused_media_cleanup`/`media_usage_scan`/`webp_generate`), capability map valid (`media_enhance→media.manage`), ext `/health`=401, MCP `tools/list`=39 with `media_enhance`, external REST read actions all OK (full token), no debug.log errors. `operation_map`=33, 39 ops/39 MCP tools.
- **✅ DEPLOY MODEL CHANGED → PULL-BASED (2026-06-15). Root cause of the push-deploy failure FOUND.** The host (Hostinger) now **blocks inbound SSH from GitHub-hosted runner IPs** — proven: GH Actions `ssh: connect to host 72.62.68.183 port 65002: Connection timed out`; the same host answers a home/allowed IP instantly (`ssh-keyscan` 8 keys <1s, manual SSH works); the deploy key is valid (`ssh-keygen -y` passes). The old push-based `deploy.yml` died silently at `ssh-keyscan` (5s timeout → exit 1 under `set -e`, stderr to `/dev/null`) BEFORE the key was ever used — NOT a key/secret/code issue (the `DEPLOY_SSH_KEY_B64` secret rotated ~15h prior is fine). **NEW deploy = server PULLS from GitHub (outbound not blocked):** server script `~/wpcc-deploy.sh` (git fetch + `reset --hard origin/main` + reactivate-if-active + `wp cache flush`; idempotent, flock-guarded, logs to `~/wpcc-deploy.log`) — INSTALLED + TESTED (deploy + no-op paths both pass). `deploy.yml` converted to a green no-op notice (no more red runs). **⚠️ ONE OWNER STEP PENDING:** add the cron in **hPanel → Advanced → Cron Jobs** (no `crontab` binary over SSH on this host), every 1–2 min: `/bin/bash /home/u916998506/wpcc-deploy.sh >/dev/null 2>&1`. Until that cron exists, deploy with the manual fallback (below) or run `~/wpcc-deploy.sh` over SSH. After the cron is added, `git push origin main` → server pulls within the interval.
...
- **NEXT (owner choice):** (a) fix the auto-deploy CI (`deploy.yml` / run `27531805301`); (b) strategic backlog — `report_media` + cross-runtime image audits (`product_image_audit`/`acf_image_audit`/`elementor_image_audit`) + optimizer-interop + the 6 resolver blind-spot sources; DEFER AVIF / cross-runtime delegating-writes / `media_duplicate`. Permanent media purge intentionally OUT OF SCOPE.
