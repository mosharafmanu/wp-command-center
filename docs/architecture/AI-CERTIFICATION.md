...
All AI clients that connect to SiteRadian go through a unified certification framework. This ensures consistent validation of MCP compatibility, security posture, and operational reliability — regardless of which vendor produced the client.
...
SiteRadian certifies **AI clients** (applications that connect via MCP), not AI models. A single client may use different AI models:
...
This means: certifying "Cursor + Qwen" is the same as certifying "Cursor" — the client handles model selection. SiteRadian interacts with the client, not the model.
...
```
AI Client → MCP → SiteRadian → Capability → Approval → Queue → Execute → Verify → Audit → Rollback
```
