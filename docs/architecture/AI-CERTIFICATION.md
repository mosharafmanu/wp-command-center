...
All AI clients that connect to SiteRadian AI go through a unified certification framework. This ensures consistent validation of MCP compatibility, security posture, and operational reliability — regardless of which vendor produced the client.
...
SiteRadian AI certifies **AI clients** (applications that connect via MCP), not AI models. A single client may use different AI models:
...
This means: certifying "Cursor + Qwen" is the same as certifying "Cursor" — the client handles model selection. SiteRadian AI interacts with the client, not the model.
...
```
AI Client → MCP → SiteRadian AI → Capability → Approval → Queue → Execute → Verify → Audit → Rollback
```
