# Assistant Certification Sprint — SiteRadian AI 1.0.0
...
| Codex CLI 0.153.4 | **CERT_PASS** | Actual owner retest passed using the approval-aware launch: native registration, 42 tools, 7 resources and `system_info`, without changing SiteRadian AI governance. |
...
| Gemini CLI 0.46.0 | **BLOCKED — EXTERNAL ACCOUNT/PROVIDER** | Setup structurally verified; Google rejects this individual Gemini Code Assist client/account and directs the owner to Antigravity. External to SiteRadian AI. |
...
| Cursor | **CERT_PASS** | Retained actual client: auth/init, 42 tools, 7 resources, `system_info` and reload/reconnect. A later High Load/model-account limitation is external to SiteRadian AI. Not Gold. |
...
contract, and cannot reach a localhost SiteRadian AI endpoint. Adding it would require approved
...
|---|---|---|
| `initialize` handshake | **PASS** | protocol `2024-11-05`, serverInfo `SiteRadian AI 1.0.0`, caps `tools`/`resources`/`prompts` |
...
|---|---|---|---|
| **GitHub Copilot** | **29% workplace adoption — #1** | Yes | Absent from SiteRadian AI's list entirely |
...
| **Roo Code** | **ARCHIVED 15 May 2026** | n/a | **Still shipping in SiteRadian AI** |
...
| Assistant | SiteRadian AI currently emits | Reality |
|---|---|---|
...
**SiteRadian AI → Settings → Connections → Access tokens**.
