...
| Gemini CLI 0.46.0 | **BLOCKED — EXTERNAL ACCOUNT/PROVIDER** | Setup contract verified; Google rejects this individual Gemini Code Assist client/account and directs the owner to Antigravity. This is external to SiteRadian. |
...
| Cursor | **CERT_PASS** | Retained actual client: auth/init, 42 tools, 7 resources, `system_info` and reload/reconnect. The latest attempt loaded MCP but model High Load/account availability prevented completion; that is external, not a SiteRadian defect. Never Gold. |
...
is not compatible with SiteRadian’s current local access-token onboarding. Gemini CLI remains
...
```json
{
  ...
  "inputs": [{
    ...
    "description": "SiteRadian token for Example Site",
    "password": true
  }]
}
```

VS Code prompts for that value and stores it securely. SiteRadian does not use OAuth for this
connection. If VS Code shows an OAuth/client-registration screen, cancel it and update
the stored SiteRadian input with a current token.
...
The key an assistant uses to talk to *you* is not the key SiteRadian uses for its own AI
...
individual tool is switched on**. SiteRadian makes no outbound AI call without one.
...
SiteRadian server and 42 tools. Ask for `system_info` and
`report_manage` / `report_site_health`. `agy mcp list` proves registration only;
SiteRadian's browser endpoint test proves server/token behavior, not native connectivity.
